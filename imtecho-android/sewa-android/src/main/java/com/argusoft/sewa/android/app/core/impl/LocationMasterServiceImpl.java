...
public class LocationMasterServiceImpl implements LocationMasterService {
    ...
    @Override
    public void clearLocationMasterTable() {
        try {
    ...
    @Override
    public Integer getLocationLevelByType(String type) {
        ...
        return locationTypeBean.getLevel();
    }
        ...
        if (parent != null) {
            try {
                locationMasterBeans = locationMasterBeanDao.queryBuilder().where()
                        .eq(FieldNameConstants.PARENT, parent).query();
            } catch (SQLException e) {
                Log.e(getClass().getSimpleName(), null, e);
            }
        }
    ...
    @Override
    public List<LocationMasterBean> retrieveLocationMasterBeansBySearchAndType(CharSequence s, List<String> types) {
    ...
    @Override
    public Map<Integer, String> retrieveHierarchyMapWithLocation(LocationMasterBean locationMasterBean) {
        Map<Integer, String> hierarchyMapWithLevelAndId = new HashMap<>();

        if (locationMasterBean == null) {
            return hierarchyMapWithLevelAndId;
        }

        try {
            hierarchyMapWithLevelAndId.put(locationMasterBean.getLevel(), locationMasterBean.getName());
            for (int i = locationMasterBean.getLevel(); i > 1; i--) {
                locationMasterBean = locationMasterBeanDao.queryBuilder().where()
                        .eq(FieldNameConstants.ACTUAL_I_D, locationMasterBean.getParent()).queryForFirst();
    ...
    @Override
    public List<LocationMasterBean> retrieveLocationMasterBeansByParent(Integer parent) {
        if (parent == null) {
    ...
    public LocationMasterBean getPhcLocationFromSelectedLocationId(Integer locationId) {

        if (locationId == null) {
            return null;
        }
        boolean isPhc = false;
        LocationMasterBean location = getLocationMasterBeanByActualId(locationId);

        if (location.getLevel() != null && location.getLevel() > 4) {
            while (!isPhc) {
                location = getLocationMasterBeanByActualId(location.getParent());
                if (location.getType().equalsIgnoreCase("P")) {
                    isPhc = true;
                }
            }
        }
