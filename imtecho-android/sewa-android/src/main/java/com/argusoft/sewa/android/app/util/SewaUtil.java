...
public class SewaUtil {
    ...
    public static void setColor(TextView textView, String colorName) {
        ...
        if (colorName != null && colorName.trim().length() > 0) {
            if (colorName.trim().equalsIgnoreCase(MorbiditiesConstant.RED_COLOR_FULL)
                ...
                color = COLOR_4;
            } else if (colorName.trim().equalsIgnoreCase(GlobalTypes.SEVERELY_MALNOURISHED)||
                    colorName.trim().equalsIgnoreCase(GlobalTypes.MALNOURISHED)) {
                color = COLOR_RED;
            } else if (colorName.trim().equalsIgnoreCase(GlobalTypes.IDEAL)) {
                color = COLOR_GREEN;
            } else if (colorName.trim().equalsIgnoreCase(GlobalTypes.OBESE) ||
                    colorName.trim().equalsIgnoreCase(GlobalTypes.SEVERELY_OBESE)) {
                color = COLOR_2;
    ...
    public static void generateToast(Context context, String msg) {
        if (context != null) {
            if (toast != null) {
                String displayedText = ((TextView) ((LinearLayout) Objects.requireNonNull(toast.getView())).getChildAt(0)).getText().toString();
    ...
    public static LinkedHashMap<String, String> setQrScanFilterData(String scanData) {
        ...
        if (scanData != null) {
            ...
            qrScanFilter.put(FieldNameConstants.IS_QR_SCAN, "true");
        } else {
            qrScanFilter.put(FieldNameConstants.IS_QR_SCAN, "false");
        }
        if ((qrScanFilter.containsKey(FieldNameConstants.HEALTH_ID) || qrScanFilter.containsKey(FieldNameConstants.HEALTH_ID_NUMBER)) &&
                Objects.equals(qrScanFilter.get(FieldNameConstants.IS_QR_SCAN), "true")) {
            qrScanFilter.put(FieldNameConstants.IS_QR_FOR_ABHA, String.valueOf(Boolean.TRUE));
        } else {
            qrScanFilter.put(FieldNameConstants.IS_QR_FOR_ABHA, String.valueOf(Boolean.FALSE));
        }
        return qrScanFilter;
    }
}
