...
public class SyncServiceImpl implements SyncService {
    ...
    @OrmLiteDao(helper = DBConnection.class)
    Dao<HealthInfraTypeLocationBean, Integer> healthInfraTypeLocationBeanDao;
    @OrmLiteDao(helper = DBConnection.class)
    ...
    public void getDataFromServer() {
        try {
            TableUtils.clearTable(stockInventoryBeanDao.getConnectionSource(), StockInventoryBean.class);
    ...
    public LoginRequestParamDetailDataBean generateSyncRequestParams(Map<String, Boolean> metadata) {
        ...
        try {
            ...
            if (gps != null) {
                if (!SewaTransformer.loginBean.getUserRole().equalsIgnoreCase(GlobalTypes.USER_ROLE_FIXED_FMR)
                    ...
                    gps.getLocation();
                    loginRequestParamDetailDataBean.setLongitude(String.valueOf(GPSTracker.longitude));
                    loginRequestParamDetailDataBean.setLatitude(String.valueOf(GPSTracker.latitude));
                }
            }
                ...
                switch (entry.getKey()) {
                    ...
                    case SyncConstant.HEALTH_INFRASTRUCTURE_BEAN:
                        if (healthInfrastructureBeanDao.countOf() > 0) {
                            ...
                            if (versionBeans != null && !versionBeans.isEmpty() && versionBeans.get(0).getValue() != null) {
                    ...
                    case SyncConstant.FAMILY_AVAILABILITY_BEAN:
                        if (familyAvailabilityBeanDao.countOf() > 0) {
                            String string = familyAvailabilityBeanDao.queryBuilder().selectRaw(MAX_MODIFIED_ON).prepareStatementString();
                            String[] result = familyAvailabilityBeanDao.queryRaw(string).getFirstResult();
                            if (result != null && result[0] != null) {
                                loginRequestParamDetailDataBean.setLastUpdateDateForFamilyAvailabilities(Objects.requireNonNull(sdf.parse(result[0])).getTime());
                            }
                        }
                        break;

                    case SyncConstant.STOCK_INVENTORY_BEAN:
                        if (stockInventoryBeanDao.countOf() > 0) {
                            String string = stockInventoryBeanDao.queryBuilder().selectRaw(MAX_MODIFIED_ON).prepareStatementString();
                            String[] result = stockInventoryBeanDao.queryRaw(string).getFirstResult();
                            if (result != null && result[0] != null) {
                                SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSSSSS");
                                try {
                                    Date date = dateFormat.parse(result[0]);
                                    loginRequestParamDetailDataBean.setLastUpdateDateForStockManagement(date.getTime());
    ...
    private void storeSyncData(LoginRequestParamDetailDataBean loginRequestParamDetailDataBean, LoggedInUserPrincipleDataBean loggedInUserPrincipleDataBean) {
        try {
            ...
            //For storing stock inventory
            if (loggedInUserPrincipleDataBean.getStockInventoryDataBeans() != null && !loggedInUserPrincipleDataBean.getStockInventoryDataBeans().isEmpty()) {
                Log.i(TAG, LabelConstants.STOCK_INVENTORY_COUNT + loggedInUserPrincipleDataBean.getStockInventoryDataBeans().size());
                this.storeStockInventoryBeans(loggedInUserPrincipleDataBean.getStockInventoryDataBeans(), loginRequestParamDetailDataBean.getLastUpdateDateForStockManagement());
            }

            //For storing ocr form beans
            if (loggedInUserPrincipleDataBean.getOcrFormDataBeans() != null && !loggedInUserPrincipleDataBean.getOcrFormDataBeans().isEmpty()) {
                Log.i(TAG, LabelConstants.OCR_FORM_COUNT + loggedInUserPrincipleDataBean.getStockInventoryDataBeans().size());
                this.storeOcrFormBeans(loggedInUserPrincipleDataBean.getOcrFormDataBeans(), loginRequestParamDetailDataBean.getLastUpdateDateForStockManagement());
            }
...
//            for storing Health Infra Type Location Details
            this.storeHealthInfraTypeLocationDetails(loggedInUserPrincipleDataBean.getHealthInfraTypeLocationBeanList());
...
//            }
            ...
            if (loggedInUserPrincipleDataBean.getFamilyAvailabilities() != null && !loggedInUserPrincipleDataBean.getFamilyAvailabilities().isEmpty()) {
                this.storeFamilyAvailabilities(loggedInUserPrincipleDataBean.getFamilyAvailabilities());
            }
    ...
    private void storeFamilyAvailabilities(List<FamilyAvailabilityDataBean> familyAvailabilityDataBeans) {
        if (familyAvailabilityDataBeans == null || familyAvailabilityDataBeans.isEmpty()) {
            return;
        }

        List<Integer> actualIds = new ArrayList<>();
        List<FamilyAvailabilityBean> beans = new ArrayList<>();
        for (FamilyAvailabilityDataBean dataBean : familyAvailabilityDataBeans) {
            actualIds.add(dataBean.getActualId());
            beans.add(new FamilyAvailabilityBean(dataBean));
        }
        try {
            DeleteBuilder<FamilyAvailabilityBean, Integer> deleteBuilder = familyAvailabilityBeanDao.deleteBuilder();
            deleteBuilder.where().in("actualId", actualIds);
            deleteBuilder.delete();

            familyAvailabilityBeanDao.create(beans);
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
    }
...
//                            .or().eq(FieldNameConstants.UUID, familyDataBean.getUuid());
//                    familyBeanDeleteBuilder.delete();
//
//                    memberBeanDeleteBuilder.where().eq(FieldNameConstants.FAMILY_ID, familyDataBean.getFamilyId())
//                            .or().eq(FieldNameConstants.MEMBERS_FAMILY_UUID, familyDataBean.getUuid());
//                    memberBeanDeleteBuilder.delete();
//                } else {
//
//                }

                if (familyDataBean.getUuid() != null) {
                    familyBeanDeleteBuilder.where().eq(FieldNameConstants.UUID, familyDataBean.getUuid());
                    familyBeanDeleteBuilder.delete();
                    memberBeanDeleteBuilder.where().eq(FieldNameConstants.MEMBERS_FAMILY_UUID, familyDataBean.getUuid());
                    memberBeanDeleteBuilder.delete();
                }

                if (familyDataBean.getFamilyId() != null) {
                    familyBeanDeleteBuilder.where().eq(FieldNameConstants.FAMILY_ID, familyDataBean.getFamilyId());
                    familyBeanDeleteBuilder.delete();
                    memberBeanDeleteBuilder.where().eq(FieldNameConstants.FAMILY_ID, familyDataBean.getFamilyId());
                    memberBeanDeleteBuilder.delete();
    ...
    private void storeStockInventoryBeans(List<StockInventoryDataBean> stockInventoryDataBeans, Long lastUpdateDate) {
        try {
            if (lastUpdateDate == null) {
                TableUtils.clearTable(stockInventoryBeanDao.getConnectionSource(), StockInventoryBean.class);
            }
            for (StockInventoryDataBean stock : stockInventoryDataBeans) {
                DeleteBuilder<StockInventoryBean, Integer> deleteBuilder = stockInventoryBeanDao.deleteBuilder();
                deleteBuilder.where().eq(FieldNameConstants.MEDICINE_ID, stock.getMedicineId()).and().eq(FieldNameConstants.REQUESTED_BY, stock.getRequestedBy());
                deleteBuilder.delete();
                StockInventoryBean stockInventoryBean = new StockInventoryBean(stock);
                stockInventoryBeanDao.create(stockInventoryBean);
            }
        } catch (SQLException e) {
            Log.e(TAG, "SQL Exception While Inserting Stock Inventory Details", e);
        }
    }

    private void storeOcrFormBeans(List<OcrFormDataBean> ocrFormDataBeans, Long lastUpdateDate) {
        try {
            if (lastUpdateDate == null) {
                TableUtils.clearTable(ocrFormBeanDao.getConnectionSource(), OcrFormBean.class);
            }
            for (OcrFormDataBean ocrFormDataBean : ocrFormDataBeans) {
                DeleteBuilder<OcrFormBean, Integer> deleteBuilder = ocrFormBeanDao.deleteBuilder();
                deleteBuilder.where().eq(FieldNameConstants.FORM_NAME, ocrFormDataBean.getFormName());
                deleteBuilder.delete();
                OcrFormBean ocrFormBean = new OcrFormBean(ocrFormDataBean);
                ocrFormBeanDao.create(ocrFormBean);
            }
        } catch (SQLException e) {
            Log.e(TAG, "SQL Exception While Inserting Ocr forms", e);
        }
    }
            ...
            for (FieldValueMobDataBean fieldValueMobDataBean : fieldValueMobDataBeans) {
                ...
                if (Boolean.TRUE.equals(fieldValueMobDataBean.getActive())) {
                    if (listValueBean == null) {
                        ...
                        listValueBean.setConstant(fieldValueMobDataBean.getConstant());
                        if (fieldValueMobDataBean.getListOrder() != null) {
                            listValueBean.setListOrder(fieldValueMobDataBean.getListOrder());
                        }
...
//    }
    ...
    private void storeHealthInfraTypeLocationDetails(List<HealthInfraTypeLocationBean> healthInfraTypeLocationBeans) {
        if (healthInfraTypeLocationBeans == null || healthInfraTypeLocationBeans.isEmpty()) {
            return;
        }

        Log.i(TAG, LabelConstants.ALL_HEALTH_INFRA_TYPE_LOCATION_COUNT + healthInfraTypeLocationBeans.size());
        try {
            for (HealthInfraTypeLocationBean bean : healthInfraTypeLocationBeans) {
                DeleteBuilder<HealthInfraTypeLocationBean, Integer> deleteBuilder = healthInfraTypeLocationBeanDao.deleteBuilder();
                deleteBuilder.where().eq("healthInfraTypeId", bean.getHealthInfraTypeId());
                deleteBuilder.delete();
            }
            healthInfraTypeLocationBeanDao.create(healthInfraTypeLocationBeans);
        } catch (SQLException e) {
            Log.e(TAG, null, e);
        }
    }
