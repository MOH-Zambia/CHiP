...
public class HighRiskPregnancyActivity extends MenuActivity implements View.OnClickListener, PagingListView.PagingListener {
    ...
    private static final String CHILDREN_DETAIL_SCREEN = "childDetailScreen";
    @Bean
            ...
            switch (screen) {
                ...
                case HIGH_RISK_WOMEN_AND_CHILD_SELECTION_SCREEN:
                    ...
                    if (selectedChildIndex != -1) {
                        ...
                        if (GlobalTypes.FLAVOUR_IMOMCARE.equals(BuildConfig.FLAVOR)) {
                            addChildDetailsScreen();
                        }else{
                        ...
                        bodyLayoutContainer.removeAllViews();
                        if (GlobalTypes.FLAVOUR_IMOMCARE.equals(BuildConfig.FLAVOR)) {
                            visits.add(FormConstants.CAM_ANC);
                        }else{
    ...
    private void addChildDetailsScreen() {
        screen = CHILDREN_DETAIL_SCREEN;
        bodyLayoutContainer.removeAllViews();
        Long dobDate = memberSelected.getDob();
        Long currentDate = System.currentTimeMillis();
        int[] ageYearMonthDayArray = UtilBean.calculateAgeYearMonthDayOnGivenDate(dobDate, currentDate);
        ...
        LinearLayout detailsLayout = MyStaticComponents.getLinearLayout(context, -1, LinearLayout.VERTICAL, new LinearLayout.LayoutParams(MATCH_PARENT, WRAP_CONTENT));
        detailsLayout.addView(MyStaticComponents.generateQuestionView(null, null, context, LabelConstants.CHILD_ID + ":"));
        detailsLayout.addView(MyStaticComponents.generateAnswerView(context, memberSelected.getUniqueHealthId() != null ? memberSelected.getUniqueHealthId() : GlobalTypes.NOT_AVAILABLE));
        detailsLayout.addView(MyStaticComponents.generateQuestionView(null, null, context, LabelConstants.FIRST_NAME + ":"));
        detailsLayout.addView(MyStaticComponents.generateAnswerView(context, memberSelected.getFirstName() != null ? memberSelected.getFirstName() : GlobalTypes.NOT_AVAILABLE));
        detailsLayout.addView(MyStaticComponents.generateQuestionView(null, null, context, LabelConstants.LAST_NAME + ":"));
        detailsLayout.addView((MyStaticComponents.generateAnswerView(context, memberSelected.getLastName() != null ? memberSelected.getLastName() : GlobalTypes.NOT_AVAILABLE)));
        detailsLayout.addView(MyStaticComponents.generateQuestionView(null, null, context, LabelConstants.GENDER + ":"));
        detailsLayout.addView(MyStaticComponents.generateAnswerView(context, memberSelected.getGender() != null ? FullFormConstants.getFullFormOfGender(memberSelected.getGender()) : GlobalTypes.NOT_AVAILABLE));

        detailsLayout.addView(MyStaticComponents.generateQuestionView(null, null, context, LabelConstants.DATE_OF_BIRTH + ":"));
        detailsLayout.addView(MyStaticComponents.generateAnswerView(context, dobDisplay));

        detailsLayout.addView(MyStaticComponents.generateQuestionView(null, null, context, LabelConstants.AGE + ":"));
        detailsLayout.addView(MyStaticComponents.generateAnswerView(context, ageDisplay));

        detailsLayout.addView(MyStaticComponents.generateQuestionView(null, null, context, LabelConstants.BIRTH_WEIGHT+ ":"));
        detailsLayout.addView(MyStaticComponents.generateAnswerView(context, memberSelected.getBirthWeight() != null ? String.valueOf(memberSelected.getBirthWeight()) : GlobalTypes.NOT_AVAILABLE));

        detailsLayout.addView(MyStaticComponents.generateQuestionView(null, null, context, LabelConstants.BIRTH_PLACE + ":"));
        detailsLayout.addView(MyStaticComponents.generateAnswerView(context, memberSelected.getPlaceOfBirth() != null ? memberSelected.getPlaceOfBirth() : GlobalTypes.NOT_AVAILABLE));

        detailsLayout.addView(MyStaticComponents.generateQuestionView(null, null, context, LabelConstants.MOTHER_ID + ":"));
        String motherId = GlobalTypes.NOT_AVAILABLE;
        if (memberSelected.getMotherId() != null) {
            MemberBean mother = fhsService.retrieveMemberBeanByActualId(memberSelected.getMotherId());
            if(mother != null){
                motherId = mother.getUniqueHealthId();
            }
        }
        detailsLayout.addView(MyStaticComponents.generateAnswerView(context, motherId));

        detailsLayout.addView(MyStaticComponents.generateQuestionView(null, null, context, LabelConstants.MOBILE_NUMBER + ":"));
        detailsLayout.addView(MyStaticComponents.generateAnswerView(context, mobileNumber != null ? mobileNumber : GlobalTypes.NOT_AVAILABLE));

        detailsLayout.addView(MyStaticComponents.generateQuestionView(null, null, context,  LabelConstants.IMMUNISATION_GIVEN +":"));
        ...
        detailsLayout.addView(MyDynamicComponents.getImmunisationGivenComponent(this, queFormBean));
    ...
    private String formatDob(Long dob) {
        ...
        return sdf.format(new Date(dob));
    }
    @UiThread
