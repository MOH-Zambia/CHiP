...
public class BloodPresserChangedListener implements View.OnFocusChangeListener, CompoundButton.OnCheckedChangeListener {
    ...
    private static final String SYSTOLIC_DIASTOLIC_CONSTRAINT = "Systolic BP cannot be lower than diastolic BP";
        ...
        if (view.getId() == IdConstants.BLOOD_PRESSURE_BOX_SYSTOLIC_ID) {
            ...
            if (!bln) {
                try {
                    ...
                    } else if (systolicDiastolicConstraint()) {
                        generateToast(SYSTOLIC_DIASTOLIC_CONSTRAINT);
    ...
    private boolean isValid() {
        if (bloodPresser[0] != -1 && bloodPresser[1] != -1) {
            if (isNotValidSys()) {
                ...
                queFormBean.setMandatorymessage(SYSTOLIC_DIASTOLIC_CONSTRAINT);
                return false;
            } else {
    ...
    private boolean systolicDiastolicConstraint() {
        return bloodPresser[0] < bloodPresser[1];
    }
