...
import com.argusoft.sewa.android.app.SewaApplication;
...
public class SewaServiceImpl implements SewaService {
    ...
    private void resetData(UserInfoDataBean userInfoDataBean, LoginBean loginBean) {
        if (loginBean != null) {
            if (!loginBean.getUsername().equals(userInfoDataBean.getUsername())) {
                ...
                clearTable(FamilyAvailabilityBean.class);
    ...
    public List<UploadFileDataBean> getFileWorkLog() {
        ...
        try {
            uploadFileDataBeans = uploadFileDataBeanDao.queryBuilder().where().isNotNull(FieldNameConstants.STATUS).query();
    ...
    @Override
    public List<UploadFileDataBean> getFileWorkLog(String checkSum) {
        List<UploadFileDataBean> uploadFileDataBeans = null;
        try {
            uploadFileDataBeans = uploadFileDataBeanDao.queryBuilder().where().isNotNull(FieldNameConstants.STATUS)
                    .and().eq(FieldNameConstants.CHECKSUM, checkSum).query();
        } catch (SQLException ex) {
            Log.e(getClass().getSimpleName(), null, ex);
        }
        return uploadFileDataBeans;
    ...
    @Override
    public List<UploadFileDataBean> getFileByFormType(String formType, String checksum) {
        if (formType != null && checksum != null) {
            try {
                return uploadFileDataBeanDao.queryBuilder().where().eq(FieldNameConstants.FORM_TYPE, formType)
                        .and().eq(FieldNameConstants.CHECKSUM, checksum).query();
    ...
    @Override
    public List<UploadFileDataBean> getFilesToBeUploaded() {
        try {
            return uploadFileDataBeanDao.queryBuilder().where().eq(FieldNameConstants.PARENT_STATUS, GlobalTypes.STATUS_SUCCESS)
                    .and().eq(FieldNameConstants.STATUS, GlobalTypes.STATUS_PENDING).query();
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return null;
    ...
    public String validateUser(String username, String password, int contextUrlInfo, boolean isRemember) {
        ...
        if (isOnline()) {
            try {
                ...
                if (BuildConfig.FLAVOR.equals(GlobalTypes.FLAVOUR_UTTARAKHAND) ||
                        BuildConfig.FLAVOR.equals(GlobalTypes.FLAVOUR_TELANGANA)) {
        ...
        if (userInfoDataBean != null) {
            ...
            return SewaConstants.SERVER_ISSUE;
        } else {
            ...
            for (int i = 0; i < recordStatusBeans.length; i++) {
                ...
                if (recordStatusBean.getStatus() != null
                    ...
                    if (loggerBean != null) {
                        ...
                        List<UploadFileDataBean> uploadFileDataBeans;
                        uploadFileDataBeans = sewaService.getFileByFormType(loggerBean.getFormType(), loggerBean.getCheckSum());
                        if (uploadFileDataBeans != null && !uploadFileDataBeans.isEmpty()) {
                            for (UploadFileDataBean uploadFileDataBean : uploadFileDataBeans) {
                                uploadFileDataBean.setParentStatus(GlobalTypes.STATUS_SUCCESS);
                                try {
                                    uploadFileDataBeanDao.update(uploadFileDataBean);
                                } catch (SQLException e) {
                                    e.printStackTrace();
                                }
                            }
                        ...
                        if (recordStatusBean.getGeneratedId() != null &&
                                (loggerBean.getFormType().equals(FormConstants.FAMILY_HEALTH_SURVEY) ||
                                        ...
                                        loggerBean.getFormType().equals(FormConstants.FHS_MEMBER_UPDATE_NEW) ||
                                        loggerBean.getFormType().equals(FormConstants.CFHC) ||
                                        loggerBean.getFormType().equals(FormConstants.CAM_FHS) ||
                                        loggerBean.getFormType().equals(FormConstants.FAMILY_FOLDER) ||
                                        loggerBean.getFormType().equals(FormConstants.HOUSE_HOLD_LINE_LIST_NEW) ||
                    ...
                    if (loggerBean != null) {
                        if (recordStatusBean.getGeneratedId() != null && FormConstants.FHS_SHEETS.contains(loggerBean.getFormType())) {
                            loggerBean.setBeneficiaryName(recordStatusBean.getGeneratedId());
                        }
    ...
    public List<FieldValueMobDataBean> getFieldValueMobDataBeanByDataMap(String datamap) {
        try {
            ...
            Collections.sort(listValueBeans, MyComparatorUtil.LIST_VALUE_BEAN_ORDER_COMPARATOR);
    ...
    public void storeException(Throwable ex, String exceptionType) {
        ...
        if (s.contains("android.jwt.DecodeException")) {
            sheetVersion = SewaApplication.getEcgToken();
        }
    ...
    public void updateAnswerBeanTokensAndUserIdsOnce() throws RestHttpException {
        try {
            ...
            for (StoreAnswerBean storeAnswerBean : answerBeansWithoutUserId) {
                if (storeAnswerBean.getAnswerEntity().equals(FormConstants.NCD_ECG_TOKEN)) {
                    storeAnswerBean.setToken(SewaTransformer.loginBean.getUserToken());
                    ...
                    storeAnswerBeanDao.update(storeAnswerBean);
                }
    ...
    public void uploadMediaToServer(UploadFileDataBean bean) {
        try {
            String path = null;
            if (bean.getFilePath() != null) {
                path = bean.getFilePath();
            } else {
                path = SewaConstants.getDirectoryPath(context, SewaConstants.DIR_IMAGE) + bean.getFileName();
            }
            File file = new File(path);
            bean.setToken(SewaTransformer.loginBean.getUserToken());
            RecordStatusBean recordStatusBean = sewaServiceRestClient.uploadMediaToServer(bean, file);
            if (recordStatusBean != null) {
                bean.setStatus(recordStatusBean.getStatus());
                uploadFileDataBeanDao.update(bean);
//                if (file.exists() && bean.getFilePath().contains("Android/data")) {
//                    android.util.Log.e(getClass().getSimpleName(), "deleting File  : " + bean.getFileName());
//                    file.delete();
//                }
            }
        } catch (Exception ex) {
    ...
    public void uploadAllMediaToServer() {
        try {
            List<UploadFileDataBean> mediaBean = sewaService.getFilesToBeUploaded();
            if (mediaBean != null && !mediaBean.isEmpty()) {
                for (UploadFileDataBean bean : mediaBean) {
                    Log.d("SYNC", "***1");
                    uploadMediaToServer(bean);
                    Log.d("SYNC", "***2");
                }
                Log.d("SYNC", "***3");
            }
    ...
    public void createUploadFileDataBean(QueFormBean queFormBean, String formType, String memberId, String checkSum, String memberUuid) {
        ...
        for (String s : fileUniqueId) {
            String fileName = SharedStructureData.imageFilesToUpload.get(s);
            assert fileName != null;
            String image = fileName.trim();
            UploadFileDataBean uploadFileDataBean = new UploadFileDataBean();
            uploadFileDataBean.setCheckSum(checkSum);
            ...
            uploadFileDataBean.setFormType(formType);
            uploadFileDataBean.setFileName(image);
            uploadFileDataBean.setUniqueId(s);
            uploadFileDataBean.setRecordUrl(WSConstants.CONTEXT_URL_TECHO);
            uploadFileDataBean.setFilePath(SewaConstants.getDirectoryPath(context, SewaConstants.DIR_IMAGE) + image);
            uploadFileDataBean.setStatus("P");
            uploadFileDataBean.setParentStatus("P");
            uploadFileDataBean.setNoOfAttemp(0);
            ...
            try {
                uploadFileDataBeanDao.create(uploadFileDataBean);
            } catch (SQLException e) {
    ...
    public void createFamilyBean(HouseHoldLineListMobileDto houseHoldLineListMobileDto, MemberBean memberBean) {
        ...
        for (HouseHoldLineListMobileDto.MemberDetails memberDetails : houseHoldLineListMobileDto.getMemberDetails()) {
            ...
            if (Boolean.TRUE.equals(memberBean.getFamilyHeadFlag()) && memberBean.getGender().equalsIgnoreCase("F")) {
                femaleHofMember = memberBean;
            }

            if (Boolean.TRUE.equals(memberBean.getFamilyHeadFlag()) && memberBean.getGender().equalsIgnoreCase("M")) {
                maleHofMember = memberBean;
            }

            if ("WIFE".equalsIgnoreCase(memberBean.getRelationWithHof()) ||
                    "HUSBAND".equalsIgnoreCase(memberBean.getRelationWithHof())) {
                hofHusbandOrWifeMember = memberBean;
            }

            if (femaleHofMember != null && hofHusbandOrWifeMember != null) {
                femaleHofMember.setHusbandUUID(hofHusbandOrWifeMember.getMemberUuid());
            }

            if (maleHofMember != null && hofHusbandOrWifeMember != null) {
                hofHusbandOrWifeMember.setHusbandUUID(maleHofMember.getMemberUuid());
            }
            MemberBean newlyAddedMemberBean = sewaFhsService.retrieveMemberBeanByUUID(memberDetails.getMemberUuid());
            memberLoopIdMap.put(loopId, newlyAddedMemberBean);
            loopId++;
        }
    ...
    private void motherChildRelation(String motherOf, Map<Integer, MemberBean> membersWithLoopIdMap) {
        ...
        try {
            for (Map.Entry<String, String> entry : motherChildMap.entrySet()) {
                MemberBean childEntity = membersWithLoopIdMap.get(Integer.parseInt(entry.getKey()));
                if (childEntity == null) {
                    continue;
                }
    ...
    private void husbandWifeRelation(String husbandOf, Map<Integer, MemberBean> membersWithLoopIdMap) {
        ...
        try {
            for (Map.Entry<String, String> entry : husbandWifeMap.entrySet()) {
                MemberBean wifeEntity = membersWithLoopIdMap.get(Integer.parseInt(entry.getKey()));
                if (wifeEntity == null) {
                    continue;
                }
                if (!entry.getValue().equals("-1")) {
                    MemberBean husbandEntity = membersWithLoopIdMap.get(Integer.parseInt(entry.getValue()));
                    if (husbandEntity != null) {
                        wifeEntity.setHusbandUUID(husbandEntity.getMemberUuid());
                        memberBeanDao.update(wifeEntity);
                    }
                } else {
                    wifeEntity.setHusbandUUID(null);
                }
            }
        } catch (SQLException e) {
            throw new RuntimeException(e);
    ...
    @Override
    public void createMemberBean(HouseHoldLineListMobileDto.MemberDetails
                                         memberDetails, MemberBean memberBean, FamilyBean familyBean) {
        HouseHoldLineListMobileMapper.convertMemberDetailsToMemberBean(memberDetails, memberBean, familyBean, false);
        try {
            memberBeanDao.create(memberBean);
        } catch (SQLException e) {
            Log.e(getClass().getSimpleName(), e.getMessage(), e);
        }
    }
    ...
    @Override
    public void updateMemberByUniqueHealthId(HouseHoldLineListMobileDto.MemberDetails
                                                     memberDetails, MemberBean memberBean, FamilyBean familyBean) {
        try {
