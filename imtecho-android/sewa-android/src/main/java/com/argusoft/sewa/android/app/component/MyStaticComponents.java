...
import com.argusoft.sewa.android.app.qrscanner.BarcodeScanningActivity;
...
public class MyStaticComponents {
    ...
    public static MaterialTextView getMandatoryTextMaterialTextView(Context context, String text, int id, int style, boolean isCenterAligned) {
        ...
        if (text == null || text.length() == 0) {
            ...
            String label = UtilBean.getMyLabel(text);
            String star = "<font color=#cc0029> *</font>"; // for appending red star at end of mandatory questions in form
            materialTextView.setText((Html.fromHtml(label.concat(star))));
        }
        if (id != -1) {
            materialTextView.setId(id);
        }
    ...
    public static MaterialTextView getMaterialTextViewWithCustomPadding(Context context, String text,
                                                                        int id, int style, boolean isCenterAligned,
        ...
        materialTextView.setPadding(paddingLeft, paddingTop, paddingRight, paddingBottom);
        materialTextView.setLayoutParams(layoutParams);
        return materialTextView;
    }
    ...
    public static MaterialTextView generateMandatoryQuestionView(String helpVideo, String hint, final Context context, String question) {
        if (question == null || question.length() == 0) {
            return null;
        } else {
            MaterialTextView textView = MyStaticComponents.getMandatoryTextMaterialTextView(context, question, -1, R.style.CustomQuestionView, false);
            if (helpVideo != null && !helpVideo.isEmpty() && textView != null) {
                addHelpVideo(helpVideo, textView, context, question);
            }
//            if (hint != null && !hint.isEmpty() && textView != null) {
...
//        final Tooltip tooltip = builder.build();
        ...
        View alertView = LayoutInflater.from(context).inflate(R.layout.bmi_color_code, null);
    ...
    public static TextInputLayout getFilteredEditText(Context context, String hint, int id, int length, int inputType, String blockedChars) {
        TextInputLayout textInputLayout = (TextInputLayout) LayoutInflater.from(context).inflate(R.layout.text_input_layout, null);
        textInputLayout.setEndIconOnClickListener(v -> {
            if (textInputLayout.getEditText() != null) {
                textInputLayout.getEditText().setText(null);
        ...
        InputFilter filter = new InputFilter() {
            ...
            private boolean isCharAllowed(char c) {
                return blockedChars == null || !blockedChars.contains(String.valueOf(c));
            }
        };
        if (id != -1) {
            textInputEditText.setId(id);
    ...
    public static TextInputLayout getChardhamEditTextWithQrScan(Context context, String hint, int id, int length, int inputType) {
        ...
        final Integer REQUEST_CODE_FOR_SCANNER_ACTIVITY = 201;
        textInputLayout.setStartIconDrawable(R.drawable.ic_qr_scan);
        textInputLayout.setEndIconMode(TextInputLayout.END_ICON_CLEAR_TEXT);
        textInputLayout.setEndIconOnClickListener(v -> {
            if (textInputLayout.getEditText() != null) {
        ...
        textInputLayout.setStartIconOnClickListener((new View.OnClickListener() {
            ...
            public void onClick(View view) {
                try {
                    ...
                    activity.startActivityForResult(intent, REQUEST_CODE_FOR_SCANNER_ACTIVITY);
                } catch (Exception e) {
                    Log.e(getClass().getSimpleName(), null, e);
                }
            }
...
//                int[] weekDayArray = UtilBean.calculateWeekDaysGapBetweenDates(longDate, new Date().getTime());
//                week = weekDayArray[0];
//                day = weekDayArray[1];
        ...
        if (queFormBean.getDatamap() != null && !queFormBean.getDatamap().isEmpty()) {
            ...
            if (list.contains("Y")) {
                ...
                editText = MyStaticComponents.getEditText(context, GlobalTypes.YEARS, IdConstants.AGE_BOX_YEAR_EDIT_TEXT_ID, 3, InputType.TYPE_CLASS_NUMBER);
                if (year != -1) {
                ...
                textView = MyStaticComponents.getMaterialTextView(context, GlobalTypes.YEARS, -1, R.style.CustomAnswerView, false);
            ...
            if (list.contains("M")) {
                ...
                linearLayout.addView(editText, new LinearLayout.LayoutParams(0, WRAP_CONTENT, 9));
                textView = MyStaticComponents.getMaterialTextView(context, GlobalTypes.MONTHS, -1, R.style.CustomAnswerView, false);
                linearLayout.addView(textView, new LinearLayout.LayoutParams(0, WRAP_CONTENT, 2));
                mainLayout.addView(linearLayout);
            }
            ...
            if (list.contains("W")) {
                // Add Week Field
                linearLayout = getLinearLayout(context, -1, LinearLayout.HORIZONTAL, new LinearLayout.LayoutParams(MATCH_PARENT, WRAP_CONTENT));
                editText = MyStaticComponents.getEditText(context, "Weeks", IdConstants.AGE_BOX_WEEK_EDIT_TEXT_ID, 2, InputType.TYPE_CLASS_NUMBER);
//                if (week != -1) {
//                    Objects.requireNonNull(editText.getEditText()).setText(String.format(Locale.getDefault(), "%d", week));
//                }
                Objects.requireNonNull(editText.getEditText()).setOnFocusChangeListener(myListener);
                linearLayout.addView(editText, new LinearLayout.LayoutParams(0, WRAP_CONTENT, 9));
                textView = MyStaticComponents.getMaterialTextView(context, "Weeks", -1, R.style.CustomAnswerView, false);
                linearLayout.addView(textView, new LinearLayout.LayoutParams(0, WRAP_CONTENT, 2));
                mainLayout.addView(linearLayout);
            }
            ...
            if (list.contains("D")) {
                // Add Day Field
                linearLayout = getLinearLayout(context, -1, LinearLayout.HORIZONTAL, new LinearLayout.LayoutParams(MATCH_PARENT, WRAP_CONTENT));
                editText = MyStaticComponents.getEditText(context, GlobalTypes.DAY, IdConstants.AGE_BOX_DAY_EDIT_TEXT_ID, 1, InputType.TYPE_CLASS_NUMBER);
//                if (day != -1) {
//                    Objects.requireNonNull(editText.getEditText()).setText(String.format(Locale.getDefault(), "%d", day));
//                }
                Objects.requireNonNull(editText.getEditText()).setOnFocusChangeListener(myListener);
                linearLayout.addView(editText, new LinearLayout.LayoutParams(0, WRAP_CONTENT, 9));
                textView = MyStaticComponents.getMaterialTextView(context, GlobalTypes.DAY, -1, R.style.CustomAnswerView, false);
                linearLayout.addView(textView, new LinearLayout.LayoutParams(0, WRAP_CONTENT, 2));
                mainLayout.addView(linearLayout);
            }
        } else {
