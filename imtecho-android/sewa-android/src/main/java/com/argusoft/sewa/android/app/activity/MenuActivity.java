...
public class MenuActivity extends BaseActivity {
    ...
    @Bean
    public MoveToProductionServiceImpl moveToProdService;
    @Bean
    public TechoServiceImpl techoService;
    @Bean
    public SewaServiceRestClientImpl serviceRestClient;
    ...
    protected void setSubTitle(String sub) {
        if (sub == null && SewaTransformer.loginBean != null) {
            ...
            if (Boolean.TRUE.equals(SewaTransformer.loginBean.isTrainingUser()) && sub.endsWith("_t")) {
    ...
    protected void setSubTitleDetails(String sub) {

        runOnUiThread(() -> {
            MaterialTextView subTitleTextView = findViewById(R.id.toolbar_subtitle_details);
            if (subTitleTextView != null) {
                if (sub == null || sub.trim().isEmpty()) {
                    subTitleTextView.setVisibility(View.GONE);
                } else {
                    subTitleTextView.setVisibility(View.VISIBLE);
                    subTitleTextView.setText(UtilBean.getMyLabel(sub));
                    subTitleTextView.setTextAppearance(context, R.style.ToolbarSubtitleDetails);
                }
            }
        });
    }
    ...
    private void onCheckUpdateComplete() {
        ...
        if (Integer.parseInt(currentVersion) > BuildConfig.VERSION_CODE) {
            ...
            if (BackgroundDownloadUtils.isApkExistsOnLocal(context, apkName)) {
                ...
                try {
                    downloadPath = serviceRestClient.getSystemConfigurationByKey("ANDROID_APP_LINK");
