package com.argusoft.sewa.android.app.constants;

/**
 * Created by prateek on 2/17/20
 */
public class FieldNameConstants {

    private FieldNameConstants() {
        throw new IllegalStateException("Utility Class");
    }

    public static final String DOWNLOAD_ID = "downloadId";

    //Common
    public static final String TITLE = "title";
    public static final String MODIFIED_ON = "modifiedOn";
    public static final String STATE = "state";
    public static final String ACTUAL_ID = "actualId";

    //Family Bean
    public static final String FAMILY_ID = "familyId";
    public static final String UUID = "uuid";
    public static final String MEMBER_UUID = "memberUuid";
    public static final String MEMBERS_FAMILY_UUID = "familyUuid";

    public static final String LOCATION_ID = "locationId";
    public static final String LOCATION = "location";
    public static final String AREA_ID = "areaId";
    public static final String LATITUDE = "latitude";
    public static final String LONGITUDE = "longitude";
    public static final String UPDATED_ON = "updatedOn";
    public static final String ADDRESS = "address1";
    public static final String IS_REVERIFICATION_FLAG = "isReverificationFlag";
    public static final String LAST_IDSP_SCREENING_DATE = "lastIdspScreeningDate";
    public static final String LAST_IDSP_2_SCREENING_DATE = "lastIdsp2ScreeningDate";
    public static final String LAST_MEMBER_NCD_SCREENING_DATE = "lastMemberNcdScreeningDate";
    public static final String PENDING_ABHA_COUNT = "pendingAbhaCount";

    //Member Bean
    public static final String UNIQUE_HEALTH_ID = "uniqueHealthId";
    public static final String IS_HIV_POSITIVE = "isHivPositive";
    public static final String IS_VDRL_POSITIVE = "isVdrlPositive";
    public static final String UNIQUE_ID = "uniqueId";
    public static final String MOTHER_ID = "motherId";
    public static final String MOTHER_UUID = "motherUUID";
    public static final String HUSBAND_UUID = "husbandUUID";
    public static final String FIRST_NAME = "firstName";
    public static final String MIDDLE_NAME = "middleName";
    public static final String LAST_NAME = "lastName";
    public static final String GRAND_FATHER_NAME = "grandFatherName";
    public static final String DATE_OF_WEDDING = "dateOfWedding";
    public static final String MARITAL_STATUS = "maritalStatus";
    public static final String GENDER = "gender";
    public static final String DOB = "dob";
    public static final String ADDITIONAL_INFO = "additionalInfo";
    public static final String MOBILE_NUMBER = "mobileNumber";
    public static final String NRC_NUMBER = "nrcNumber";
    public static final String PASSPORT_NUMBER = "passportNumber";
    public static final String FAMILY_HEAD_FLAG = "familyHeadFlag";
    public static final String IS_PREGNANT_FLAG = "isPregnantFlag";
    public static final String IS_TB_CURED = "isTbCured";
    public static final String RDT_STATUS = "rdtStatus";
    public static final String IS_TB_SUSPECTED = "isTbSuspected";
    public static final String CUR_PREGNANT_DETAILED_ID = "curPregRegDetId";
    public static final String MENOPAUSE_ARRIVED = "menopauseArrived";
    public static final String HYSTERECTOMY_DONE = "hysterectomyDone";
    public static final String EDD = "edd";
    public static final String LMP_DATE = "lmpDate";
    public static final String LAST_DELIVERY_DATE = "lastDeliveryDate";
    public static final String IS_HIGH_RISK_CASE = "isHighRiskCase";
    public static final String FHSR_PHONE_VERIFIED = "fhsrPhoneVerified";
    public static final String IS_MOBILE_NUMBER_VERIFIED = "isMobileNumberVerified";
    public static final String NPCB_STATUS = "npcbStatus";
    public static final String NPCB_SCREENING_DATE = "npcbScreeningDate";
    public static final String CBAC_DATE = "cbacDate";
    public static final String HYP_YEAR = "hypYear";
    public static final String ORAL_YEAR = "oralYear";
    public static final String DIABETES_YEAR = "diabetesYear";
    public static final String BREAST_YEAR = "breastYear";
    public static final String CERVICAL_YEAR = "cervicalYear";
    public static final String MENTAL_HEALTH_YEAR = "mentalHealthYear";
    public static final String HEALTH_ID = "healthId";
    public static final String SEARCH_STRING = "searchString";
    public static final String HEALTH_ID_NUMBER = "healthIdNumber";
    public static final String SUSPECTED_FOR_DIABETES = "suspectedForDiabetes";
    public static final String CONFIRMED_DIABETES = "confirmedDiabetes";
    public static final String SUSPECTED_FOR_HYPERTENSION = "sufferingHypertension";
    public static final String MEDICINE_ID = "medicineId";
    public static final String REQUESTED_BY = "requestedBy";
    public static final String MO_CONFIRMED_DIABETES = "moConfirmedDiabetes";
    public static final String MO_CONFIRMED_HYPERTENSION = "moConfirmedHypertension";
    public static final String NUTRITION_STATUS = "nutritionStatus";

    //Location Bean
    public static final String ACTUAL_I_D = "actualID";
    public static final String PARENT = "parent";
    public static final String LEVEL = "level";

    //Notification bean
    public static final String MEMBER_ID = "memberId";
    public static final String NOTIFICATION_CODE = "notificationCode";
    public static final String NOTIFICATION_ID = "notificationId";
    public static final String BENEFICIARY_NAME = "beneficiaryName";
    public static final String SCHEDULED_DATE = "scheduledDate";
    public static final String EXPIRY_DATE = "expiryDate";

    //List Value Bean
    public static final String ID_OF_VALUE = "idOfValue";
    public static final String CONSTANT = "constant";
    public static final String Field = "field";

    //Covid Travellers Bean
    public static final String IS_ACTIVE = "isActive";

    //Health Infrastructure Bean
    public static final String TYPE_ID = "typeId";
    public static final String TYPE_NAME = "typeName";
    public static final String TYPE_NAME_KIOSK = "Kiosk";
    public static final String NAME = "name";
    public static final String ENGLISH_NAME = "englishName";

    //Library Bean
    public static final String FILE_NAME = "fileName";
    public static final String FORM_NAME = "formName";
    public static final String TAG = "tag";
    public static final String IS_DOWNLOADED = "isDownloaded";
    public static final String CATEGORY = "category";

    //Version Bean
    public static final String KEY = "key";

    //StoreAnswerBean
    public static final String USER_ID = "userId";
    public static final String RECORD_URL = "recordUrl";
    public static final String TOKEN = "token";

    //Bookmark Bean
    public static final String VIDEO_ID = "videoId";
    public static final String MEDIA_ID = "mediaId";
    public static final String TOPIC_ID = "topicId";
    public static final String COURSE_ID = "courseId";
    public static final String LESSON_ID = "lessonId";
    public static final String MODULE_ID = "moduleId";
    public static final String IS_TRANSCRIPT_FILE = "isTranscriptFile";
    public static final String IS_MEDIA_DOWNLOADED = "isMediaDownloaded";
    public static final String IS_TRANSCRIPT_DOWNLOADED = "isTranscriptDownloaded";
    public static final String SCHEDULE_DATE = "scheduleDate";
    public static final String IS_DEFAULT = "isDefault";
    public static final String HEALTH_INFRASTRUCTURE_ID = "healthInfrastructureId";
    public static final String HEALTH_INFRA_TYPE_ID = "healthInfraTypeId";

    public static final String EVENING_AVAILABILITY = "eveningAvailability";
    public static final String QR_DATA = "qrData";
    public static final String IS_QR_SCAN = "isQrCodeScan";
    public static final String IS_QR_FOR_ABHA = "isQrForAbha";

    //Family Availability Bean
    public static final String AVAILABILITY_STATUS = "availabilityStatus";

    public static final String FORM_TYPE = "formType";
    public static final String CHECKSUM = "checkSum";
    public static final String PARENT_STATUS = "parentStatus";
    public static final String STATUS = "status";
    public static final String BCG_SURVEY_STATUS = "bcgSurveyStatus";

    public static final String BCG_ELIGIBLE = "bcgEligible";
    public static final String BCG_WILLING = "bcgWilling";
    public static final String BCG_ELIGIBLE_FILLED = "bcgEligibleFilled";
    public static final String FAMILY_STATUS = "familyStatus";

    //Sickle cell survey
    public static final String HTN_DB_ABOVE_30 = "HTN_DB_ABOVE_30";
    public static final String SICKLE_CELL_DISEASE = "SICKLE_CELL_DISEASE";
    public static final String CHILDREN_6_TO_24 = "CHILD_BETWEEN_6_TO_24";
    public static final String RESIDENCE_STATUS = "residenceStatus";

}
