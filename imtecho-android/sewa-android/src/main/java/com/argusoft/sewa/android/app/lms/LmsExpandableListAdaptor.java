...
public class LmsExpandableListAdaptor extends BaseExpandableListAdapter {
    ...
    private void addQuizListViews(LinearLayout llQuiz, final LmsTopicDataBean topic, final LmsLessonDataBean media, final List<LmsQuestionSetDataBean> questionSets, final String testFor) {
        ...
        if (questionSets.size() > 0) {
            ...
            for (Map.Entry<Integer, List<LmsQuestionSetDataBean>> entry : questionSetMap.entrySet()) {
                if (Boolean.TRUE.equals(Objects.requireNonNull(course.getTestConfigJson().get(entry.getKey())).getIsCaseStudyQuestionSetType())) {
            ...
            for (final LmsQuestionSetDataBean questionSet : finalListOfQuestionSets) {
                ...
                if (topic != null) {
                    if (lmsService.isQuizCompleted(course.getCourseId(), topic.getTopicId(), null, questionSet.getQuestionSetType())) {
                        imageButton.setImageResource(R.drawable.test_completed);
                    } else {
                        imageButton.setImageResource(R.drawable.test);
                    }
                } else if (media != null) {
                    if (lmsService.isQuizCompleted(course.getCourseId(), media.getTopicId(), media.getActualId(), questionSet.getQuestionSetType())) {
                        imageButton.setImageResource(R.drawable.test_completed);
                    } else {
                        imageButton.setImageResource(R.drawable.test);
                    }
                ...
                inflate.setOnClickListener(v -> openLmsTestModule(topic, media, questionSet, testFor));
    ...
    private void openLmsTestModule(LmsTopicDataBean topic, LmsLessonDataBean media, LmsQuestionSetDataBean questionSet, String testFor) {
        ...
        if (topic != null) {
            for (LmsLessonDataBean mediaDataBean : topic.getTopicMedias()) {
                if ((Boolean.TRUE.equals(mediaDataBean.getCompulsory()) ||
                        mediaDataBean.getCompulsory() == null) &&
                        !viewedLessons.contains(mediaDataBean.getActualId())) {
                    allLessonsCompleted = false;
                    break;
                }
            }
        } else if (media != null) {
            allLessonsCompleted = Boolean.FALSE.equals(media.getCompulsory())
                    || viewedLessons.contains(media.getActualId());
        }

        //if (allLessonsCompleted) {
        if (allLessonsCompleted || (questionSet.getAllowedToSkipLessons() != null && questionSet.getAllowedToSkipLessons())) {
            Intent intent;
            if (course != null && course.getTestConfigJson() != null && Boolean.TRUE.equals(course.getTestConfigJson().get(questionSet.getQuestionSetType()).getProvideInteractiveFeedbackAfterEachQuestion())) {
                intent = new Intent(context, LmsInteractiveQuestionModuleActivity_.class);
            } else if (course != null && course.getTestConfigJson() != null
                    && questionSet.getQuestionSetType() == 2641) { //todo make id value dynamic as per field value name [2640 staging] [2641 production]
            ...
            if (topic != null && topic.getTopicId() != null) {
                intent.putExtra("moduleId", topic.getTopicId());
            }
    ...
    public View getChildView(int listPosition, final int expandedListPosition,
        ...
        addQuizListViews(llQuiz, listDataBeans.get(listPosition).getModule(), child, child.getQuestionSet(), child.getTitle());
    ...
    public int getChildrenCount(int listPosition) {
        if (Objects.equals(listDataBeans.get(listPosition).getGroupType(), LmsConstants.LMS_COURSE)) {
            return 0;
        } else if (Objects.equals(listDataBeans.get(listPosition).getGroupType(), LmsConstants.LMS_GROUP_QUIZ)) {
    ...
    private View getGroupViewOfTopic(final LmsTopicDataBean group, int listPosition, boolean isExpanded) {
        ...
        if (Objects.equals(listDataBeans.get(listPosition).getGroupType(), LmsConstants.LMS_GROUP_QUIZ)) {
            llQuiz.setVisibility(View.VISIBLE);
    ...
    private View getGroupViewOfQuiz(LmsQuestionSetDataBean questionSet) {
        ...
        if (lmsService.isQuizCompleted(course.getCourseId(), null, null, questionSet.getQuestionSetType())) {
