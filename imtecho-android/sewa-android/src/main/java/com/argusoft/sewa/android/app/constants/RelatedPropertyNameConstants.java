package com.argusoft.sewa.android.app.constants;

public class RelatedPropertyNameConstants {

    private RelatedPropertyNameConstants() {
        throw new IllegalStateException("Utility Class");
    }

    //Common Related Property Name
    public static final String STATE = "state";
    public static final String LOOP_COUNT = "loopCount";

    //Related Property Name for CFHC
    //Family Related
    public static final String FAMILY_ID = "familyId";
    public static final String HEAD_OF_FAMILY = "headOfFamily";
    public static final String IS_HOF_AVAILABLE = "isHOFAvailable";
    public static final String IS_AADHAAR_UPDATE = "isAadharUpdate";
    public static final String LOCATION_ID = "locationId";
    public static final String AREA_ID = "areaId";
    public static final String FAMILY_FOUND = "familyFound";
    public static final String UUID = "uuid";
    public static final String MEMBER_UUID = "memberUuid";
    public static final String IS_IT_INDEX_CASE = "isItIndexCase";
    public static final String MEMBER_UUID_WPD = "memberUuidForWpd";
    public static final String MEMBERS_FAMILY_UUID = "familyUuid";
    public static final String ABHA_CONSENT_CHECKBOX_MODEL = "abhaConsentCheckboxModel";
    public static final String AADHAAR_MATCH_WITH_BENEFICIARY_CONSENT = "aadhaarMatchWithBeneficiaryConsent";
    public static final String REVERIFICATION_COMMENT = "reverificationComment";
    public static final String HOUSE_NUMBER = "houseNumber";
    public static final String ADDRESS_1 = "address1";
    public static final String ADDRESS_2 = "address2";
    public static final String HOUSE_STATUS = "houseStatus";
    public static final String MAA_VATSALYA_NUMBER = "maaVatsalyaNumber";
    public static final String RSBY_CARD_NUMBER = "rsbyCardNumber";
    public static final String RATION_CARD_NUMBER = "rationCardNumber";
    public static final String BPL_CARD_NUMBER = "bplCardNumber";
    public static final String BPL_FLAG = "bplFlag";
    public static final String TOILET_AVAILABLE_FLAG = "toiletAvailableFlag";
    public static final String VULNERABLE_FLAG = "vulnerableFlag";
    public static final String SEASONAL_MIGRANT_FAMILY = "seasonalMigrantFlag";
    public static final String IS_PROVIDING_CONSENT = "isProvidingConsent";
    public static final String ELIGIBLE_FOR_PMJAY = "eligibleForPmjay";
    public static final String VULNERABILITY_CRITERIA = "vulnerabilityCriteria";
    public static final String DEFAULT_CASTE = "defaultCaste";
    public static final String DEFAULT_RELIGION = "defaultReligion";
    public static final String NRC_NUMBER = "nrcNumber";
    public static final String BIRTH_CERTIFICATE_NUMBER = "birthCertificateNumber";
    public static final String PASSPORT_NUMBER = "passportNumber";
    public static final String IDENTIFICATION_PROOF = "idProof";
    public static final String DEFAULT_PHC = "defaultPhc";
    public static final String DEFAULT_ANGANWADI = "defaultAnganwadi";
    public static final String GENERATE_ABDM_HEALTH_ID = "generateAbdmHealthId";
    public static final String TYPE_OF_HOUSE = "typeOfHouse";
    public static final String MOTORIZED_VEHICLE = "motorizedVehicle";
    public static final String DRINKING_WATER_SOURCE = "drinkingWaterSource";
    public static final String COOKING_GAS = "cookingGas";
    public static final String ELECTRIC_CONNECTION = "electricConnection";
    public static final String TOILET_AVAILABILITY = "toiletAvailability";
    public static final String TYPE_OF_FUEL = "typeOfFuel";
    public static final String TYPE_OF_VEHICLE = "typeOfVehicle";
    public static final String ANNUAL_INCOME = "annualIncome";
    public static final String HOUSE_OWNERSHIP = "houseOwnership";
    public static final String HEALTH_ID_NUMBER = "healthIdNumber";
    public static final String HEALTH_ID = "healthId";

    //Related Property Name for CFHC
    //Member Related
    public static final String MEMBER_ID = "memberId";
    public static final String UNIQUE_HEALTH_ID = "uniqueHealthId";
    public static final String IS_MEMBER_DEAD = "isMemberDead";
    public static final String MEMBER_FULL_NAME = "memberFullName";
    public static final String FIRST_NAME = "firstName";
    public static final String MIDDLE_NAME = "middleName";
    public static final String LAST_NAME = "lastName";
    public static final String MEMBER_STATE = "memberState";
    public static final String DOB = "dob";
    public static final String DOB_DISPLAY = "dobDisplay";
    public static final String AGE_AS_PER_DATE = "ageAsPerDate";
    public static final String GENDER = "gender";
    public static final String HOF_GENDER = "hofGender";
    public static final String HOF_NAME = "hofName";
    public static final String ANS_12 = "ans12"; //Used for GENDER //1 for Male //2 for Female
    public static final String MINOR_MOBILE_NUMBER = "minorMobileNumber";
    public static final String MOBILE_NUMBER = "mobileNumber";
    public static final String ALTERNATIVE_MOBILE_NUMBER = "alternativeMobileNumber";
    public static final String AADHAR_NUMBER = "aadharNumber";
    public static final String AADHAR_STATUS = "aadhaarStatus";
    public static final String BANK_ACCOUNT_NUMBER = "bankAccountNumber";
    public static final String IFSC = "ifsc";
    public static final String DEFAULT_MARITAL_STATUS = "defaultMaritalStatus";
    public static final String MARITAL_STATUS = "maritalStatus";
    public static final String RELATION_WITH_HOF = "relationWithHOF";
    public static final String FAMILY_HEAD_FLAG = "familyHeadFlag";
    public static final String GRANDFATHER_NAME = "grandfatherName";
    public static final String HEAD_COUNT = "headCount";
    public static final String MOTHER_ID = "motherId";
    public static final String MOTHER_NAME = "motherName";
    public static final String HUSBAND_ID = "husbandId";
    public static final String HUSBAND_NAME = "husbandName";
    public static final String AADHAR_AVAILABLE = "aadharAvailable";
    public static final String DEFAULT_NORMAL_CYCLE_DAYS = "defaultNormalCycleDays";
    public static final String MENOPAUSE_ARRIVED = "menopauseArrived";
    public static final String DEFAULT_FAMILY_PLANNING_METHOD = "defaultFamilyPlanningMethod";
    public static final String DEFAULT_IS_PREGNANT = "defaultIsPregnant";
    public static final String DEFAULT_LMP = "defaultLmp";
    public static final String LAST_DELIVERY_DATE = "lastDeliveryDate";
    public static final String LAST_METHOD_OF_CONTRACEPTION = "lastMethodOfContraception";
    public static final String MOBILE_NUMBER_VERIFIED = "mobileNumberVerfied";
    public static final String EDUCATION_STATUS = "educationStatus";
    public static final String MEMBER_STATUS = "memberStatus";
    public static final String CONGENITAL_STATUS = "congenitalStatus";
    public static final String CONGENITAL_ANOMALY_IDS = "congenitalAnomalyIds";
    public static final String HIV_STATUS_FROM_ANC = "hivStatusFromANC";
    public static final String CHRONIC_DISEASE_STATUS = "chronicDiseaseStatus";
    public static final String CHRONIC_DISEASES = "chronicDiseases";
    public static final String CHRONIC_DISEASE_IDS = "chronicDiseaseIds";
    public static final String CURRENT_DISEASE_STATUS = "currentDiseaseStatus";
    public static final String CURRENT_DISEASE_IDS = "currentDiseaseIds";
    public static final String EYE_ISSUE_STATUS = "eyeIssueStatus";
    public static final String EYE_ISSUE_IDS = "eyeIssueIds";
    public static final String AADHAR_SCANNED = "aadharScanned";
    public static final String AADHAR_AGREEMENT = "aadharAgreement";
    public static final String IS_VULNERABLE = "isVulnerable";
    public static final String AUDIO_FILE_NAME = "AUDIO_FILE";
    public static final String HEIGHT = "height";
    public static final String WEIGHT = "weight";
    public static final String BMI = "bmi";
    public static final String BMI_CATEGORY = "bmiCategory";
    public static final String RISK_FACTOR = "riskFactor";
    public static final String DISEASE_HISTORY = "diseaseHistory";
    public static final String OTHER_DISEASE_HISTORY = "otherDiseaseHistory";
    public static final String ANY_INSURANCE_SCHEME = "anyInsuranceScheme";
    public static final String SCHEME_DETAILS = "schemeDetails";
    public static final String HAVE_PMJAY = "havePMJAY";
    public static final String ALCOHOL_ADDICTION = "alcoholAddiction";
    public static final String SMOKING_ADDICTION = "smokingAddiction";
    public static final String TOBACCO_ADDICTION = "tobaccoAddiction";
    public static final String RATION_CARD_COLOR = "colorOfRationCard";
    public static final String PMJAY_ENROLLMENT = "pmjayEnrollment";
    public static final String TRAVELLED_FOREIGN = "travelledToForeign";
    public static final String RESIDENCE_STATUS = "residenceStatus";
    public static final String OCCUPATION = "occupation";
    public static final String DISABILITIES = "disabilities";
    public static final String OTHER_DISABILITIES = "otherDisabilities";
    public static final String CHRONIC_DISEASE = "chronicDiseaseIds";
    public static final String OTHER_CHRONIC_DISEASE = "otherChronicDisease";
    public static final String EYE_ISSUE = "eyeIssueIds";
    public static final String OTHER_EYE_ISSUE = "otherEyeIssue";
    public static final String CATARACT_SURGERY = "cataractSurgery";
    public static final String SICKLE_CELL_THALASSEMIA = "sickleCellThalassemiaStatus";
    public static final String IS_CHILD_GOING_SCHOOL = "isChildGoingSchool";
    public static final String ON_TREATMENT = "onTreatment";
    public static final String STANDARD = "standard";
    public static final String CHRONIC_DISEASE_TREATMENT = "chronicDiseaseIdsForTreatment";
    public static final String OTHER_CHRONIC_DISEASE_TREATMENT = "otherChronicDiseaseForTreatment";

    //Related Property Name for Morbidity ANC
    public static final String AGE_FOR_MORB = "ageForMorb";
    public static final String COMPLICATION_DURING_LAST_PREGNANCY = "complicationDuringLastPregnancy";
    public static final String GRAVIDA_VALUE = "gravidaValue";
    public static final String GESTATIONAL_WEEK = "gestationalWeek";
    public static final String IS_FIRST_ANC_HOME_VISIT_DONE = "IsFirstANChomevisitDone";

    //Related Property Name for Morbidity PNC
    public static final String PRETERM_DELIVERY = "pretermDelivery";
    public static final String PRETERM_DELIVERY_POV = "pretermDeliveryPOV";
    public static final String PRETERM_DELIVERY_LMP = "pretermDeliveryLMP";
    public static final String CHILD_DOB = "childDOB";
    public static final String TEMPERATURE_FOR_MORB = "temperatureForMorb";
    public static final String CHILD_LATEST_WEIGHT = "childLatestWeight";
    public static final String CHILD_LAST_WEIGHT = "childLastWeight";
    public static final String IS_CHILD_FIRST_PNC_DONE = "isChildsFirstPNCDone";
    public static final String WHEN_DID_BABY_CRY = "whenDidBabyCry";
    public static final String PNC_SEPSIS_CONDITION = "pncSepsisCondition";

    //Related Property Name for Morbidity Child Care
    public static final String LATEST_WEIGHT = "latestWeight";
    public static final String PREV_WEIGHT = "prevWeight";
    public static final String MALNUTRITION_GRADE = "malnutritionGrade";
    public static final String AGE_FROM_ECR = "ageFromECR";

    // Related Property Name for RCH forms
    public static final String MONTH_START = "monthStart";
    public static final String MEMBER_ID_AND_NAME = "memberIdAndName";
    public static final String NOTIFICATION_ID = "notificationId";
    public static final String VISIT_NUMBER = "visitNumber";
    public static final String ASHA_INFO = "ashaInfo";
    public static final String ANGANWADI_ID = "anganwadiId";
    public static final String UPDATE_FAMILY_ANGANWADI = "updateFamilyAnganwadi";
    public static final String MEMBER_NAME = "memberName";
    public static final String NAME_OF_BENEFICIARY = "nameOfBeneficiary";
    public static final String SUB_TITLE_DETAILS = "subtitleDetails";
    public static final String MEMBER_UNIQUE_ID = "memberUniqueId";
    public static final String VISIT_DATE = "visitDate";
    public static final String MEMBER_ACTUAL_ID = "memberActualId";
    public static final String CUR_PREG_REG_DET_ID = "curPregRegDetId";
    public static final String PHONE_NUMBER = "phoneNumber";
    public static final String ADDRESS = "address";
    public static final String AGE_DISPLAY = "ageDisplay";
    public static final String AGE_DISPLAY_WEEKS = "ageDisplayWeeks";
    public static final String NAME_BASED_ON_AADHAR = "nameBasedOnAadhar";
    public static final String LAST_LMP = "lastLmp";
    public static final String LMP_DATE = "lmpDate";
    public static final String EDD = "edd";
    public static final String LAST_DELIVERY_OUTCOME = "lastDeliveryOutcome";
    public static final String PREGNANCY_OUTCOME = "pregnancyOutcome";
    public static final String HAS_DELIVERY_HAPPENED = "hasDeliveryHappened";
    public static final String PREGNANCY_OUTCOME_LIVE_BIRTH = "LBIRTH";
    public static final String PREGNANCY_OUTCOME_PREMATURE = "PREMATURE";
    public static final String PREMATURE = "PREMATURE";
    public static final String BABIES_GENDER = "babysGender";
    public static final String LAST_METHOD_OF_CONTRACEPTION_DISPLAY = "lastMethodOfContraceptionDisplay";
    public static final String FP_INSERT_OPERATE_DATE = "fpInsertOperateDate";
    public static final String RELIGION = "religion";
    public static final String CASTE = "caste";
    public static final String IS_BPL = "isBpl";
    public static final String FAMILY_PHONE_NUMBER = "familyPhoneNumber";
    public static final String MOTHER_HEALTH_ID = "motherHealthId";
    public static final String IS_JSY_BENEFICIARY = "isJsyBeneficiary";
    public static final String IS_JSY_PAYMENT_DONE = "isJsyPaymentDone";
    public static final String BLOOD_GROUP = "bloodGroup";
    public static final String LAST_WEIGHT_AND_DATE = "lastWeightAndDate";
    public static final String CURRENT_GRAVIDA = "currentGravida";
    public static final String DEFAULT_GRAVIDA = "defaultGravida";
    public static final String CURRENT_PARA = "currentPara";
    public static final String DEFAULT_PARA = "defaultPara";
    public static final String WEEKS_OF_PREGNANCY = "weeksOfPregnancy";
    public static final String LAST_SERVICE_DATE = "lastServiceDate";
    public static final String MOTHER_ALIVE = "motherAlive";
    public static final String FP_SERVICE_DATE = "fpServiceDate";
    public static final String DEVELOPMENTAL_DELAYS = "developmentalDelays";
    public static final String MARRIAGE_YEAR = "marriageYear";
    public static final String AGE_AT_WEDDING = "ageAtWedding";
    public static final String DATE_OF_WEDDING = "dateOfWedding";
    public static final String DATE_OF_WEDDING_DISPLAY = "dateOfWeddingDisplay";
    public static final String MEMBERS_UNDER_20_AVAILABLE = "membersUnder20Available";
    public static final String PERSONALLY_USING_FP = "personalFp";
    public static final String TOTAL_LIVING_CHILDREN = "totalLivingChildren";
    public static final String TOTAL_MALE_CHILDREN = "totalMaleChildren";
    public static final String TOTAL_FEMALE_CHILDREN = "totalFemaleChildren";
    public static final String LATEST_CHILD_NAME = "latestChildName";
    public static final String LATEST_CHILD_AGE = "latestChildAge";
    public static final String LATEST_CHILD_GENDER = "latestChildGender";
    public static final String PREGNANT_LAST_3_YRS = "pregnantLast3Yrs";
    public static final String TT = "tt";
    public static final String TT_1_GIVEN_ON = "tt1GivenOn";
    public static final String PREVIOUS_ILLNESS = "previousIllness";
    public static final String LAST_HAEMOGLOBIN = "lastHaemoglobin";
    public static final String LAST_HAEMOGLOBIN_DISPLAY = "lastHaemoglobinDisplay";
    public static final String LAST_WEIGHT = "lastWeight";
    public static final String ELIGIBILITY_CRITERIA = "eligibilityCriteria";
    public static final String JSY_BENEFICIARY = "jsyBeneficiary";
    public static final String JSY_PAYMENT = "jsyPayment";
    public static final String KPSY_BENEFICIARY = "kpsyBeneficiary";
    public static final String IAY_BENEFICIARY = "iayBeneficiary";
    public static final String CHIRANJEEVI_BENEFICIARY = "chiranjeeviBeneficiary";
    public static final String GESTATIONAL_AGE = "gestationalAge";
    public static final String CUR_PREG_REG_DATE = "curPregRegDate";
    public static final String EARLY_REGISTRATION = "earlyRegistration";
    public static final String HIGH_RISK_CONDITIONS = "highRiskConditions";
    public static final String IMMUNISATION_GIVEN = "immunisationGiven";
    public static final String PREVIOUS_PREGNANCY_COMPLICATION = "previousPregnancyComplication";
    public static final String HBSAG_TEST = "hbsagTest";
    public static final String HIV_TEST = "hivTest";
    public static final String HEPATITIS_TEST = "hepatitisTest";
    public static final String VDRL_TEST = "vdrlTest";
    public static final String BIRTH_PLAN = "birthPlan";
    public static final String SICKLE_CELL_TEST = "sickleCellTest";
    public static final String MEMBER_HEIGHT = "memberHeight";
    public static final String ANC_MORB_ASHA = "ancMorbAsha";
    public static final String PREG_REGISTRATION_DATE = "pregRegistrationDate";
    public static final String ANC_DETAILS = "ancDetails";
    public static final String PLACE_OF_BIRTH = "placeOfBirth";
    public static final String CHILD_FIRST_NAME = "childFirstName";
    public static final String BIRTH_WEIGHT = "birthWeight";
    public static final String BIRTH_WEIGHT_DISPLAY = "birthWeightDisplay";
    public static final String AGE = "age";
    public static final String COMPLIMENTARY_FEEDING = "complimentaryFeeding";
    public static final String REMAINING_VACCINES = "remainingVaccines";
    public static final String IMMUNISED_IN_TWO_YEAR = "immunisedInTwoYear";
    public static final String FULLY_IMMUNIZED_OR_NOT = "fullyImmOrNot";
    public static final String IMMUNISED_IN_ONE_YEAR = "immunisedInOneYear";
    public static final String CP_SCREENING = "cpScreening";
    public static final String WT_GAIN_STATUS = "wtGainStatus";
    public static final String GIVEN_RUTF = "givenRUTF";
    public static final String CP_STATUS = "cpStatus";
    public static final String SAM_SCREENING_DONE = "samScreeningDone";
    public static final String LAST_SAM_SCREENING_DATE = "lastSamScreeningDate";
    public static final String LAST_THR_SERVICE_DATE = "lastTHRServiceDate";
    public static final String DELIVERY_DATE_DISPLAY = "deliveryDateDisplay";
    public static final String DELIVERY_DATE = "deliveryDate";
    public static final String SERVICE_NUMBER = "serviceNumber";
    public static final String PNC_IFA = "pncIfa";
    public static final String PNC_CALCIUM = "pncCalcium";
    public static final String UNIQUE_HEALTH_ID_CHILD = "uniqueHealthIdChild";
    public static final String CHILD_NAME = "childName";
    public static final String CONTACT_DETAILS = "contactDetails";
    public static final String GENDER_DISPLAY = "genderDisplay";
    public static final String ACTUAL_GRAVIDA = "actualGravida";
    public static final String SELECTED_CHILDREN = "selectedChildren";
    public static final String DISPLAY_MORBIDITY_CHILD = "displayMorbidityChild";
    public static final String LOOP_COUNTER_FOR_ALIVE_MEMBERS = "loopCounterForAliveMembers";
    public static final String LOOP_COUNTER_FOR_FEMALE_MARRIED_MEMBERS = "loopCounterForFemaleMarriedMembers";
    public static final String LOOP_COUNTER_FOR_WIFE_MEMBERS = "loopCounterForWifeMembers";
    public static final String LOOP_COUNTER_FOR_MALE_MARRIED_MEMBERS = "loopCounterForMaleMarriedMembers";
    public static final String HEALTH_INFRASTRUCTURE_TYPE = "healthInfrastructureType";
    public static final String HEALTH_INFRASTRUCTURE_TYPE_ID = "healthInfrastructureTypeId";
    public static final String DELIVERY_PLACE = "deliveryPlace";
    public static final String ABORTION_COUNT = "abortionCount";
    public static final String PMMVY_BENEFICIARY = "pmmvyBeneficiary";
    public static final String PMSMA_BENEFICIARY = "pmsmaBeneficiary";
    public static final String DIKRI_BENEFICIARY = "dikriBeneficiary";

    // Related property for member update form
    public static final String NORMAL_CYCLE_DAYS = "normalCycleDays";
    public static final String FAMILY_PLANNING_METHOD = "familyPlanningMethod";
    public static final String IS_PREGNANT = "isPregnant";
    public static final String LMP = "lmp";

    // Related property for NCD forms
    public static final String CBAC_DONE = "cbacDone";
    public static final String CBAC_SCORE = "cbacScore";
    public static final String CBAC_DATE = "cbacDate";
    public static final String FAMILY_ACTUAL_ID = "familyActualId";
    public static final String ANY_MEMBER_CBAC_DONE = "anyMemberCbacDone";
    public static final String SCREENING_DATE = "screeningDate";
    public static final String IS_EARLIER_DIAGNOSED_HYPERTENSION = "isEarlierDiagnosedHypertension";
    public static final String IS_UNDER_TREATMENT_FOR_HYPERTENSION = "currentlyTreatmentHypertension";
    public static final String IS_EARLIER_DIAGNOSED_DIABETES = "isEarlierDiagnosedDiabetes";
    public static final String COMPLICATION_LIST = "complicationList";
    public static final String HMIS_ID = "hmisId";
    public static final String NUTRITION_STATUS = "nutritionStatus";

    // Related property for NPCB forms
    public static final String BLOOD_SUGAR = "bloodSugar";

    // Related property for other notification forms
    public static final String IRON_SUCROSE_DUE_DATE = "ironSucroseDueDate";
    public static final String LMP_DATE_DISPLAY = "lmpDateDisplay";
    public static final String IMMUNISATION = "immunisation";

    // Related property for covid traveller form
    public static final String ACTUAL_ID = "actualId";
    public static final String NAME = "name";
    public static final String TRAVEL_DATE = "travelDate";
    public static final String TRAVEL_DATE_DISPLAY = "travelDateDisplay";
    public static final String INFO_RECEIVED_DATE = "infoReceivedDate";
    public static final String INFO_RECEIVED_DATE_DISPLAY = "infoReceivedDateDisplay";
    public static final String PIN_CODE = "pinCode";
    public static final String TRACKING_START_DATE = "trackingStartDate";
    public static final String TRACKING_DAY = "trackingDay";

    //Related Property Name Constants for Morbidity
    public static final String DISEASE_NAME = "diseaseName";
    public static final String REMAINING_DISEASE = "remainingDisease";
    public static final String RDT_STATUS = "rdtStatus";
    public static final String REMAINING_RED_DISEASE = "remainingRedDisease";
    public static final String CALL_HOSPITAL = "callHospital";
    public static final String OVERALL_STATUS = "overallStatus";

    //RelatedPropertyNameForDynamicForm
    public static final String FORM_START_DATE = "formStartDate";


    //Weekly Visits
    public static final String CLINIC_DATE = "clinicDate";
    public static final String CLINIC_TYPE = "clinicType";
    public static final String MO_COMMENT = "moComment";
    public static final String DIABETES_CONFIRMED = "diabetes";
    public static final String HYPERTENSION_CONFIRMED = "hypertension";
    public static final String MENTAL_HEALTH_CONFIRMED = "mentalHealth";
    public static final String PREVIOUS_DIABETES = "previousDiabetes";
    public static final String PREVIOUS_HYPERTENSION = "previousHypertension";
    public static final String PREVIOUS_MENTAL_HEALTH = "previousMentalHealth";
    public static final String MEDICINE_ID = "medicineId";
    public static final String MEDICINE_NAME = "medicineName";
    public static final String FREQUENCY = "frequency";
    public static final String DURATION = "duration";
    public static final String QUANTITY = "quantity";
    public static final String SPECIAL_INSTRUCTION = "specialInstruction";
    public static final String STOCK = "stock";
    public static final String TOTAL_MEDICINE = "totalMedicine";
    public static final String DRUGS = "drugs";
    public static final String PREVIOUS_MEDICINES = "previousMedicines";
    public static final String CONFIRMED_DISEASE = "confirmedDisease";
    public static final String VISIT_TYPE = "unscheduled";
    public static final String ANY_DISEASE = "anyDisease";
    public static final String PHYSICAL_ACTIVITY = "physicalActivity";
    public static final String USER_ROLE = "userRole";
    public static final String OBSERVATIONS = "observations";
    public static final String OTHER_OBSERVATIONS = "otherObservations";
    public static final String LAST_VISIT_DATE = "lastVisited";
    public static final String LAST_DIABETES = "lastBloodSugar";
    public static final String LAST_HYPERTENSION = "lastBp";
    public static final String LAST_MENTAL_HEALTH = "lastMentalHealthValue";

    public static final String OXYGEN_LEVEL = "oxygenLevel";
    public static final String BLOOD_PRESSURE_LEVEL = "bpLevel";
    public static final String BLOOD_SUGAR_LEVEL = "bloodSugarLevel";
    public static final String TEMPERATURE_LEVEL = "temperatureLevel";
    public static final String CAME_FROM = "cameFrom";
    public static final String MEMBER_FULL_NAME_AND_UNIQUE_ID = "memberFullNameAndUniqueId";
    public static final String MRP_TREATMENT_FLOW = "mrpTreatmentFlow";
    public static final String CHARDHAM_MEMBER_UNIQUE_ID = "chardhamMemberUniqueId";
    public static final String CHARDHAM_MEMBER_GENDER = "chMemberGender";
    public static final String CHARDHAM_CONTACT_NUMBER = "chPhoneNumber";
    public static final String CHARDHAM_DEFAULT_HEALTH_INFRA_ID = "chardhamDefaultHealthInfraId";
    public static final String CHARDHAM_MEMBER_AGE = "chMemberAge";
    public static final String CHARDHAM_MEMBER_WEIGHT = "chMemberWeight";
    public static final String SCREENING_STATUS = "screeningStatus";
    public static final String SCREENING_SYMPTOMS = "symptoms";
    public static final String SCREENING_OTHER_SYMPTOMS = "otherSymptoms";
    public static final String SCREENING_TREATMENT = "treatment";
    public static final String SCREENING_OTHER_TREATMENT = "otherTreatment";
    public static final String SCREENING_DIAGNOSIS = "diagnosis";
    public static final String TREATMENT_STATUS = "treatmentStatus";
    public static final String SCREENING_OTHER_DIAGNOSIS = "otherDiagnosis";
    public static final String SCREENING_COURSE_OF_TREATMENT = "courseOfTreatment";
    public static final String IS_FROM_DEVICE_AND_NOT_REGISTERED = "isFromDeviceAndNotRegistered";
    public static final String CHARDHAM_SCREENING_START_TIME = "screeningStartTime";
    public static final String CHECKSUM_IF_FROM_DEVICE = "checkSumWhenScannedFromDevice";

    public static final String HAS_BREATHLESSNESS = "isBreathlessness";
    public static final String HAS_HIGH_BP = "isHighBp";
    public static final String HAS_ASTHMA = "isAsthma";
    public static final String HAS_DIABETES = "isDiabetes";
    public static final String HAS_HEART_CONDITIONS = "isHeartConditions";
    public static final String IS_CH_MEMBER_PREGNANT = "isChMemberPregnant";

    public static final String CH_MEMBER_HAS_FEVER = "hasFever";
    public static final String LANGUAGE_PREFERENCE = "languagePreference";
    public static final String IS_SYNCED_WITH_TOURISM_DATA = "isSyncedWithTourismData";

    //RelatedPropertyName for worklog
    public static final String BENEFICIARY_NAME_FOR_LOG = "nameForLog";
    public static final String ELIGIBLE_WOMEN_COUNT = "eligibleWomenNumber";
    public static final String ELIGIBLE_CHILDREN_COUNT = "eligibleChildrenNumber";
    public static final String IS_HOLD_ID_POOR = "isHoldIdPoor";
    public static final String HAVE_NSSF = "haveNSSF";
    public static final String NSSF_CARD_NUMBER = "nSSFCardNumber";
    public static final String FAMILY_AVAILABILITY_ID = "familyAvailabilityId";
    public static final String WIDOW_PENSION = "widowPension";
    public static final String SENIOR_CITIZEN_PENSION = "seniorPension";
    public static final String DISABILITY_PENSION = "disabilityPension";

    public static final String SCHOOL_NAME = "schoolName";
    public static final String SCHOOL_ACTUAL_ID = "schoolActualId";
    public static final String CURRENT_STUDYING_STANDARD = "currentStudyingStandard";
    public static final String CAME_FROM_SCHOOL = "cameFromSchool";
    public static final String NEW_ENTRY = "newEntry";
    public static final String NEW_FAMILY = "newFamily";
    public static final String ADOLESCENT_SCREENING_DATE = "adolescentScreeningDate";
    public static final String HAEMOGLOBIN = "haemoglobin";
    public static final String COUNSELLING_DONE = "counsellingDone";
    public static final String ABSORBENT_MATERIAL_USED = "absorbentMaterialUsed";
    public static final String ISSUE_WITH_MENSTRUATION = "issueWithMenstruation";
    public static final String SERVICE_LOCATION = "serviceLocation";
    public static final String IFA_TAB_TAKEN_NOW = "ifaTabTakenNow";
    public static final String IFA_TAB_TAKEN_LAST_MONTH = "ifaTabTakenLastMonth";
    public static final String IS_SANITARY_PAD_GIVEN = "isSanitaryPadGiven";


    //Medicine Constants
    public static final String IFA_TABLETS = "ifaTablets";
    public static final String FA_TABLETS = "faTablets";
    public static final String CALCIUM_TABLETS = "calciumTablets";
    public static final String ALBENDAZOLE_TABLETS = "albendazoleTablets";
    public static final String TT_VACCINE = "ttVaccine";
    public static final String TT_ONE_VACCINE = "ttOneVaccine";
    public static final String TT_TWO_VACCINE = "ttTwoVaccine";


    public static final String NUMBER_OF_SANITARY_PADS_GIVEN = "numberOfSanitaryPadsGiven";
    public static final String IS_HAVING_MENSTRUAL_PROBLEMS = "isHavingMenstrualProblem";
    public static final String IS_TD_INJECTION_GIVEN = "isTDInjectionGiven";
    public static final String IS_TB_SUSPECTED = "isTBSuspected";
    public static final String IS_TB_CURED = "isTBCured";
    public static final String HIV_TEST_RESULT = "hivTestResult";
    public static final String TD_INJECTION_DATE = "tdInjectionDate";
    public static final String IS_ALBANDAZOLE_GIVEN = "isAlbandazoleGivenInLastSixMonths";
    public static final String IS_PERIOD_STARTED = "isPeriodStarted";
    public static final String IS_HAEMOGLOBIN_MEASURED = "isHaemoglobinMeasured";
    public static final String HIGH_RISK_ADOLESCENT = "highRiskFactor";
    public static final String MALARIA_TYPE = "malariaType";
    public static final String POTENTIAL_MALARIA_CASE = "potentialMalariaCase";
    public static final String NUMBER_OF_MEMBERS = "numberOfMembers";
    public static final String TB_SCREENING_TYPE = "tbScreeningType";
    public static final String PATIENT_UUID = "patient_uuid";
    public static final String PATIENT_FHIR_RESOURCES_DATA = "patient_fhir_resource_data";
    public static final String LAB_ID = "labId";

    public static final String UID = "uid";
    public static final String ACTUAL_BMI_VALUE = "actualbmivalue";
    public static final String IS_BMI_LE_18 = "isBmiLe18";
    public static final String LAST_ANC_VISIT_DATE = "lastAncVisitDate";

    //RelatedPropertyName for sickle cell survey
    public static final String HYP_DB_ABOVE_30 = "hypDiaAbove30";
    public static final String IS_MOTHER_MEMBER = "isMotherMember";


    public static final String IS_SMAG_TRAINED = "isSmagTrained";
    public static final String IS_DEFAULT_HEALTH_INFRA_ASSIGNED = "isHealthInfraAssigned";
    public static final String USER_ASSIGNED_HEALTH_INFRA_ID = "userAssignedHealthInfra";
    public static final String USER_ASSIGNED_HEALTH_INFRA_NAME = "assignedHealthInfra";
    public static final String IS_FROM_NEARBY_HOUSE_HOLD = "isFromNearby";
    public static final String HOUSEHOLD_NUMBER = "householdNumber";
    public static final String GPS_LOCATION = "gpsLocation";
    public static final String LATITUDE = "latitude";
    public static final String LONGITUDE = "longitude";
    public static final String INDEX_CASE = "indexCase";
    public static final String ASSIGNED_HEALTH_CENTER = "assignedHealthCenter";
    public static final String DISTANCE_FROM_HEALTH_CENTER = "distanceToHealthCenter";
    public static final String ALBENDAZOLE_GIVEN = "albendazoleGiven";
    public static final String IS_QR_ENABLED = "isQrEnabled";
    public static final String ANC_VISIT_NUMBER = "ancVisitNumber";
    public static final String CURRENT_TRIMESTER = "currentTrimester";
    public static final String HEAD_OF_FAMILY_NUMBER = "headMobileNumber";
    public static final String RCH_ID = "rchId";
}
