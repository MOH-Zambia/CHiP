...
public class FamilySplitActivity extends MenuActivity implements View.OnClickListener, CompoundButton.OnCheckedChangeListener {
    ...
    private static final String REL_WITH_HOF_SCREEN = "relWithHofScreen";
    ...
    private final Map<String, String> relationWithHofMap = new HashMap<>();
...
//                    }
                ...
                case HEAD_MEMBER_SELECTION_SCREEN:
                    ...
                    if (headSelected) {
                        ...
                        selectedHeadMember = selectedMembers.get(headRadioGroup.getCheckedRadioButtonId());
                    }

                    if (headSelected) {
                        if (unselectedMembers.size() > 1) {
                            unselectedMembers.remove(selectedHeadMember);
                            getRelWithHofScreen(unselectedMembers);
                        } else {
                            setConfirmationScreen();
                        }
                    } else {
                        if (selectedMembers.size() > 1) {
                            selectedMembers.remove(selectedHeadMember);
                            getRelWithHofScreen(selectedMembers);
                        } else {
                            setConfirmationScreen();
                        }
                    }
                ...
                case REL_WITH_HOF_SCREEN:
                    if (headSelected) {
                        unselectedMembers.add(selectedHeadMember);
                    } else {
                        selectedMembers.add(selectedHeadMember);
                    }
                    setConfirmationScreen();
                    break;
                ...
                case MIGRATE_QUE_SCREEN:
                    if (migrateQueRadioGroup.getCheckedRadioButtonId() == -1) {
                        ...
                        intent.putExtra("existingHeadSelected", headSelected);
                        intent.putExtra("relationWithHofMap", (Serializable) relationWithHofMap);
                        if (selectedHeadMember != null) {
                            intent.putExtra("newHead", selectedHeadMember.getId());
                        }
    ...
    private void setMemberSelectionScreen() {
        ...
        bodyLayoutContainer.addView(MyStaticComponents.generateQuestionView(
                null, null, context,
                LabelConstants.SELECT_THE_MEMBERS_WHOM_YOU_WANT_TO_SPLIT));
    ...
    private void setHeadMemberSelectionScreen() {
        ...
        if (headSelected) {
            ...
            bodyLayoutContainer.addView(MyStaticComponents.generateInstructionView(context,
                    LabelConstants.NOT_SELECTED_TO_MOVE_THE_HEAD_FROM_THE_FAMILY_TO_NEW_SPLIT_FAMILY));
        }
        ...
        if (headSelected) {
            ...
            for (MemberDataBean memberDataBean : selectedMembers) {
                RadioButton radioButton = MyStaticComponents.getRadioButton(context,
                        memberDataBean.getUniqueHealthId() + " - " + UtilBean.getMemberFullName(memberDataBean),
                        counter);
                if (selectedHeadMember != null && selectedHeadMember.getId().equals(memberDataBean.getId())) {
                    radioButton.setChecked(true);
                }
                headRadioGroup.addView(radioButton);
                counter++;
    ...
    private void getRelWithHofScreen(List<MemberDataBean> listOfMembers) {
        screenName = REL_WITH_HOF_SCREEN;
        bodyLayoutContainer.removeAllViews();
        bodyLayoutContainer.addView(MyStaticComponents.generateInstructionView(context, "Select relation of below members with new head of family : " + UtilBean.getMemberFullName(selectedHeadMember)));
        LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams(MATCH_PARENT, MATCH_PARENT);
        LinearLayout verticalList = getLinearLayout(context, 10000, VERTICAL, layoutParams);
        for (MemberDataBean memberDataBean : listOfMembers) {
            verticalList.addView(getMemberAndRelWithHofView(memberDataBean));
        }
        bodyLayoutContainer.addView(verticalList);
    }

    public LinearLayout getMemberAndRelWithHofView(MemberDataBean memberDataBean) {
        LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams(MATCH_PARENT, WRAP_CONTENT);
        LinearLayout marriedMemberView = getLinearLayout(context, 10000, VERTICAL, layoutParams);
        marriedMemberView.addView(MyStaticComponents.generateInstructionView(context, UtilBean.getMemberFullName(memberDataBean)));
        final Spinner relationsSpinner = MyStaticComponents.getSpinner(context, GlobalTypes.RELATION_WITH_HOF_LIST.toArray(new String[0]), 0, 2);
        marriedMemberView.addView(relationsSpinner);
        relationsSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {

                relationWithHofMap.put(memberDataBean.getId(), relationsSpinner.getSelectedItem().toString());
            }
    ...
    private void saveSplitFamily() {
        ...
        migration.setExistingHeadSelected(headSelected);

        if (selectedHeadMember != null) {
            migration.setNewHead(Long.parseLong(selectedHeadMember.getId()));
        }

        if (relationWithHofMap != null && !relationWithHofMap.isEmpty()) {
            migration.setRelationWithHofMap(relationWithHofMap);
        }
