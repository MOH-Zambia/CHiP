...
public class LmsContentFragment extends Fragment {
    ...
    public void initView() {
        ...
        if (GlobalTypes.FLAVOUR_TELANGANA.equalsIgnoreCase(BuildConfig.FLAVOR)) {
            for (int i = 0; i < expandableListAdaptor.getGroupCount(); i++) {
    ...
    private void openLmsTestModule(LmsQuestionSetDataBean questionSet, String testFor) {
        ...
        for (LmsTopicDataBean topicDataBean : selectedCourse.getTopics()) {
            for (LmsLessonDataBean mediaDataBean : topicDataBean.getTopicMedias()) {
                if ((Boolean.TRUE.equals(mediaDataBean.getCompulsory()) ||
                        ...
                        !activity.lmsService.getViewedLessonList().contains(mediaDataBean.getActualId())) {
        ...
        if (allLessonsCompleted || (questionSet.getAllowedToSkipLessons() != null && questionSet.getAllowedToSkipLessons())) {
            ...
            if (activity.getSelectedModule() != null && activity.getSelectedModule().getTopicId() != null) {
                intent.putExtra("moduleId", activity.getSelectedModule().getTopicId());
            }
                ...
                if (UtilBean.isFileExists(path)) {
                    if (activity.getSelectedLesson() == null || !selectedLesson.getActualId().equals(activity.getSelectedLesson().getActualId())) {
                        if (Boolean.TRUE.equals(selectedCourse.getAllowedToSkipLessons()) || Boolean.TRUE.equals(activity.lmsService.getAllowedMediaMap(selectedCourse.getCourseId()).get(selectedLesson.getActualId()))) {
                            if (Boolean.TRUE.equals(activity.lmsService.isQuizCompleted(selectedModule.getCourseId(), selectedModule.getTopicId(), selectedLesson.getActualId()))) {
    ...
    private ExpandableListView.OnGroupCollapseListener getOnCollapseGroupListener() {
        return groupPosition -> expandableListView.clearChoices();
    }
