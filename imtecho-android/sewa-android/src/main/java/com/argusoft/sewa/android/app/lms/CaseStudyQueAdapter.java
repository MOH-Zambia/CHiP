...
public class CaseStudyQueAdapter extends RecyclerView.Adapter<CaseStudyQueAdapter.ViewHolder> {
    ...
    private QuestionImageClickListener questionImageClickListener;

    public CaseStudyQueAdapter(Context context, Activity activity, List<LmsQuestionConfigDataBean> questionConfigDataBeansList, IGetCaseStudyInterface iGetCaseStudyInterface, QuestionImageClickListener questionImageClickListener) {
        this.context = context;
        this.questionConfigDataBeansList = questionConfigDataBeansList;
        this.activity = activity;
        this.iGetCaseStudyInterface = iGetCaseStudyInterface;
        this.questionImageClickListener = questionImageClickListener;
    }
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        if (queDrawable != null) {
            String path = SewaConstants.getDirectoryPath(context, SewaConstants.DIR_LMS) + UtilBean.getLMSFileName(questionConfigDataBeansList.get(position).getMediaId(),
                    questionConfigDataBeansList.get(position).getMediaName());
            holder.questionImageView.setVisibility(View.VISIBLE);
            holder.questionImageView.setImageDrawable(queDrawable);

            holder.questionImageView.setOnClickListener(view -> {
                questionImageClickListener.questionImageClickListener(path);
            });
        }
    ...
    public interface QuestionImageClickListener {
        void questionImageClickListener(String path);
    }
}
