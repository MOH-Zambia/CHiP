...
course_status as (
	select enrolled_data.role_id as role_id,
		count(1) filter (
			where (
					course_progress between 1 and 99
				)
		) as in_progress,
		count(1) filter (
...
INSERT INTO QUERY_MASTER (uuid, created_by, created_on, modified_by, modified_on, code, params, query, description, returns_result_set, state )
VALUES (
'3d15d603-e3f6-42e6-a7d2-fd399b896947', 97157,  current_date , 97157,  current_date , 'lms_course_engagement_data',
'locationId,roleId',
'with counts as(
	...
	from tr_course_wise_count_analytics tcwca
		inner join um_user_location uul on tcwca.user_id = uul.user_id
	...
	where lhcd.parent_id = #locationId#
		and case
			when #roleId# is not null then uu.role_id= cast(#roleId# as integer) else true end group by tcwca.course_id),
			course_counts as(
				select counts.*,
					tcm.course_name
				from counts
			...
			course_details as(
				select count(distinct attendee_id) + count(distinct additional_attendee_id) filter(where uuld2 is not null and uu2 is not null) as enrolled_user_id,
					ttcr.course_id,
					tcm.course_name
				from tr_training_attendee_rel ttar
					...
					left join um_user_login_det uuld2 on uuld2.user_id = uul2.user_id
					left join um_user uu2 on uu2.id = uul.user_id and uu2.state=''ACTIVE''
					and lhcd2.parent_id = #locationId#	where tcm.course_state = ''ACTIVE'' and tcm.course_type=''ONLINE''
