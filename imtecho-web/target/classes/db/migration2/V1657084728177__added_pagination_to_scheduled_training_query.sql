...
VALUES ( 
'40a243ce-5253-4a6d-ae6e-45c0a93f1d74', 74724,  current_date , 74724,  current_date , 'tr_scheduled_trainings', 
	...
	inner join (
		 select count(*) as "total",
                ...
                from (
                        ...
                        from tr_training_attendee_rel inner join um_user
                            on tr_training_attendee_rel.attendee_id = um_user.id
                            where um_user.state =''ACTIVE''
                        union
                        select additional_attendee_id as attendee_id ,training_id
                        from tr_training_additional_attendee_rel inner join um_user
                            on tr_training_additional_attendee_rel.additional_attendee_id = um_user.id
                            where um_user.state =''ACTIVE''
                    ) r
                group by r.training_id) total on
