...
('Helpdesk Tool - Search','manage',TRUE,'techo.manage.searchfeature','{}');
...
f.migratory_flag, f.toilet_available_flag, f.vulnerable_flag, f.basic_state, m.id, emm.emamtalocationname',true,'ACTIVE','Retrieve family and members basic info using familyId');

DELETE FROM public.query_master
 WHERE code='retrieve_anc_information';
insert into query_master(created_by,created_on,code,params,query,returns_result_set,state,description)
values(1,now(),'retrieve_anc_information','healthid','select * from rch_anc_master where member_id = (select id from imt_member where unique_health_id = ''#healthid#'' limit 1) limit 3',true,'ACTIVE','Retrieve ANC Information of User');
