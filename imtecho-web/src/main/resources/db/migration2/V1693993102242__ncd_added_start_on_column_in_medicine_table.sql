...
latest3VisitTest AS (
	...
	WHERE NOT EXISTS (
		...
		and member_id = #memberId# ORDER BY disease_code,visit_date desc
),
getBmi as (
	select member_id,
		weight,
	...
	SELECT fmvt.member_id,
		...
		(
			...
			where getBmi.member_id = fmvt.member_id
				and cast(screening_date as date) = cast(fmvt.visit_date as date)
	...
	SELECT lvt.member_id,
		...
		(
			...
			where getBmi.member_id = lvt.member_id
				and cast(screening_date as date) = cast(lvt.visit_date as date)
