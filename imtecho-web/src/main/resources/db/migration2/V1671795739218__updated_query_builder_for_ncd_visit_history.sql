...
'memberId',
'select member_id,visit_date,disease_code "diseaseCode",status,reading,case when visit_by is not null then concat(visit_by,'': '',um_user.first_name,'' '',um_user.last_name) else concat(um_user.first_name,'' '',um_user.last_name) end "DiagnosedBy" from ncd_visit_history
