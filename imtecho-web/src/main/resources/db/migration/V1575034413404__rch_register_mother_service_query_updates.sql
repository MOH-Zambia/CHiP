...
,basic_member_info as (
    ...
    preg.member_name,
    case
    	when TRUNC(DATE_PART(''day'', cast(preg.date_of_delivery as timestamp)  - cast(preg.lmp_date as timestamp ))/7) > 11 then ''Yes''
    	else ''No''
...
,pre_delivery_care as (
	select cast( json_agg(t) as text) as pre_delivery_care_json from (
		...
		case
			when ins_place.value is null then ''N/A''
			else ins_place.value
		...
		case
			when cast(haemoglobin_count as text) is null then ''N/A''
		...
		case
			when cast(ifa_tablets_given as text) is null then ''N/A''
		...
		case
			when cast(anc.referral_done as text) is null
			then ''N/A'' else cast(anc.referral_done as text)
		...
		case
			when cast(dead_flag as text) is null then ''N/A''
			else cast(dead_flag as text)
		end as dead_flag,
		case
...
,delivery_result as(
	select cast(json_agg(t) as text) as delivery_result_json from (
		...
		case
			when was_premature is null then ''N/A''
		...
		case
			when child.breast_feeding_in_one_hour is null then ''N/A''
...
,pnc_visit as (
	...
	cast(json_agg(t) as text) as pnc_visit_json from (
		...
		case
			when mother_pnc.is_alive is null then ''N/A''
	    	when mother_pnc.is_alive then ''Yes''
	    	else ''No''
    	end as is_alive,
		case
			when mother_pnc.death_reason is null then ''N/A''
			else mother_pnc.death_reason
		end as death_reason,
		case
		...
		group by mas.id,mother_pnc.ifa_tablets_given, mother_pnc.other_danger_sign, referral.value,
			...
			mother_pnc.death_reason, mother_pnc.death_date, mother_pnc.place_of_death, im.dob
