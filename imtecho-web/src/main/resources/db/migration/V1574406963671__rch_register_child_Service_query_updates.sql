...
basic_info as (
	...
	case
		...
		case
			when child.birth_weight is not null then cast(ROUND(CAST(child.birth_weight as numeric), 2) as text)
			else null
		end as birth_weight,
		case
		...
		case
			when is_breastfed_till_six_month is null then ''N/A''
...
case
	when dpt_booster_child_weight is not null then cast(dpt_booster_child_weight as text)
	else ''N/A''
