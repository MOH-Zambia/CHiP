alter table rch_member_death_deatil add column other_death_reason text;

update query_master set 
params = 'member_id,family_id,location_id,location_hierarchy_id,action_by,death_date,place_of_death,death_reason,other_death_reason',
query = 'update techo_notification_master set state = ''MARK_AS_DEATH'', 
...
INSERT INTO public.rch_member_death_deatil(
	member_id, family_id, location_id, location_hierarchy_id, dod, death_reason, other_death_reason, place_of_death, created_on, created_by
)
VALUES(
	#member_id#, #family_id#, #location_id#, #location_hierarchy_id#, ''#date_of_death#'', 
...
and um_user.role_id in (select id from um_role_master where code in (''mo_corporation'',''mo_uphc'',''mo_phc''))
limit 1
