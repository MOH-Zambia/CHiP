...
values
((select id from menu_group where group_name='Reports' and group_type='manage'),true,'Distinct Performance Dashboard','techo.manage.districtperformancedashboard','manage');
...
'select
location_id
,(cast(concat(substr(#financial_year#,6,10),''-01-01'') as date) - cast(concat(substr(#financial_year#,1,4),''-01-01'') as date)) AS days
,financial_year
,expected_mother_reg
,anc_reg
,per_anc_reg
...
,sum(expected_no_mmr) as expected_no_mmr
...
,sum(expected_no_imr) as expected_no_imr
...
,case when sum(weighed) = 0 then 0 else round(sum(weighed_less_than_2_5)*100.0/sum(weighed) ,2) end as per_lbw
,null as lbw_rank
...
,case when sum(total_anemic) = 0 then 0 else round(sum(total_severe_anemic)*100.0/sum(total_anemic) ,2) end as per_total_sever_anemic
