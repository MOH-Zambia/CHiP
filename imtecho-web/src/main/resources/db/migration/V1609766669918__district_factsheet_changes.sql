...
,COALESCE(location_wise_analytics.fhs_total_member, 0) as fhs_total_member
,(select count(1) from location_master where type in (''D'', ''C'')) as total_number_of_districts
,(select count(1) from location_master where parent = (select parent from location_master where id = rch_dashboard_analytics.location_id)) as total_number_of_locations
from rch_dashboard_analytics
inner join location_wise_analytics on location_wise_analytics.loc_id = rch_dashboard_analytics.location_id
...
,COALESCE(sum(expected_fully_immu), 0) as fhs_total_member
,null as total_number_of_districts
