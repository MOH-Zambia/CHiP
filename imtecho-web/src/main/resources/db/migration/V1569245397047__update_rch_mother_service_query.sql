...
,pre_delivery_care as ( 
	select cast( json_agg(t) as text) as pre_delivery_care_json from (
		...
		case when immun.immunisation_given is not null and immun.immunisation_given = ''TT1'' then cast(immun.given_on as text) else ''N/A'' end as tt1,
		case when immun.immunisation_given is not null and immun.immunisation_given = ''TT2'' then cast(immun.given_on as text) else ''N/A'' end as tt2,
		case when immun.immunisation_given is not null and immun.immunisation_given = ''TT_BOOSTER'' then cast(immun.given_on as text) else ''N/A'' end as ttb,
        ...
        left join rch_immunisation_master immun on anc.id = immun.visit_id and immun.immunisation_given in ( ''TT1'',''TT2'',''TT_BOOSTER'') 		
