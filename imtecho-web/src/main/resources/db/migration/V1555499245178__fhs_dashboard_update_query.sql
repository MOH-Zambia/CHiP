update query_master set query = 'with data as (
...
coalesce(sum("totalMemberUnder5Year"),0),coalesce(sum("inReverification"),0),''Total'',null,#locationId#,coalesce(sum(worker),0),null from for_total;' where code = 'fhs_dashboard_locationwise_data';