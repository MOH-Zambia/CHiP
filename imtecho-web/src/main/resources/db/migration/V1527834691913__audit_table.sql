-- ********************* TO AUDIT A TABLE. YOU CAN Exclude FEILDS. SEE BELOW HELP
-- SELECT audit_table('tablename');
-- ********************* STOPE AUDITING A TABLE
-- SELECT stop_auditing('tablename');

-- ********************* FETCH DATA AS YOU WISH
-- select (r).* from
-- (select populate_record(null::test2 ,CASE WHEN action='U' THEN row_data || changed_fields
--      ELSE row_data
-- END  )as r,* from logged_actions where table_name = 'test2') s  
...
-- Time zone concerns
-- TIMESTAMP WITH TIME ZONE fields are always stored in UTC and are displayed in local time by default. You can control display using AT TIME ZONE in queries, or with SET timezone = 'UTC' as a per-session GUC. 
-- ********************************************
...
CREATE INDEX logged_actions_relid_idx ON public.logged_actions(relid);
CREATE INDEX logged_actions_action_tstamp_tx_stm_idx ON public.logged_actions(action_tstamp_stm);
CREATE INDEX logged_actions_action_idx ON public.logged_actions(action);
...
COMMENT ON FUNCTION public.if_modified_func() IS $body$
...
CREATE OR REPLACE FUNCTION public.audit_table(target_table regclass, audit_rows boolean, audit_query_text boolean, ignored_cols text[]) RETURNS void AS $body$
DECLARE
...
COMMENT ON FUNCTION public.audit_table(regclass, boolean, boolean, text[]) IS $body$
...
CREATE OR REPLACE FUNCTION public.audit_table(target_table regclass, audit_rows boolean, audit_query_text boolean) RETURNS void AS $body$
...
--
CREATE OR REPLACE FUNCTION public.audit_table(target_table regclass) RETURNS void AS $body$
SELECT public.audit_table($1, BOOLEAN 't', BOOLEAN 't');
$body$ LANGUAGE 'sql';

COMMENT ON FUNCTION public.audit_table(regclass) IS $body$
Add auditing support to the given table. Row-level changes will be logged with full client query text. No cols are ignored.
$body$;

CREATE OR REPLACE FUNCTION public.stop_auditing(target_table regclass) RETURNS void AS $body$
DECLARE
...
select audit_table('report_master');
select audit_table('report_parameter_master');
select audit_table('query_master');
select audit_table('report_query_master');
select audit_table('event_configuration');
select audit_table('event_configuration_type');
select audit_table('event_mobile_configuration');
select audit_table('event_mobile_notification_pending');