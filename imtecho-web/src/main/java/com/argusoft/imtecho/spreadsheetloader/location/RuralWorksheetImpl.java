...
/**
 * <p>
 *     Defines methods for rural work sheet.
 * </p>
 * @author avani
 * @since 07/09/2020 10:30
 */
public class RuralWorksheetImpl extends LocationWorkSheetImpl {

    public static final Logger logger = Logger.getLogger(RuralWorksheetImpl.class);

    private String workSheetName;

    private LocationMaster[] parentLocation;

    private LocationConstants.RuralHeader[] columnNames = null;

    public RuralWorksheetImpl(String workSheetName) {
    ...
    /**
     * Read Rural worksheet and parse location
     */
    @Override
    public void loadLocations() {
        ...
        parentLocation = new LocationMaster[LocationConstants.RuralHeader.values().length];
    ...
    private String renderToLocation(String errorKey, Map<String, Map<Integer, List<String>>> rowColumnMap) {
        ...
        columnNames = LocationConstants.RuralHeader.values();
                    ...
                    if (!value.equalsIgnoreCase("")) {
                        ...
                        if (errorKey != null && !errorKey.equalsIgnoreCase("")) {
                            ...
                            if (!identifyParent(locationTypestr, (LocationConstants.LocationType.valueOf(enumConstant)).getType())) {
                                ...
                                if (!LocationConstants.RuralHeader.EMAMTA_CODE.name().equalsIgnoreCase(column)) {
                                    this.increment(results, LocationResultEnum.SKIP_LOCATION);
                                }
                                if (!rowColumnMap.containsKey(String.valueOf(wsReader.getCurrentRowIndex()))) {
                        ...
                        try {
                            ...
                            if (subString.matches(LocationConstants.DIGIT_REGEX)) {
                                ...
                                if (LocationConstants.LocationType.AREA.getType().equalsIgnoreCase(locationType)) {
                                    sb.delete(value.indexOf("("), sb.length());
                                    value = sb.toString().trim();
                                }
                            }
    ...
    public String processLocation(int i, String column, String value, String errorKey, String enumConstant, String locationCode) {
        ...
        try {
            ...
            if (treeSet.isEmpty()) {
                ...
                } else if (LocationConstants.RuralHeader.VILLAGE.name().equalsIgnoreCase(column)) {

                    if (!checkEmamtaCodeExistsOrNot(i)) {
                        throw new LocationException(LocationConstants.EMAMTA_CODE_NOT_EXIST_ERROR_RURAL);
                    } else {
     ...
     * @param locationCode Location code.
     * @param index Index.
     * @throws LocationException Location exception.
     */
    public void setLocationCodeToLocation(String locationCode, int index) throws LocationException {
        ...
        try {
            ...
            if (LocationConstants.RuralHeader.VILLAGE.name().equalsIgnoreCase(prevColumn)) {

                if (prevValue != null && !prevValue.equalsIgnoreCase("")) {

                    String prevEnumConstant = prevColumn.replace(" / ", "_").replace(" ", "_");
                    ...
                    if (prevValue.contains("(")) {
                        ...
                        if (prevValue.contains(")")) {
                            subString = prevValue.substring(prevValue.indexOf("(") + 1, prevValue.indexOf(")"));
                        } else {
                            subString = prevValue.substring(prevValue.indexOf("(") + 1, sb.length());
                        }
                        if (subString.matches(LocationConstants.DIGIT_REGEX)) {
            ...
            throw new LocationException(LocationConstants.EMAMTA_CODE_ERROR_RURAL);
        }
    }
    ...
    private LocationMaster setLocationObjectValues(String locationKey, String value, int i, int rowNum, String locationType, String locationCode) throws LocationException {
        ...
        //Set location type as HierarchyId_LocationType , HierarchyId is 'H1,H2,H3'
    ...
    /**
     * Identify parent for skip location when error occurred
     *
     * @param errorlocationType Error location type.
     * @param locationType Location type.
     * @return boolean false if need to skip.
     */
    public boolean identifyParent(String errorlocationType, String locationType) {

        if (LocationConstants.LocationType.BLOCK.getType().equalsIgnoreCase(errorlocationType)) {
            if (LocationConstants.LocationType.DISTRICT.getType().equalsIgnoreCase(locationType)
                    || LocationConstants.LocationType.BLOCK.getType().equalsIgnoreCase(locationType)) {
                return true;
            } else if (LocationConstants.LocationType.PHC.getType().equalsIgnoreCase(locationType)
                    || LocationConstants.LocationType.SUB_CENTER.getType().equalsIgnoreCase(locationType)
                    || LocationConstants.LocationType.VILLAGE.getType().equalsIgnoreCase(locationType)
                    || LocationConstants.LocationType.AREA.getType().equalsIgnoreCase(locationType)) {
                return false;
            }

        } else if (LocationConstants.LocationType.PHC.getType().equalsIgnoreCase(errorlocationType)) {

            if (LocationConstants.LocationType.DISTRICT.getType().equalsIgnoreCase(locationType)
                    || LocationConstants.LocationType.BLOCK.getType().equalsIgnoreCase(locationType)
                    || LocationConstants.LocationType.PHC.getType().equalsIgnoreCase(locationType)) {
                return true;
            } else if (LocationConstants.LocationType.SUB_CENTER.getType().equalsIgnoreCase(locationType)
                    || LocationConstants.LocationType.VILLAGE.getType().equalsIgnoreCase(locationType)
                    || LocationConstants.LocationType.AREA.getType().equalsIgnoreCase(locationType)) {
        ...
        } else if (LocationConstants.LocationType.SUB_CENTER.getType().equalsIgnoreCase(errorlocationType)) {
            ...
            if (LocationConstants.LocationType.DISTRICT.getType().equalsIgnoreCase(locationType)
                    ...
                    || LocationConstants.LocationType.SUB_CENTER.getType().equalsIgnoreCase(locationType)) {
                return true;
            } else if (LocationConstants.LocationType.VILLAGE.getType().equalsIgnoreCase(locationType)
                    || LocationConstants.LocationType.AREA.getType().equalsIgnoreCase(locationType)) {
                return false;
            }

        } else if (LocationConstants.LocationType.VILLAGE.getType().equalsIgnoreCase(errorlocationType)) {
            ...
            if (LocationConstants.LocationType.DISTRICT.getType().equalsIgnoreCase(locationType)
                    ...
                    || LocationConstants.LocationType.VILLAGE.getType().equalsIgnoreCase(locationType)) {
                return true;
            } else if (LocationConstants.LocationType.AREA.getType().equalsIgnoreCase(locationType)) {
                return false;
            }

        } else if (LocationConstants.LocationType.AREA.getType().equalsIgnoreCase(errorlocationType)) {
            return true;
    ...
    /**
     * Copy data from one spread sheet to other and highlight error column
     *
                ...
                if (columnDetail != null && !columnDetail.isEmpty()) {
                    ...
                    for (Map.Entry<Integer, List<String>> columnValues : columnDetail.entrySet()) {
                        ...
                        while (col < columnNames.length) {
                            ...
                            XSSFCell outCell = outRow.createCell(col);
                            XSSFCell cell = row.getCell(col);
                            if (col != 5 && outCell != null && cell != null) {
    ...
    private String prepareErrorMap(String locationKey, int i, String errorKey, String exception, String errorHierarchy) {
        ...
        if (LocationConstants.LocationType.AREA.getType().equalsIgnoreCase(type)) {
            ...
            rowColumnMap.put(String.valueOf(wsReader.getCurrentRowIndex() - 1), columnValuesMap);
            ...
            columnValuesMap.put(i, Arrays.asList(errorHierarchy.split(">")));
            ...
            try {
                ...
                if (errorKey != null && !errorKey.equalsIgnoreCase("")) {
                    ...
                    if (!this.identifyParent(locationType, locationObj.getKey().split("_")[1])) {
                        ...
                        if (!rowColumnMap.containsKey(String.valueOf(rowIndexMap.get(locationObj.getKey()).get(0)))) {
                ...
                if (dbLocation == null) {
                    ...
                    if (LocationConstants.LocationType.DISTRICT.getType().equalsIgnoreCase(location.getType())) {
                ...
                if (LocationConstants.LocationType.AREA.getType().equalsIgnoreCase(location.getType())) {
                    ...
                    rowColumnMap.put(rowIndexMap.get(locationObj.getKey()).get(0).toString(), columnValuesMap);
                }
    ...
    /**
     * Set state as parent in District
     *
     * @param district Location district details.
     */
    private void setStateInDistict(LocationMaster district) {
        ...
        if (district != null) {
            ...
            if (state != null && !state.isEmpty()) {
                for (LocationMaster stateLocation : state) {
                    ...
                    if (LocationConstants.GUJARAT_STATE_ENGLISH.equalsIgnoreCase(stateName) || LocationConstants.GUJARAT_STATE_GUJARATI.equals(stateName)) {
                        district.setParentMaster(stateLocation);
                    }
            ...
            getLocationDao().update(district);
        }
    ...
    public String getWorkSheetHierarchy() {
        return LocationConstants.RURAL_HIERARCHY;
    }
    
    /**
     * Check if emamta code is exists or not 
     * @param i Index.
     * @return Returns true/false based on emamta code is exists or not.
     */
