...
public class ReportMasterDaoImpl
    ...
    public List<UUID> getAllUUIDs() {
        String queryString = "SELECT DISTINCT r.uuid FROM ReportMaster r ORDER BY r.uuid ASC ";
        Query<UUID> query = sessionFactory.getCurrentSession().createQuery(queryString);
        return query.list();
    }
    ...
    public ReportMaster getReportMasterWithParamterByUUIDOrCode(UUID uuid, String code) {
        ...
        Query<ReportMaster> query = sessionFactory.getCurrentSession().createQuery(queryString);
        if (code != null && !code.isEmpty()) {
            query.setParameter("codeParam", code);
        } else {
...
//        query.setParameter("sortBy", sortBy);
    ...
    @Override
    public ReportMaster retrieveByUUID(UUID uuid) {
        Session session = sessionFactory.getCurrentSession();
