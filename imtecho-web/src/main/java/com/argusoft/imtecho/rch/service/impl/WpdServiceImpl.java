...
import com.argusoft.imtecho.rch.dao.RchInstitutionMasterDao;
...
import com.argusoft.imtecho.rch.mapper.WpdMapper;
...
public class WpdServiceImpl implements WpdService {
    ...
    public Integer storeWpdVisitForm(ParsedRecordBean parsedRecordBean, Map<String, String> keyAndAnswerMap, UserMaster user) {
        ...
        wpdMotherMaster.setLocationHierarchyId(locationLevelHierarchy.getId());
        ...
        for (Map.Entry<String, String> entrySet : childKeyAndAnswerMap.entrySet()) {
            ...
            if (keyAndAnswerMap.containsKey("8673")) {
                if (keyAndAnswerMap.get("-20") != null && !keyAndAnswerMap.get("-20").equalsIgnoreCase("null")) {
                    wpdChildMaster.setReferralPlace(Integer.valueOf(keyAndAnswerMap.get("-20")));
                }
            }
        }
            ...
            if (wpdChildMaster.getPregnancyOutcome() != null
                ...
                if (!loopId.equals("0")) {
                    ...
                    if (keyAndAnswerMap.get("87" + "." + loopId) != null && !keyAndAnswerMap.get("87" + "." + loopId).equals("NONE")) {
                        ...
                        ImmunisationMaster immunisationMaster = new ImmunisationMaster(familyId, childEntity.getId(), MobileConstantUtil.CHILD_BENEFICIARY,
                                MobileConstantUtil.WPD_VISIT, wpdChildMaster.getId(), Integer.valueOf(parsedRecordBean.getNotificationId()),
                                vaccineGiven.trim(), givenOn, user.getId(), locationId, locationLevelHierarchy.getId(), null);
                        StringBuilder immunisationGiven = new StringBuilder();
                        if (immunisationService.checkImmunisationEntry(immunisationMaster)) {
                            immunisationService.createImmunisationMaster(immunisationMaster);
    ...
    private void setAnswersToWpdMotherMaster(String key, String answer, WpdMotherMaster wpdMotherMaster, Map<String, String> keyAndAnswerMap) {
        switch (key) {
            ...
            case "77":
                if (keyAndAnswerMap.get("51").equalsIgnoreCase(RchConstants.MEMBER_STATUS_AVAILABLE)
                        && keyAndAnswerMap.get("511").equalsIgnoreCase("1")
                        && keyAndAnswerMap.get("7").equalsIgnoreCase(RchConstants.OUT_OF_UT)) {
                    wpdMotherMaster.setOtherStateOfDelivery(answer);
                }
                break;
            case "76":
                if (keyAndAnswerMap.get("51").equalsIgnoreCase(RchConstants.MEMBER_STATUS_AVAILABLE)) {
            ...
            case "2705":
                if (keyAndAnswerMap.get("51").equalsIgnoreCase(RchConstants.MEMBER_STATUS_AVAILABLE)
                        && keyAndAnswerMap.get("511").equalsIgnoreCase("1")
                        && keyAndAnswerMap.get("20").equalsIgnoreCase("2")) {
                    wpdMotherMaster.setOtherDeathPlace(answer);
                }
                break;
            case "26":
                if (keyAndAnswerMap.get("51").equalsIgnoreCase(RchConstants.MEMBER_STATUS_AVAILABLE)
            ...
            case "8001":
                ...
                break;
            case "3333":
                if (!answer.trim().isEmpty() && keyAndAnswerMap.get("51").equalsIgnoreCase(RchConstants.MEMBER_STATUS_AVAILABLE)
                        && keyAndAnswerMap.get("511").equalsIgnoreCase("1")
                        && keyAndAnswerMap.get("20").equals("1")) {
                    wpdMotherMaster.setReferralReason(answer);
                }
                break;
            case "3334":
                if (!answer.trim().isEmpty() && keyAndAnswerMap.get("51").equalsIgnoreCase(RchConstants.MEMBER_STATUS_AVAILABLE)
                        && keyAndAnswerMap.get("511").equalsIgnoreCase("1")
                        && keyAndAnswerMap.get("20").equals("1")) {
                    wpdMotherMaster.setReferralFor(answer);
                }
                break;
            case "8989":
                if (keyAndAnswerMap.get("51").equalsIgnoreCase(RchConstants.MEMBER_STATUS_AVAILABLE)) {
                    wpdMotherMaster.setIecGiven(ImtechoUtil.returnTrueFalseFromInitials(answer));
                }
                break;
            case "141":
                if (keyAndAnswerMap.get("51").equalsIgnoreCase(RchConstants.MEMBER_STATUS_AVAILABLE)
            ...
            case "2101":
                if (keyAndAnswerMap.get("51").equalsIgnoreCase(RchConstants.MEMBER_STATUS_AVAILABLE)
                        && keyAndAnswerMap.get("511").equalsIgnoreCase("1")
                        && keyAndAnswerMap.containsKey("21") && keyAndAnswerMap.get("21").equalsIgnoreCase("2")) {
                    wpdMotherMaster.setNoBreastFeedingReason(answer);
                }
                break;
            case "1801":
            ...
            case "66":
                ...
                break;
            case "6667":
                wpdMotherMaster.setEarlyDischargeReason(answer);
                break;
            default:
        }
    }
    ...
    private void setAnswersToWpdChildMaster(String key, String answer, WpdChildMaster wpdChildMaster, Map<String, String> keyAndAnswerMap, String childCount) {
        switch (key) {
            ...
            case "8004":
                ...
                break;
            case "2222":
                if (!answer.trim().isEmpty() && (keyAndAnswerMap.get("14").equalsIgnoreCase(RchConstants.PREGNANCY_OUTCOME_LIVE_BIRTH)
                        || keyAndAnswerMap.get("14").equalsIgnoreCase(RchConstants.PREGNANCY_OUTCOME_PREMATURE))) {
                    wpdChildMaster.setReferralReason(answer);
                }
                break;
            case "2555":
                if (!answer.trim().isEmpty() && (keyAndAnswerMap.get("14").equalsIgnoreCase(RchConstants.PREGNANCY_OUTCOME_LIVE_BIRTH)
                        || keyAndAnswerMap.get("14").equalsIgnoreCase(RchConstants.PREGNANCY_OUTCOME_PREMATURE))) {
                    wpdChildMaster.setReferralFor(answer);
                }
                break;
            case "16":
                if (keyAndAnswerMap.get("14").equalsIgnoreCase(RchConstants.PREGNANCY_OUTCOME_LIVE_BIRTH)
            ...
            case "161":
                if (keyAndAnswerMap.get("14").equalsIgnoreCase(RchConstants.PREGNANCY_OUTCOME_LIVE_BIRTH)
                        || keyAndAnswerMap.get("14").equalsIgnoreCase(RchConstants.PREGNANCY_OUTCOME_STILL_BIRTH)
                        || keyAndAnswerMap.get("14").equalsIgnoreCase(RchConstants.PREGNANCY_OUTCOME_PREMATURE)) {
                    wpdChildMaster.setBirthLength(Integer.valueOf(answer));
                }
                break;
            case "17":
                if (keyAndAnswerMap.get("14").equalsIgnoreCase(RchConstants.PREGNANCY_OUTCOME_LIVE_BIRTH)
            ...
            case "1332":
                if (keyAndAnswerMap.get("14").equalsIgnoreCase(RchConstants.PREGNANCY_OUTCOME_LIVE_BIRTH)
                        || keyAndAnswerMap.get("14").equalsIgnoreCase(RchConstants.PREGNANCY_OUTCOME_PREMATURE)) {
                    wpdChildMaster.setAntiRetroViralTherapyStarted(ImtechoUtil.returnTrueFalseFromInitials(answer));
                }
                break;
            default:
        }
    ...
    private List<String> getKeysForWpdMotherMasterQuestions() {
        ...
        keys.add("3333");       //Referral Reason
        keys.add("3334");       //Referral Reason
        keys.add("8989");       //IEC
    ...
    public List<MemberDto> create(WpdMasterDto wpdMasterDto) {
        ...
        if (wpdMotherMaster.getLocationHierarchyId() == null) {
            wpdMotherMaster.setLocationHierarchyId(locationLevelHierarchyDao.retrieveByLocationId(wpdMotherMaster.getLocationId()).getId());
        }
        ...
        if (!wpdMotherMaster.getDeliveryPlace().equals("108_AMBULANCE") && (!wpdMotherMaster.getDeliveryPlace().equals("HOME")) && wpdMotherMaster.getHealthInfrastructureId() == null) {
            throw new ImtechoUserException("Please Select Health Infrastructure", 1);
        }
        wpdMotherDao.create(wpdMotherMaster);
        updateMemberAdditionalInfo(memberEntity, wpdMotherMaster, null);

        for (WpdChildDto wpdChildDto : wpdMasterDto.getChildDetails()) {
            WpdChildMaster wpdChildMaster = WpdMapper.convertWpdChildDtoToWpdChildMaster(wpdChildDto);
            if (wpdChildMaster != null) {
                if (wpdChildMaster.getLocationHierarchyId() == null) {
                    wpdChildMaster.setLocationHierarchyId(wpdMotherMaster.getLocationHierarchyId());
                }
                ...
                for (ImmunisationDto immunisationDto : wpdChildDto.getImmunisationDetails()) {
                    ImmunisationMaster immunisationMaster = new ImmunisationMaster(wpdChildMaster.getFamilyId(), wpdChildMaster.getMemberId(),
                            ...
                            wpdChildMaster.getLocationId(), wpdChildMaster.getLocationHierarchyId(), null);
                    immunisationService.createImmunisationMaster(immunisationMaster);
                }
            }
        }
                    ...
                    if (childMemberEntity != null) {
                        ...
                        for (ImmunisationDto immunisationDto : wpdChildDto.getImmunisationDetails()) {
                            ImmunisationMaster immunisationMaster = new ImmunisationMaster(wpdChildDto.getFamilyId(), wpdChildDto.getMemberId(),
                                    ...
                                    immunisationDto.getImmunisationGiven().trim(), immunisationDto.getImmunisationDate(), user.getId(),
                                    wpdChildDto.getLocationId(), wpdChildDto.getLocationHierarchyId(), null);
                            immunisationService.createImmunisationMaster(immunisationMaster);
                        }
                    }
                }
        ...
        if (memberEntity.getAdditionalInfo() != null && !memberEntity.getAdditionalInfo().isEmpty()) {
            ...
            if (keyAndAnswerMap != null && keyAndAnswerMap.containsKey("29")) {
                if (keyAndAnswerMap.get("29") != null
                    ...
                    memberAdditionalInfo.setLastServiceLongDate(new Date(Long.parseLong(keyAndAnswerMap.get("29"))).getTime());
                    isUpdate = true;
                }
            }
