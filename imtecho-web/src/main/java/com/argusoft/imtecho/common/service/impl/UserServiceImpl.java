...
import com.argusoft.imtecho.common.databean.OtpDataBean;
...
import com.argusoft.imtecho.common.model.*;
...
import com.argusoft.imtecho.mobile.dao.MobileMenuMasterDao;
import com.argusoft.imtecho.mobile.dto.LoggedInUserPrincipleDto;
import com.argusoft.imtecho.mobile.dto.MenuDataBean;
import com.argusoft.imtecho.mobile.dto.MobUserInfoDataBean;
...
import com.argusoft.imtecho.training.dao.TrainingDao;
...
//
//        }
        if (Boolean.TRUE.equals(this.checkIfUserExistsWithSamePhone(user.getContactNumber(), user.getId(), null)) && user.getId() == null) {
            throw new ImtechoUserException("Phone Number is alerady linked to another User", 101);
        }


        if (user.getPassword() == null && user.getId() != null) {
            user.setPassword(userDao.retrieveById(user.getId()).getPassword());
            user.setFirstTimePasswordChanged(true);
        }
        if (user.getId() == null) {
...
//        }
    ...
    private void handleInvalidOtp(UserMaster user, UserMasterDto userDto) {
        ...
        if (Boolean.TRUE.equals(this.checkIfUserExistsWithSamePhone(user.getContactNumber(), user.getId(), user.getRoleId())
                && user.getId() == null)) {
    ...
    public UserMasterDto retrieveByUserName(String userName) {
        ...
        if (retrieveByUserName != null) {
            ...
            UserLoginDetailMaster userLoginDetailMaster = userLoginDetailMasterDao.getLastLoginDetailByUserId(retrieveByUserName.getId());
            if (Objects.nonNull(userLoginDetailMaster)) {
                userMasterDto.setUserLastLogin(userLoginDetailMaster.getCreatedOn());
            }
     ...
     */
    @Override
    public String validateAndGenerateOtp(String username) {
        ...
        String contactNumber = usermaster.getContactNumber();

        String message = "Your code for forgot password verification is " + randInt + ". DO NOT share this with anyone.";
...
//    private void sendTwillioSms(String mobile, String msg) {
//        try {
//            Twilio.init("AC37a85d64c7cffd0e8d9e5b74ca5363c0", "27f8df8d61f5e0370754e8f04615d2b9");
//            Message message = Message.creator(
//                            new com.twilio.type.PhoneNumber("+91" + mobile),
//                            new com.twilio.type.PhoneNumber("+19177468807"),
//                            msg)
//                    .create();
//        } catch (Exception e) {
//            e.printStackTrace();
...
//        }
     ...
     */
    @Override
    public Boolean checkIfUserExistsWithSamePhone(String contactNumber, Integer userId, Integer roleId) {
    ...
    private void addLocationTYpeAsPerRole(UserMaster mobileUser, List<String> locationTypes, List<Integer> parentIds, MobUserInfoDataBean mobileUserDataBean) {
        ...
        if (userLocations != null && !userLocations.isEmpty()) {
            ...
            throw new ImtechoMobileException(MobileConstantUtil.LOCATION_NOT_ASSIGNED, 1);
        }
    }


    private MobUserInfoDataBean convertUserModelToMobUserInfoDatabean(MobUserInfoDataBean userInfo, UserMaster user, List<LocationMaster> userLocations) {
        ...
        if (user != null) {
            ...
            userInfo.setFirstTimePasswordChanged(Boolean.TRUE.equals(user.getFirstTimePasswordChanged()));
            userInfo.setIsSmagTrained(user.getUserSmagTrained() != null ? user.getUserSmagTrained() : false);
            userInfo.setGetLastLoginDate(getLastLoginDate(user.getId()));
    ...
    private MobUserInfoDataBean getUserToken(MobUserInfoDataBean user, boolean checkTokenValidity) {
        String token;
    ...
    @Override
    public Long getLastLoginDate(Integer userId) {
        UserLoginDetailMaster userLoginDetailMaster = userLoginDetailMasterDao.getLastLoginDetailByUserId(userId);
        if (userLoginDetailMaster != null) {
            return userLoginDetailMaster.getCreatedOn().getTime();
        }
        return null;
    }
    ...
    @Override
    public void updateUserDetailsFromMobile(String token, MobUserInfoDataBean userDetails) {
        UserMaster user = getUserByValidToken(token);
        if (user == null) {
            throw new ImtechoMobileException("Unauthorized", 403);
        }
