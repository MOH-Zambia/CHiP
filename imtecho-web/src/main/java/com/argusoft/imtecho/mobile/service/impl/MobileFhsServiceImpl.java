...
import com.argusoft.imtecho.mobile.dao.MobileFormDao;
...
import com.argusoft.imtecho.rch.service.MemberPregnancyStatusService;
import com.argusoft.imtecho.systemconstraint.mapper.SystemConstraintComponentTagDtoMapper;
import com.argusoft.imtecho.systemconstraint.service.SystemConstraintService;
                ...
                if (paramDetailDto.getSheetNameVersionMap() != null && !paramDetailDto.getSheetNameVersionMap().isEmpty()) {
                    ...
                    sheetNameVersionMap.forEach((String sheetNameVersionKey, Integer version) -> {
                        ...
                        if (version.intValue() != systemVersion.intValue()) {
                            loggedInUserPrincipleDto.setRetrievedXlsData(SystemConstantUtil.xlsDataFhw);
...
//        System.out.println("notification time:" + (new Date().getTime() - d1.getTime()));
    ...
    public QueryDto retrieveLabels(Long createdOnDate, String prefferedLanguage) {
        if (prefferedLanguage != null) {
            ...
            if ("GU".equals(prefferedLanguage)) {
                givenLanguage = prefferedLanguage;

                //condition due to old app version had only 2 language option (English-EN, Implementation_wise_language-GU)
                switch (ConstantUtil.IMPLEMENTATION_TYPE) {
                    case ConstantUtil.MEDPLAT_IMPLEMENTATION:
                        prefferedLanguage = "gu"; //Gujarati
                        break;
                    case ConstantUtil.TELANGANA_IMPLEMENTATION:
                        prefferedLanguage = "te"; //Telugu
                        break;
                    default:
                        prefferedLanguage = "hi"; //Hindi
                }
            }
            ...
            parameters.put("givenLanguage", givenLanguage);
            queryDto.setParameters(parameters);
            List<QueryDto> queryDtos = new LinkedList<>();
            queryDtos.add(queryDto);
            List<QueryDto> resultQueryDto = queryMasterService.executeQuery(queryDtos, true);
                ...
                if (version.intValue() != systemVersion.intValue()) {
                    switch (sheetNameVersionKey) {
                        ...
                        case SystemConstantUtil.ASHA_SHEET_VERSION:
                            loggedInUserPrincipleDto.setRetrievedXlsData(SystemConstantUtil.xlsDataAsha);
                            break;
                        case SystemConstantUtil.AWW_SHEET_VERSION:
                            loggedInUserPrincipleDto.setRetrievedXlsData(SystemConstantUtil.xlsDataAww);
                            break;
                        default:
                            return;
                    }
