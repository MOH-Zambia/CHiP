package com.argusoft.imtecho.database.common.impl;

import com.argusoft.imtecho.common.model.IdAnnotation;
...
public abstract class GenericDaoImpl<EntityType, IDType extends Serializable>
    ...
    public EntityType retrieveById(IDType id) {
        ...
        if (persistentClass.isAnnotationPresent(IdAnnotation.class)) {
            CriteriaBuilder cb = session.getCriteriaBuilder();
            CriteriaQuery<EntityType> cq = cb.createQuery(persistentClass);
            Root<EntityType> root = cq.from(persistentClass);
            cq.select(root).where(cb.equal(root.get(persistentClass.getAnnotation(IdAnnotation.class).idFieldName()), id));
