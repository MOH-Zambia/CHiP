...
import com.argusoft.imtecho.location.dto.LocationElasticHierarchyDto;
...
public class LocarionMasterDaoImpl extends GenericDaoImpl<LocationMaster, Integer> implements LocationMasterDao {
    ...
    public List<LocationMaster> retrieveUserLocationByLevel(Integer userId, Integer level) {
        ...
        subquery.select(join.get(LocationMaster.Fields.LOCATION_HIERARCHY).get(getLevel(level))).distinct(true);
    ...
    public List<LocationDetailDto> retrieveUserLocationByParentLocation(Integer userId, List<Integer> locationIds, Integer level, String languagePreference) {
        ...
        sQLQuery.setParameter(LocationLevelHierarchy.Fields.LOCATION_ID, locationIds);
        sQLQuery.setParameter(LocationLevelHierarchy.Fields.USER_ID, userId);
        sQLQuery.setParameter(LocationLevelHierarchy.Fields.LEVEL, level);
        sQLQuery.setParameter(LocationLevelHierarchy.Fields.PREFERRED_LANG, languagePreference);
        ...
        sQLQuery.addScalar(LocationLevelHierarchy.Fields.LOC_TYPE, StandardBasicTypes.STRING);
    ...
    public List<LocationMaster> retrieveLocationByLevel(Integer level) {
        ...
        Subquery<LocationLevelHierarchy> subquery = criteria.subquery(LocationLevelHierarchy.class);
        Root<LocationLevelHierarchy> subqueryForm = subquery.from(LocationLevelHierarchy.class);
        subquery.select(subqueryForm.get(getLevel(level)));
    ...
    private String getLevel(Integer level) {
        return LocationLevelHierarchy.Fields.LEVEL + level;
    }
    ...
    public List<LocationMaster> getLocationsByNameAndParent(String name, String type, LocationMaster parent) {
        ...
        CriteriaQuery<LocationMaster> criteria = criteriaBuilder.createQuery(LocationMaster.class);
        Root<LocationMaster> root = criteria.from(LocationMaster.class);
        root.join(LocationMaster.Fields.LOCATION_HIERARCHY);
        List<Predicate> predicates = new ArrayList<>();
        if (type != null) {
    ...
    public List<LocationMasterDataBean> retrieveAllActiveLocationsWithWorkerInfo(Date lastUpdatedOn) {
        ...
        if (lastUpdatedOn != null) {
            ...
            query = "with t as (\n" +
                    ...
                    "\tas fhw_names from um_user u inner join um_user_location ul on u.id = ul.user_id and u.role_id in (30,24,245) and ul.state = 'ACTIVE' \n" +
    ...
    public List<SurveyLocationMobDataBean> retrieveAllLocationForMobileByUserId(Integer userId, Integer roleId) {
        ...
        NativeQuery<SurveyLocationMobDataBean> q = session.createNativeQuery(query)
                ...
                .addScalar("name", StandardBasicTypes.STRING)
                .addScalar(LocationLevelHierarchy.Fields.PARENT, StandardBasicTypes.INTEGER)
                .addScalar(LocationLevelHierarchy.Fields.IS_ACTIVE, StandardBasicTypes.BOOLEAN)
                .addScalar(LocationLevelHierarchy.Fields.LEVEL, StandardBasicTypes.INTEGER)
                .addScalar("lgdCode", StandardBasicTypes.STRING);
        q.setParameter(LocationLevelHierarchy.Fields.USER_ID, userId);
     ...
     */
    @Override
    public List<SurveyLocationMobDataBean> retrieveAllLocationsAssignedToFhsrForMobileByUserId(Integer userId) {
        String query = "with loc_det as( "
                + "select child_id from location_hierchy_closer_det where  location_hierchy_closer_det.parent_id in ( "
                + "select loc_id from um_user_location where user_id = :userId and state = 'ACTIVE' "
                + ") "
                + ") "
                + "select distinct location_master.id, "
                + "location_master.state, "
                + "location_master.name, "
                + "location_master.parent, "
                + "case when location_master.state = 'ACTIVE' then true else false end as \"isActive\", "
                + "case "
                + "when location_master.type in ('P', 'U') then 4 "
                + "when location_master.type in ('SC', 'ANM') then 5 "
                ...
                + "end as level "
                + "from loc_det "
                + "inner join location_master on loc_det.child_id = location_master.id";
    ...
    public String retrieveLocationHierarchyById(Integer locationId) {
        ...
        q.setParameter(LocationLevelHierarchy.Fields.LOCATION_ID, locationId);
    ...
    public List<LocationMaster> retrieveBlockLocationsByUserId(Integer userId) {
        String query = "select * from location_master where id in\n"
    ...
    public String retrieveWorkerInfoByLocationId(Integer locationId) {
        ...
        sQLQuery.setParameter(LocationLevelHierarchy.Fields.LOCATION_ID, locationId);
    ...
    public List<LocationElasticHierarchyDto> retriveLocationListByIds(List<Integer> ids) {
        ...
        List<LocationElasticHierarchyDto> elasticHierarchyDtos = sQLQuery
                .addScalar(LocationLevelHierarchy.Fields.LOCATION_ID, StandardBasicTypes.INTEGER)
    ...
    @Override
    public List<Integer> getParentStateLevelLocationId() {

        String query = "select id from location_master where type='S'";
        Session session = sessionFactory.getCurrentSession();
    ...
    @Override
    public List<String> getParentStateLevelLocationName() {

        String query = "select name from location_master where type='S'";
        Session session = sessionFactory.getCurrentSession();
