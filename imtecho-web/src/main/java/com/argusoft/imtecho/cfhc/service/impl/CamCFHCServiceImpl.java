...
@Service
@Transactional
public class CamCFHCServiceImpl implements CamCFHCService {
    ...
    private List<String> getKeysForFamilyQuestions() {
        ...
        keys.add("6");          //House Number
        keys.add("7");          //Address Line 1
        keys.add("8");          //Address Line 2
        ...
        keys.add("13");         //Eligible women count
        keys.add("14");         //Eligible child count
        keys.add("15");         //Does household hold an ID poor?
        ...
        keys.add("-2");         //Latitude
        keys.add("5");          //Are family providing consent to participate in the i-MoMCARE research?
    ...
    public Map<String, String> storeComprehensiveFamilyHealthCensusForm(ParsedRecordBean parsedRecordBean, UserMaster user) {
        ...
        if (keyAndAnswerMap.get("5") != null && keyAndAnswerMap.get("5").equals("2")) {
            return returnMap;
        }
    ...
    private void newFamilyMsg(FamilyEntity familyEntity, Map<String, MemberEntity> mapOfMemberWithLoopIdAsKey, Map<String, String> returnMap, Map<Integer, String> failedOfflineAbhaMessageMap, ParsedRecordBean parsedRecordBean) {
        ...
        for (Map.Entry<String, MemberEntity> entry : mapOfMemberWithLoopIdAsKey.entrySet()) {
            ...
            if (Boolean.TRUE.equals(member.getFamilyHeadFlag())) {
                returnMap.put("familyId", familyEntity.getFamilyId() + " - " + member.getFirstName() + " " + member.getLastName());
            }
    ...
    private void setAnswersToFamilyDto(String key, String answer, FamilyEntity familyEntity) {
        switch (key) {
            case "5":
                familyEntity.setIsProvidingConsent(ImtechoUtil.returnTrueFalseFromInitials(answer));
                break;
            case "6":
...
//                break;
            case "12":
                ...
                break;
            case "13":
                familyEntity.setEligibleWomenCount(Integer.valueOf(answer.trim()));
                break;
            case "14":
                familyEntity.setEligibleChildrenCount(Integer.valueOf(answer.trim()));
                break;
            case "15":
                familyEntity.setIsHoldIdPoor(ImtechoUtil.returnTrueFalseFromInitials(answer));
                break;
            case "-3":
            ...
            case "43":
                if(answer != null){
                    if(!answer.equalsIgnoreCase("OTHER")){
                        memberEntity.setEducationStatus(Integer.parseInt(answer));
                    }
                }
                break;
            case "117":
                ...
                break;
            case "453":
                memberEntity.setHaveNssf(ImtechoUtil.returnTrueFalseFromInitials(answer));
                break;
            case "454":
