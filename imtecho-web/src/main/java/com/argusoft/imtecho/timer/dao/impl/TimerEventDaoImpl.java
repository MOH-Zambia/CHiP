 ...
 */
package com.argusoft.imtecho.timer.dao.impl;
...
public class TimerEventDaoImpl extends GenericDaoImpl<TimerEvent, Integer> implements TimerEventDao {
     ...
     */
    @Override
    public List<TimerEvent> retrieveTimerEventsByConfig(UUID eventConfigUUID) {
        Session session = sessionFactory.getCurrentSession();
        CriteriaBuilder criteriaBuilder = session.getCriteriaBuilder();
        CriteriaQuery<TimerEvent> criteria = criteriaBuilder.createQuery(TimerEvent.class);
        Root<TimerEvent> root = criteria.from(TimerEvent.class);
        ...
        Predicate configId = criteriaBuilder.equal(root.get(TimerEvent.TimerEventFields.EVENT_CONFIG_UUID), eventConfigUUID);
