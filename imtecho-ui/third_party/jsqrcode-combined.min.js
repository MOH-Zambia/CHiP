function ECB(count, dataCodewords) {
  ...
  function ECBlocks(ecCodewordsPerBlock, ecBlocks1, ecBlocks2) {
    (this.ecCodewordsPerBlock = ecCodewordsPerBlock),
      ...
      this.__defineGetter__("NumBlocks", function () {
        for (var total = 0, i = 0; i < this.ecBlocks.length; i++)
    ...
    (this.totalCodewords = total),
      ...
      (this.buildFunctionPattern = function () {
        ...
        for (var max = this.alignmentPatternCenters.length, x = 0; max > x; x++)
          for (var i = this.alignmentPatternCenters[x] - 2, y = 0; max > y; y++)
            (0 == x && (0 == y || y == max - 1)) ||
              (x == max - 1 && 0 == y) ||
    ...
    (this.image = image),
      ...
      (this.sizeOfBlackWhiteBlackRun = function (fromX, fromY, toX, toY) {
        ...
        for (
          ...
          if (
            (1 == state
              ? this.image[realX + realY * qrcode.width] && state++
              : this.image[realX + realY * qrcode.width] || state++,
            3 == state)
          ) {
            var diffX = x - fromX,
      ...
      (this.sizeOfBlackWhiteBlackRunBothWays = function (fromX, fromY, toX, toY) {
        ...
        return (
          ...
          0 > otherToY
            ...
            : otherToY >= qrcode.height &&
              ((scale = (qrcode.height - 1 - fromY) / (otherToY - fromY)),
              (otherToY = qrcode.height - 1)),
          (otherToX = Math.floor(fromX + (otherToX - fromX) * scale)),
          (result += this.sizeOfBlackWhiteBlackRun(
            fromX,
            fromY,
            otherToX,
            otherToY
      ...
      (this.calculateModuleSizeOneWay = function (pattern, otherPattern) {
        ...
        return isNaN(moduleSizeEst1)
          ...
          ? moduleSizeEst1 / 7
          : (moduleSizeEst1 + moduleSizeEst2) / 14;
      }),
      ...
      (this.computeDimension = function (
        topLeft,
        topRight,
        bottomLeft,
        moduleSize
        ...
        var tltrCentersDimension = Math.round(
          ...
          tlblCentersDimension = Math.round(
      ...
      (this.findAlignmentInRegion = function (
        ...
        var allowance = Math.floor(allowanceFactor * overallEstModuleSize),
          ...
          alignmentAreaRightX = Math.min(
            qrcode.width - 1,
            estAlignmentX + allowance
          );
        if (3 * overallEstModuleSize > alignmentAreaRightX - alignmentAreaLeftX)
          throw "Error";
        var alignmentAreaTopY = Math.max(0, estAlignmentY - allowance),
          alignmentAreaBottomY = Math.min(
            qrcode.height - 1,
      ...
      (this.processFinderPatternInfo = function (info) {
        ...
        return (
          (points =
            null == alignmentPattern
              ? new Array(bottomLeft, topLeft, topRight)
              : new Array(bottomLeft, topLeft, topRight, alignmentPattern)),
          new DetectorResult(bits, points)
        );
      }),
  ...
  function BitMatrix(width, height) {
    if ((height || (height = width), 1 > width || 1 > height))
    ...
    this.__defineGetter__("Width", function () {
      ...
      (this.setRegion = function (left, top, width, height) {
        ...
        for (var y = top; bottom > y; y++)
          for (var offset = y * this.rowSize, x = left; right > x; x++)
            this.bits[offset + (x >> 5)] |= 1 << (31 & x);
      });
  }
  ...
  function BitMatrixParser(bitMatrix) {
    ...
    (this.bitMatrix = bitMatrix),
      ...
      (this.readFormatInformation = function () {
        ...
        for (var formatInfoBits = 0, i = 0; 6 > i; i++)
        ...
        formatInfoBits = 0;
        for (var iMin = dimension - 8, i = dimension - 1; i >= iMin; i--)
          formatInfoBits = this.copyBit(i, 8, formatInfoBits);
        for (var j = dimension - 7; dimension > j; j++)
          formatInfoBits = this.copyBit(8, j, formatInfoBits);
        if (
          ((this.parsedFormatInfo =
            FormatInformation.decodeFormatInformation(formatInfoBits)),
      ...
      (this.readCodewords = function () {
        ...
        for (
          ...
          for (var count = 0; dimension > count; count++)
            for (
              var i = readingUp ? dimension - 1 - count : count, col = 0;
              2 > col;
              col++
            )
              functionPattern.get_Renamed(j - col, i) ||
                (bitsRead++,
                (currentByte <<= 1),
                this.bitMatrix.get_Renamed(j - col, i) && (currentByte |= 1),
                8 == bitsRead &&
  ...
  function DataMask000() {
    (this.unmaskBitMatrix = function (bits, dimension) {
      for (var i = 0; dimension > i; i++)
        for (var j = 0; dimension > j; j++)
          this.isMasked(i, j) && bits.flip(j, i);
    }),
  ...
  function ReedSolomonDecoder(field) {
    (this.field = field),
        ...
        if (!noError)
          for (
            var syndrome = new GF256Poly(this.field, syndromeCoefficients),
              ...
              errorMagnitudes = this.findErrorMagnitudes(
                omega,
                errorLocations,
                dataMatrix
              ),
            ...
            var position =
              received.length - 1 - this.field.log(errorLocations[i]);
      ...
      (this.runEuclideanAlgorithm = function (a, b, R) {
        ...
        for (
          var rLast = a,
            ...
            t = this.field.One;
          r.Degree >= Math.floor(R / 2);
  
        ) {
          var rLastLast = rLast,
            sLastLast = sLast,
            tLastLast = tLast;
          if (((rLast = r), (sLast = s), (tLast = t), rLast.Zero))
      ...
      (this.findErrorLocations = function (errorLocator) {
        ...
        for (
          var result = new Array(numErrors), e = 0, i = 1;
          256 > i && numErrors > e;
          i++
        )
          0 == errorLocator.evaluateAt(i) &&
      ...
      (this.findErrorMagnitudes = function (
        ...
        for (
          var s = errorLocations.length, result = new Array(s), i = 0;
          s > i;
          i++
        ) {
          for (
          ...
          (result[i] = this.field.multiply(
            ...
            dataMatrix && (result[i] = this.field.multiply(result[i], xiInverse));
        }
        return result;
      });
  }
  function GF256Poly(field, coefficients) {
    if (null == coefficients || 0 == coefficients.length)
    ...
    if (coefficientsLength > 1 && 0 == coefficients[0]) {
      for (
        var firstNonZero = 1;
        coefficientsLength > firstNonZero && 0 == coefficients[firstNonZero];
    ...
    this.__defineGetter__("Zero", function () {
      ...
      (this.evaluateAt = function (a) {
        ...
        if (1 == a) {
          for (var result = 0, i = 0; size > i; i++)
            result = GF256.addOrSubtract(result, this.coefficients[i]);
          return result;
        }
        for (var result2 = this.coefficients[0], i = 1; size > i; i++)
      ...
      (this.multiply1 = function (other) {
        ...
        for (
          ...
          for (var aCoeff = aCoefficients[i], j = 0; bLength > j; j++)
      ...
      (this.multiply2 = function (scalar) {
        ...
        for (
          var size = this.coefficients.length, product = new Array(size), i = 0;
          size > i;
          i++
        )
      ...
      (this.divide = function (other) {
        ...
        for (
          var quotient = this.field.Zero,
            ...
            inverseDenominatorLeadingTerm = this.field.inverse(
              denominatorLeadingTerm
            );
          remainder.Degree >= other.Degree && !remainder.Zero;
  ...
  function GF256(primitive) {
    ...
    for (var x = 1, i = 0; 256 > i; i++)
      (this.expTable[i] = x), (x <<= 1), x >= 256 && (x ^= primitive);
    ...
    (at1[0] = 1),
      ...
      (this.buildMonomial = function (degree, coefficient) {
        ...
        for (
          var coefficients = new Array(degree + 1), i = 0;
      ...
      (this.multiply = function (a, b) {
        return 0 == a || 0 == b
          ...
          : 1 == b
          ? a
          : this.expTable[(this.logTable[a] + this.logTable[b]) % 255];
      });
  }
    ...
    (this.x = posX),
      ...
      (this.aboutEquals = function (moduleSize, i, j) {
        if (
          ...
          return (
            1 >= moduleSizeDiff || moduleSizeDiff / this.estimatedModuleSize <= 1
          );
        }
        return !1;
      });
    ...
    (this.image = null),
      ...
      (this.crossCheckVertical = function (
        ...
        for (
          var image = this.image,
            ...
            i = startI;
          i >= 0 && image[centerJ + i * qrcode.width];
        ...
        for (i = startI + 1; maxI > i && image[centerJ + i * qrcode.width]; )
          stateCount[2]++, i++;
        ...
        var stateCountTotal =
          ...
          stateCount[3] +
          stateCount[4];
        return 5 * Math.abs(stateCountTotal - originalStateCountTotal) >=
      ...
      (this.crossCheckHorizontal = function (
        ...
        for (
          var image = this.image,
            ...
            stateCount = this.CrossCheckStateCount,
            j = startJ;
          j >= 0 && image[j + centerI * qrcode.width];
        ...
        for (
          ...
          stateCount[0]++, j--;
        if (stateCount[0] > maxCount) return NaN;
        for (j = startJ + 1; maxJ > j && image[j + centerI * qrcode.width]; )
          stateCount[2]++, j++;
      ...
      (this.handlePossibleCenter = function (stateCount, i, j) {
        var stateCountTotal =
          ...
          centerI = this.crossCheckVertical(
            i,
            Math.floor(centerJ),
            stateCount[2],
            stateCountTotal
      ...
      (this.selectBestPatterns = function () {
        ...
        if (startSize > 3) {
          for (var totalModuleSize = 0, i = 0; startSize > i; i++)
            totalModuleSize += this.possibleCenters[i].EstimatedModuleSize;
          for (
            var average = totalModuleSize / startSize, i = 0;
            i < this.possibleCenters.length && this.possibleCenters.length > 3;
            i++
          ) {
            var pattern = this.possibleCenters[i];
            Math.abs(pattern.EstimatedModuleSize - average) > 0.2 * average &&
              (this.possibleCenters.remove(i), i--);
          }
        }
        return (
          this.possibleCenters.Count > 3,
          new Array(
      ...
      (this.haveMultiplyConfirmedCenters = function () {
        for (
          ...
          var pattern = this.possibleCenters[i];
          pattern.Count >= CENTER_QUORUM &&
            (confirmedCount++, (totalModuleSize += pattern.EstimatedModuleSize));
        }
        if (3 > confirmedCount) return !1;
        for (
          var average = totalModuleSize / max, totalDeviation = 0, i = 0;
          max > i;
          i++
        )
          (pattern = this.possibleCenters[i]),
            (totalDeviation += Math.abs(pattern.EstimatedModuleSize - average));
        return 0.05 * totalModuleSize >= totalDeviation;
      }),
      (this.findFinderPattern = function (image) {
        ...
        for (
          var done = !1, stateCount = new Array(5), i = iSkip - 1;
          maxI > i && !done;
          i += iSkip
        ) {
          (stateCount[0] = 0),
            ...
            if (image[j + i * qrcode.width])
              ...
              if (4 == currentState)
                if (this.foundPatternCross(stateCount)) {
                  ...
                  if (confirmed)
                    if (((iSkip = 2), this.hasSkipped))
                      done = this.haveMultiplyConfirmedCenters();
                    else {
                      var rowSkip = this.findRowSkip();
                      rowSkip > stateCount[2] &&
                        ((i += rowSkip - stateCount[2] - iSkip), (j = maxJ - 1));
                    }
                  else {
                    do j++;
                    while (maxJ > j && !image[j + i * qrcode.width]);
                    j--;
                  }
                  (currentState = 0),
                    (stateCount[0] = 0),
          ...
          if (this.foundPatternCross(stateCount)) {
            ...
            confirmed &&
              ((iSkip = stateCount[0]),
              this.hasSkipped && (done = haveMultiplyConfirmedCenters()));
          }
        }
  ...
  function AlignmentPatternFinder(
    ...
    (this.image = image),
      ...
      (this.foundPatternCross = function (stateCount) {
        for (
          var moduleSize = this.moduleSize, maxVariance = moduleSize / 2, i = 0;
          3 > i;
          i++
        )
      ...
      (this.find = function () {
        for (
          ...
          var i =
            middleI + (0 == (1 & iGen) ? (iGen + 1) >> 1 : -((iGen + 1) >> 1));
          (stateCount[0] = 0), (stateCount[1] = 0), (stateCount[2] = 0);
  ...
  function QRCodeDataBlockReader(blocks, version, numErrorCorrectionCode) {
    (this.blockPointer = 0),
      ...
      9 >= version
        ...
        : version >= 27 && 40 >= version && (this.dataLengthMode = 2),
      (this.getNextBits = function (numBits) {
        ...
        if (numBits < this.bitPointer + 1 + 8) {
          for (var mask1 = 0, i = 0; i < this.bitPointer + 1; i++)
            mask1 += 1 << i;
          return (
            (bits =
        ...
        if (numBits < this.bitPointer + 1 + 16) {
          for (var mask1 = 0, mask3 = 0, i = 0; i < this.bitPointer + 1; i++)
            mask1 += 1 << i;
          var bitsFirstBlock =
            (this.blocks[this.blockPointer] & mask1) <<
            (numBits - (this.bitPointer + 1));
        ...
        do
          length >= 3
            ? ((intData = this.getNextBits(10)),
              100 > intData && (strData += "0"),
              10 > intData && (strData += "0"),
              (length -= 3))
            : 2 == length
            ? ((intData = this.getNextBits(7)),
              10 > intData && (strData += "0"),
              (length -= 2))
            : 1 == length && ((intData = this.getNextBits(4)), (length -= 1)),
            (strData += intData);
        while (length > 0);
        return strData;
      }),
      (this.get8bitByteArray = function (dataLength) {
        var length = dataLength,
          intData = 0,
          output = new Array();
        do (intData = this.getNextBits(8)), output.push(intData), length--;
        while (length > 0);
        return output;
      }),
      (this.getKanjiString = function (dataLength) {
        ...
        do {
          ...
          var lowerByte = intData % 192,
            ...
            shiftjisWord = 0;
          (shiftjisWord =
            40956 >= tempWord + 33088 ? tempWord + 33088 : tempWord + 49472),
      ...
      this.__defineGetter__("DataByte", function () {
        for (
          ...
          if (((dataLength = this.getDataLength(mode)), dataLength < 1))
  ...
  (GridSampler = {}),
    (GridSampler.checkAndNudgePoints = function (image, points) {
      for (
        var width = qrcode.width, height = qrcode.height, nudged = !0, offset = 0;
        offset < points.Length && nudged;
        offset += 2
      ) {
        ...
        (nudged = !1),
          -1 == x
            ? ((points[offset] = 0), (nudged = !0))
            : x == width && ((points[offset] = width - 1), (nudged = !0)),
          -1 == y
            ? ((points[offset + 1] = 0), (nudged = !0))
            : y == height && ((points[offset + 1] = height - 1), (nudged = !0));
      }
      nudged = !0;
    ...
    (GridSampler.sampleGrid3 = function (image, dimension, transform) {
      for (
        ...
        for (var max = points.length, iValue = y + 0.5, x = 0; max > x; x += 2)
    ...
    (Version.decodeVersionInformation = function (versionBits) {
      for (
        var bestDifference = 4294967295, bestVersion = 0, i = 0;
  ...
  var FORMAT_INFO_MASK_QR = 21522,
    FORMAT_INFO_DECODE_LOOKUP = new Array(
      new Array(21522, 0),
      new Array(20773, 1),
      new Array(24188, 2),
      new Array(23371, 3),
    ...
    BITS_SET_IN_HALF_BYTE = new Array(
      0,
      1,
      1,
      2,
  ...
  (FormatInformation.numBitsDiffering = function (a, b) {
    return (
      (a ^= b),
      BITS_SET_IN_HALF_BYTE[15 & a] +
        BITS_SET_IN_HALF_BYTE[15 & URShift(a, 4)] +
        BITS_SET_IN_HALF_BYTE[15 & URShift(a, 8)] +
        BITS_SET_IN_HALF_BYTE[15 & URShift(a, 12)] +
        BITS_SET_IN_HALF_BYTE[15 & URShift(a, 16)] +
        BITS_SET_IN_HALF_BYTE[15 & URShift(a, 20)] +
        BITS_SET_IN_HALF_BYTE[15 & URShift(a, 24)] +
        BITS_SET_IN_HALF_BYTE[15 & URShift(a, 28)]
    );
  }),
    (FormatInformation.decodeFormatInformation = function (maskedFormatInfo) {
      var formatInfo =
        FormatInformation.doDecodeFormatInformation(maskedFormatInfo);
      return null != formatInfo
        ? formatInfo
    ...
    (FormatInformation.doDecodeFormatInformation = function (maskedFormatInfo) {
      for (
        var bestDifference = 4294967295, bestFormatInfo = 0, i = 0;
  ...
  (DataBlock.getDataBlocks = function (rawCodewords, version, ecLevel) {
    ...
    for (
      var ecBlocks = version.getECBlocksForLevel(ecLevel),
        ...
        i = 0;
      i < ecBlockArray.length;
      i++
    )
      totalBlocks += ecBlockArray[i].Count;
    for (
      var result = new Array(totalBlocks), numResultBlocks = 0, j = 0;
      j < ecBlockArray.length;
      j++
    )
    ...
    for (
      var shorterBlocksNumDataCodewords =
        ...
        i = 0;
      shorterBlocksNumDataCodewords > i;
      i++
    )
      for (var j = 0; numResultBlocks > j; j++)
        result[j].codewords[i] = rawCodewords[rawCodewordsOffset++];
    for (var j = longerBlocksStartAt; numResultBlocks > j; j++)
      result[j].codewords[shorterBlocksNumDataCodewords] =
        rawCodewords[rawCodewordsOffset++];
    for (
      var max = result[0].codewords.length, i = shorterBlocksNumDataCodewords;
      max > i;
      i++
    )
      for (var j = 0; numResultBlocks > j; j++) {
    ...
    (Decoder.correctErrors = function (codewordBytes, numDataCodewords) {
      for (
        ...
        codewordsInts[i] = 255 & codewordBytes[i];
      ...
      for (var i = 0; numDataCodewords > i; i++)
    ...
    (Decoder.decode = function (bits) {
      ...
      for (
        var resultBytes = new Array(totalBytes), resultOffset = 0, j = 0;
        j < dataBlocks.length;
        j++
      ) {
    ...
    (qrcode.decode = function (src) {
      ...
      (image.onload = function () {
        var canvas_qr = document.createElement("canvas"),
          context = canvas_qr.getContext("2d"),
          canvas_out = document.getElementById("out-canvas");
        if (null != canvas_out) {
      ...
      if (qrcode.debug) {
        for (var y = 0; y < qrcode.height; y++)
          for (var x = 0; x < qrcode.width; x++) {
            ...
            (qrcode.imagedata.data[point] = (image[x + y * qrcode.width], 0)),
              (qrcode.imagedata.data[point + 1] =
                (image[x + y * qrcode.width], 0)),
    ...
    (qrcode.binarize = function (th) {
      for (
        var ret = new Array(qrcode.width * qrcode.height), y = 0;
        y < qrcode.height;
        y++
      )
        for (var x = 0; x < qrcode.width; x++) {
        ...
        for (var ax = 0; numSqrtArea > ax; ax++) {
          ...
          for (var dy = 0; areaHeight > dy; dy++)
            for (var dx = 0; areaWidth > dx; dx++) {
              ...
              target < minmax[ax][ay][0] && (minmax[ax][ay][0] = target),
                target > minmax[ax][ay][1] && (minmax[ax][ay][1] = target);
            }
        }
      for (var middle = new Array(numSqrtArea), i3 = 0; numSqrtArea > i3; i3++)
        middle[i3] = new Array(numSqrtArea);
      for (var ay = 0; numSqrtArea > ay; ay++)
        for (var ax = 0; numSqrtArea > ax; ax++)
          middle[ax][ay] = Math.floor(
            (minmax[ax][ay][0] + minmax[ax][ay][1]) / 2
          );
      return middle;
    }),
  ...
  qrcode.orderBestPatterns = function (patterns) {
    ...
    if (
      (oneTwoDistance >= zeroOneDistance && oneTwoDistance >= zeroTwoDistance
        ? ((pointB = patterns[0]), (pointA = patterns[1]), (pointC = patterns[2]))
        : zeroTwoDistance >= oneTwoDistance && zeroTwoDistance >= zeroOneDistance
        ? ((pointB = patterns[1]), (pointA = patterns[0]), (pointC = patterns[2]))
        : ((pointB = patterns[2]),
          (pointA = patterns[0]),
          (pointC = patterns[1])),
      crossProductZ(pointA, pointB, pointC) < 0)
    ) {
      var temp = pointA;
