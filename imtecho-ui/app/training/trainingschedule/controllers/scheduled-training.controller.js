(function () {
    function ScheduledTrainingCtrl(ConstantService, AuthenticateService, $filter,toaster, $uibModal, Mask, $timeout, QueryDAO, $state, CourseService, PagingForQueryBuilderService, GeneralUtil, $sessionStorage) {
        ...
        ctrl.tomorrow = moment(moment(ctrl.todayDate).add(1, 'days').format('MM/DD/YYYY')).format('x');
        ctrl.scheduleFilter = {};
        ...
        ctrl.getTrainingStatuses = function () {
            AuthenticateService.getLoggedInUser().then(function (res) {
                ...
                ctrl.dateFrom = ctrl.dateRange.from_date_range ? moment(ctrl.dateRange.from_date_range).format('MM/DD/YYYY') : '01/01/1970';
                ctrl.dateTo = ctrl.dateRange.to_date_range == '01/01/1970' ? moment(ctrl.todayDate).format('MM/DD/YYYY') : moment(ctrl.dateRange.to_date_range).format('MM/DD/YYYY');
                Mask.show();
                var queryDto = {
                ...
                PagingForQueryBuilderService.getNextPage(QueryDAO.execute, queryDto, ctrl.trainingScheduleList, null).then((response) => {
                    ...
                    Mask.hide();
                    ctrl.trainingScheduleList.forEach((item,index)=>{
                        if(item.effectiveDate>=ctrl.tomorrow){
                            ctrl.trainingScheduleList[index].canDeleteItem = true;
                        }else{
                            ctrl.trainingScheduleList[index].canDeleteItem = false;
                        }
                    })
                });
            });
        ...
        ctrl.rescheduleTraining = function (training) {
            if (ctrl.getStatus(training) == 4 || ctrl.getStatus(training) == 6 || training.courseType === 'ONLINE') {
                var modalInstance = $uibModal.open({
                    ...
                    resolve: {
                        training: function () {
                            return [training.trainingId,training.courseType];
                        }
                    }
                });
        ...
        ctrl.dateFromPast = function (date) {
            return $filter('date')(date, 'yyyy-MM-dd') <= $filter('date')(new Date(), 'yyyy-MM-dd') ;
        };
        ctrl.deleteScheduledTraining = function (trainingId) {
            var queryDto = {
                code: "delete_scheduled_training",
                parameters: {
            ...
            QueryDAO.execute(queryDto).then(function () {
                toaster.pop('success','Training Schedule Deleted Successfully')
                ctrl.initPage();
            }, GeneralUtil.showMessageOnApiCallFailure).finally(function () {
                Mask.hide();
            });
        }
        ctrl.openConfirmationModal = function (trainingId) {
            var modalInstance = $uibModal.open({
                ...
                resolve: {
                    message: function () {
                        return "Are you sure you want to delete this scheduled training?";
                    }
                }
            });
            modalInstance.result.then(function () {
                ctrl.deleteScheduledTraining(trainingId);
            }, function () {
            });
        };
