(function () {
    function CourseListController(CourseService, Mask, $uibModal, toaster, QueryDAO, GeneralUtil, AuthenticateService) {
        var ctrl = this;
        ...
        ctrl.refreshCourseSize = function () {
            Mask.show();

            CourseService.refreshCourseSize().then((res) => {
                ctrl.retrieveAllCourses();
            }).catch((error) => {
                GeneralUtil.showMessageOnApiCallFailure(error);
            }).finally(() => {
        ...
        ctrl.init = function () {
            ...
            AuthenticateService.getAssignedFeature('techo.manage.courselist').then((res) => {
                ctrl.canRefreshCourseSize = res.featureJson.canRefreshCourseSize;    
            });
