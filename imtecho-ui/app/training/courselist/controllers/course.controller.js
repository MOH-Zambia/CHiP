(function () {
    function CourseController(CourseService, toaster, GeneralUtil, $filter, Mask, $uibModal, $timeout, RoleDAO, $stateParams, $state, QueryDAO, $q, AuthenticateService, APP_CONFIG, SohElementConfigurationDAO, $sce, PushNotificationDAO) {
        let ctrl = this;
        ...
        ctrl.showNotificationForm = false;
        ctrl.notificationConfig = [{ day: '', type: '', heading: '', message: '' }]
        ctrl.maxDays = parseInt(50);

        ctrl.checkIfNotificationTypeExists = () => {
            let latestNotification = ctrl.notificationConfig[ctrl.notificationConfig.length - 1]
            let latestNotificationType = latestNotification.type
            if(!latestNotificationType) return;
            PushNotificationDAO.checkIfNotificationTypeExists(latestNotificationType).then(function(res) {
                if(res.result == 'true') {
                    ctrl.notificationTypeExists = ctrl.notificationConfig.length - 1;
                }
            })
        }
        ...
        ctrl.getCoursebyId = (id) => {
            ...
            CourseService.getCourseById(id).then((response) => {
                ...
                if (ctrl.courseObj.courseType === 'OFFLINE') {
                    ...
                    ctrl.resHasNotificationConfig = response.hasNotificationConfiguration;
                    if (response.hasNotificationConfiguration) {
                        ctrl.notificationConfig = response.pushNotificationDtos;
                        ctrl.alreadyConfiguredNotificationCount = response.pushNotificationDtos.length;
                    }
                }
        ...
        ctrl.createCourse = () => {
            ...
            let isFormValidNotification = true;
            if ((ctrl.courseObj.courseType === 'ONLINE' && ctrl.courseObj.hasNotificationConfiguration)) {
                ctrl.notificationForm.$setSubmitted();
                isFormValidNotification = ctrl.notificationForm.$valid
            }
            if (ctrl.courseForm.$valid && _checkTestConfigQuestionValidation() && ctrl.validCourseName && isFormValidNotification) {
                if (Array.isArray(ctrl.topics) && ctrl.topics.length) {
                    ...
                    if (ctrl.courseObj.courseType === 'ONLINE') {
                        ...
                        course.pushNotificationDtos = ctrl.notificationConfig;
                    }
        ...
        ctrl.addTopic = () => {
            ctrl.topicDay++;
            if (ctrl.topicDay != 1 || ctrl.isUpdateForm) {
                if (ctrl.count) {
                    ctrl.topicDay = Object.keys(ctrl.count).length + 1;
                } else {
                    ctrl.topicDay = 1
                }
            }
            let modalInstance = $uibModal.open({
                ...
                resolve: {
                    topics: () => {
                        return [ctrl.topics, true, ctrl.topicDay, false];
                    }
                }
            });
        ...
        ctrl.deleteTopic = (topic) => {
            let modalInstance = $uibModal.open({
                ...
                resolve: {
                    message: () => {
                        if (ctrl.courseObj.courseType === 'OFFLINE') {
                            return `Are you sure you want to delete Day - ${topic.topicDay}?`;
                        } else {
            ...
            modalInstance.result.then(() => {
                ...
                if (ctrl.courseObj.courseType === 'OFFLINE') {
                    const indexes = [];
                    for (t of ctrl.topics) {
                        if (topic.topicDay === t.topicDay) {
                            const index = ctrl.topics.indexOf(t);
                            indexes.push(index);

                        }
                    }
                    ctrl.topics.splice(indexes[0], indexes.length);
                    ctrl.topics.forEach(function (t) {
                        if (t.topicDay > topic.topicDay) {
                            t.topicDay--;
                        }
                    })
                } else {
                ...
                if (ctrl.courseObj.courseType === 'OFFLINE') {
                    ctrl.count = _.groupBy(ctrl.topics, 'topicDay');
                    toaster.pop('success', 'Day deleted successfully');
                } else {
        ...
        ctrl.editDay = (topic) => {
            let modalInstance = $uibModal.open({
                ...
                resolve: {
                    topics: () => {
                        return [ctrl.topics, true, topic.topicDay, true];
                    }
                }
            });
            modalInstance.result.then((topicsList) => {
        ...
        ctrl.updateCourse = () => {
            ctrl.courseForm.$setSubmitted();
            ctrl.notificationForm.$setSubmitted();
            if (ctrl.courseForm.$valid && _checkTestConfigQuestionValidation() && ctrl.validCourseName && ctrl.notificationForm.$valid) {
                if (Array.isArray(ctrl.topics) && ctrl.topics.length) {
                    if (ctrl.courseObj.courseType === 'ONLINE') {
                        ctrl.topics = ctrl.transformMediaDataListToTopicMediaList(ctrl.topics);
                        ctrl.courseObj.testConfigJson = JSON.stringify(ctrl.testConfig);
        ...
        ctrl.courseTypeChanged = () => {
            if (ctrl.courseObj.courseType === 'ONLINE') {
                ctrl.showNotificationForm = true
            }
            ...
            topicsList.forEach(topic => {
                if (Array.isArray(topic.mediaDataList) && topic.mediaDataList.length) {
                    ...
                    topic.mediaDataList.forEach(media => {
                        ...
                        obj.isCompulsory = media.isCompulsory;
        ...
        ctrl.addNotification = function () {
            ctrl.notificationForm.$setSubmitted();
            if (ctrl.notificationForm.$valid) {
                ctrl.notificationConfig.push({ day: '', heading: '', type: '', message: '' });
            }
        }
