(function () {
    ...
    function LayoutController($filter, $state, AuthenticateService, $uibModal, $interval, Mask, UserDAO, GeneralUtil) {
        ...
        function init() {
            ...
            layout.env = GeneralUtil.getEnv();
            layout.isReportAllowed = allowedEnv.includes(layout.env)
            layout.userDetail;
            layout.userObj = {};
            layout.allMenu = [];
        ...
        var parentHover = function () {
            ...
            $('.cst-sidebar-nav li.has-sub').mouseover(function () {
                ...
                if ($(submenu).hasClass('child')) {
                    ...
                    else if (itemHeight > winHeight - curPos) {
                        posTop = 0;
                        maxHeight = itemHeight > $(this).parent().height() ? maxHeight = $(this).parent().height() : itemHeight;
                    }
                } else {
                    if (winHeight - 100 <= itemHeight) {
                        posTop = 60;
