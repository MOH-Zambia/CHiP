    ...
    angular.module('imtecho.directives').directive('inputcomponent', function ($sce, $templateCache, $timeout, $rootScope) {
        return {
            ...
            link: function ($scope, element, attrs) {
                ...
                $scope.onDateRangeTypeChange = function (dateRangeType) {
                    $scope.data.dateRangeType = dateRangeType;
                };
                ...
                if (!!$scope.config.setDateRange) {
                    $scope.fixedMinDate = new Date(new Date().setDate(new Date().getDate() - $scope.config.setDateRange));
                    $scope.fixedMaxDate = new Date();
                }
                if (!!$scope.config.setCustomMinDate || $scope.config.setCustomMinDate === 0) {
                    $scope.fixedMinDate = new Date(new Date().setDate(new Date().getDate() - $scope.config.setCustomMinDate));
                }

                if (!!$scope.config.setCustomMaxDate || $scope.config.setCustomMaxDate === 0) {
                    $scope.fixedMaxDate = new Date(new Date().setDate(new Date().getDate() + $scope.config.setCustomMaxDate));
                }
                ...
                if ($scope.config?.config?.dateRangeOptions) {
                    $scope.dateRangeOptions = $scope.config.config.dateRangeOptions;
                }
                ...
                if($scope.config.isSetDefault && $scope.config.isSetDefault==true){
                    switch($scope.config.fieldType){
                        ...
                        case "onlyMonthFromTo" :
                            switch($scope.config.defaultValueOption){
                                case "currentmonth&lastmonth" : 
                                    $scope.data[$scope.onlyMonthFrom] = new Date(new Date().setMonth(new Date().getMonth() - 1)).setDate(1);
                                    $scope.data[$scope.onlyMonthTo] = new Date(new Date().setMonth(new Date().getMonth())).setDate(1);
                                    break;
                                case "current&lastnmonth" : 
                                    $scope.data[$scope.onlyMonthFrom] = new Date(new Date().setMonth(new Date().getMonth() - $scope.config.setLastNdays)).setDate(1);
                                    $scope.data[$scope.onlyMonthTo] = new Date(new Date().setMonth(new Date().getMonth())).setDate(1);
                                    break;
                                default : 
                                    break;    
                        ...
                        case "onlyMonth" :
                            switch($scope.config.defaultValueOption){
                                case "currentmonth" : 
                                    $scope.data[$scope.config.fieldName] = new Date(new Date().setMonth(new Date().getMonth())).setDate(1);
                                    break;
                                case "lastmonth" : 
                                    $scope.data[$scope.config.fieldName] = new Date(new Date().setMonth(new Date().getMonth() - 1)).setDate(1);
                                    break;
                                default : 
                                    break;    
                            }
