// (function () {
//     function DailyActivityRegister(QueryDAO, Mask, GeneralUtil, $state,FormAssociationsIDAR) {
//         let ctrl = this;

//         ctrl.init = function () {
//             ctrl.formObj = {};
//             ctrl.getUserData();
//         }

//         ctrl.getUserData = function () {
//             let dto = {
//                 code: 'retrieve_idar_user_details',
//                 parameters: {
//                     userId: $state.params.id
//                 }
//             }
//             Mask.show();
//             QueryDAO.execute(dto).then(function(res) {
//                 ctrl.formObj.chw = res.result[0].chw || 'N/A';
//                 ctrl.formObj.zone = res.result[0].zone || 'N/A';
//                 ctrl.formObj.chw_type = res.result[0].chw_type || 'N/A';
//                 ctrl.formObj.area = res.result[0].area || 'N/A' ;
//             }, GeneralUtil.showMessageOnApiCallFailure).finally(function () {
//                 Mask.hide();
//             });
//         }

//         ctrl.getFormData = function () {
//             let dtoList = [
//                 {
//                     code: 'retrieve_idar_client_details',
//                     parameters: {
//                         userId: $state.params.id,
//                         month:ctrl.formObj.month
//                     },
//                     sequence: 0
//                 },
//                 {
//                     code: 'retrieve_idar_member_details',
//                     parameters: {
//                         userId: $state.params.id,
//                         month:ctrl.formObj.month
//                     },
//                     sequence: 1
//                 },
//                 {
//                     code: 'retrieve_idar_anc_details',
//                     parameters: {
//                         userId: $state.params.id,
//                         month:ctrl.formObj.month
//                     },
//                     sequence: 2
//                 },
//                 {
//                     code: 'retrieve_idar_wpd_details',
//                     parameters: {
//                         userId: $state.params.id,
//                         month:ctrl.formObj.month
//                     },
//                     sequence: 3
//                 },
//                 {
//                     code: 'retrieve_idar_pnc_details',
//                     parameters: {
//                         userId: $state.params.id,
//                         month:ctrl.formObj.month
//                     },
//                     sequence: 4
//                 },
//                 {
//                     code: 'retrieve_idar_csv_details',
//                     parameters: {
//                         userId: $state.params.id,
//                         month:ctrl.formObj.month
//                     },
//                     sequence: 5
//                 },
//                 {
//                     code: 'retrieve_idar_tb_details',
//                     parameters: {
//                         userId: $state.params.id,
//                         month:ctrl.formObj.month
//                     },
//                     sequence: 6
//                 },
//                 {
//                     code: 'retrieve_idar_malaria_details',
//                     parameters: {
//                         userId: $state.params.id,
//                         month:ctrl.formObj.month
//                     },
//                     sequence: 7
//                 },
//                 {
//                     code: 'retrieve_idar_hiv_details',
//                     parameters: {
//                         userId: $state.params.id,
//                         month:ctrl.formObj.month
//                     },
//                     sequence: 8
//                 },
//                 {
//                     code: 'retrieve_idar_pregnancy_details',
//                     parameters: {
//                         userId: $state.params.id,
//                         month:ctrl.formObj.month
//                     },
//                     sequence: 9
//                 }
//             ];
//             Mask.show();
//             QueryDAO.executeAll(dtoList).then(function(res) {
//                 for (let val = 0; val < dtoList.length; val++){
//                     for (let key in res[val].result[0]) {
//                         ctrl.formObj[key] = res[val].result[0][key];
//                     }
//                 }
               
//             }, GeneralUtil.showMessageOnApiCallFailure).finally(function () {
//                 ctrl.markNA();
//                 Mask.hide();
//             });
//         }


//         ctrl.printPdf = function () {
//             $(document).ready(function(){
//                 $('#area').css({
//                     'width' : '45%'
//                 })

//                 $('#idar-form').printThis({
//                     importCSS: true,
//                     formValue : true,
//                     header: '',
//                     base: "./",
//                     pageTitle: 'Integrated Daily Activity Register',
//                     afterPrint : function(){
//                         $('#area#').css({
//                             'width' : ''
//                         })
//                     }
//                 });
//             });
//         };

//         ctrl.printExcel = function(){
//             let data = ctrl.formObj;
//             let finalSet = {};
//             for (let key in FormAssociationsIDAR) {
//                 if (FormAssociationsIDAR.hasOwnProperty(key)) {
//                   var newKey = FormAssociationsIDAR[key];
//                   var value = data[key];
//                   if(value != undefined)
//                   finalSet[newKey] = value;
//                 }
//               }

//             let wb = XLSX.utils.book_new();
//             let ws = XLSX.utils.json_to_sheet([]);

//             let rowIndex = 0;
//             for (let key in finalSet) {
//                 if (finalSet.hasOwnProperty(key)) {
//                     let rowData = {};
//                     rowData['Field'] = key; 
//                     rowData['Value'] = finalSet[key];
//                     XLSX.utils.sheet_add_json(ws, [rowData], { header: ['Field', 'Value'], skipHeader: true, origin: `A${++rowIndex}` });
//                 }
//             }
//             XLSX.utils.book_append_sheet(wb, ws, 'Sheet1');
//             let date = new Date().toJSON();
//             XLSX.writeFile(wb, `Integrated Daily Activity Register ${date}.xlsx`);
            
//         }

//         // Remove this function after all remanining datapoints are made available
//         ctrl.markNA = function () {
//             ctrl.formObj.area = 'N/A';
//             ctrl.formObj.counter = 'N/A';
//             ctrl.formObj.dov = 'N/A';
//             ctrl.formObj.hin = 'N/A';
//             ctrl.formObj.noc = 'N/A';
//             ctrl.formObj.chw_type = 'CBV';
//             ctrl.formObj.chw1_02 = 'N/A';

//             ctrl.formObj.chw2_07 = 'N/A';
//             ctrl.formObj.chw2_08 = 'N/A';
//             ctrl.formObj.chw2_09 = 'N/A';
//             ctrl.formObj.chw2_12 = 'N/A';

//             ctrl.formObj.chw3_01 = 'N/A';
//             ctrl.formObj.chw3_09 = 'N/A';
//             ctrl.formObj.chw3_11 = 'N/A';
//             ctrl.formObj.chw3_12 = 'N/A';
//             ctrl.formObj.chw3_13 = 'N/A';

//             ctrl.formObj.chw4_04 = 'N/A';

//             ctrl.formObj.chw5_07 = 'N/A';

//             ctrl.formObj.chw7_01 = 'N/A';
//             ctrl.formObj.chw7_02 = 'N/A';
//             ctrl.formObj.chw7_03 = 'N/A';
//             ctrl.formObj.chw7_04 = 'N/A';
//             ctrl.formObj.chw7_05 = 'N/A';

//             ctrl.formObj.chw8_01 = 'N/A';
//             ctrl.formObj.chw8_02 = 'N/A';
//             ctrl.formObj.chw8_03 = 'N/A';
//             ctrl.formObj.chw8_04 = 'N/A';
//         }
        
//         ctrl.init();
//     }
//     angular.module('imtecho.controllers').controller('DailyActivityRegister', DailyActivityRegister);
// })();