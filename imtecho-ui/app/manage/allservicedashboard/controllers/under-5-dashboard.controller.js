(function () {
  function Under5DashboardController(
    $scope,
    QueryDAO,
    GeneralUtil,
    Mask,
    ...
    $scope.$watch("ctrl.selectedLocationId", function (newVal, oldVal) {
      if (newVal && oldVal) {
        ...
        Mask.show();
        QueryDAO.executeQuery(healthInfraDto)
          .then(function (res) {
    ...
    ctrl.chartCount = () => {
      let pieQueryDto = {
        code: "fetch_under_five_pie_chart_data",
        parameters: {
          location_id: ctrl.selectedLocationId,
        },
      };
      Mask.show();
      QueryDAO.executeQuery(pieQueryDto)
        .then(function (res) {
      ...
      let barQueryDto = {
        code: "fetch_under_five_bar_chart_data",
        parameters: {
          location_id: ctrl.selectedLocationId,
        },
      };

      QueryDAO.executeQuery(barQueryDto)
        .then(function (res) {
    ...
    ctrl.setBarChart = (barResult) => {
      ...
      ctrl.barChartSeries = ["Under 5 children healthwise count"];
      ctrl.barChartColors = [
        {
    ...
    ctrl.tableCount = () => {
      let tableQueryDto = {
        code: "fetch_under_five_table_data",
        parameters: {
          location_id: ctrl.selectedLocationId,
        },
      };
      Mask.show();
      QueryDAO.executeQuery(tableQueryDto)
        .then(function (res) {
          ctrl.underFiveTableData = res.result;
        }, GeneralUtil.showMessageOnApiCallFailure)
        .finally(function () {
          Mask.hide();
        });
  ...
  angular
    .module("imtecho.controllers")
    .controller("Under5DashboardController", Under5DashboardController);
})();
