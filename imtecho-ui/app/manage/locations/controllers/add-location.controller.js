(function () {
    function AddLocationController(Mask, toaster, LocationService, GeneralUtil, QueryDAO, $uibModal,AuthenticateService,$q,) {
        var ctrl = this;
        ...
        ctrl.init = () => {
            ...
            $q.all(promises).then((response) => {
                ...
                ctrl.locationTypes = ctrl.locationTypes.filter(location => {
                    return response.result.some(r => {
                        return r.location_type === location.type
                    });
                })
            }).catch((error) => {
            ...
            if (form.$valid) {
                if (ctrl.selectedLocationId && (ctrl.selectedLocation.finalSelected.level >= ctrl.requiredLevel.level - 1)) {
                    ...
                    QueryDAO.execute(queryDto).then(function (resForData) {
                        // if (resForData.result[0] && resForData.result[0].is_present && ctrl.env!=='chip') {
