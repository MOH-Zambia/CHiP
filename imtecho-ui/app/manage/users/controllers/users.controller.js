            ...
            UserDAO.retrieveByCriteria(ctrl.criteria).then(function (res) {
                ...
                if (excelData.length !== 0) {
                    for (var i = 0; i < excelData.length; i++) {
                        ...
                        let areaOfIntervention = {}
                        let distinctAreaOfIntervention = excelData[i].areaOfIntervention.split(/\r?\n/);
                        distinctAreaOfIntervention.forEach(function(item,index){
                            areaOfIntervention["Area Of Intervention - "+(index+1)] = item
                        })
                        var excelObj = {
                    ...
                    ctrl.items.forEach(function(item,index){
                        if(Object.keys(item).length > maxKeyNumber){
                            maxKeyNumber = Object.keys(item).length;
                            maxKeyIndex = index;
                        }
                    })
                    let maxKeyElement = ctrl.items.splice(maxKeyIndex,1);
                    ctrl.items.unshift(maxKeyElement[0]);
                    var mystyle = {
                        headers: true,
