(function () {
    function ManageUserController(UserDAO, RoleDAO, $stateParams, USER, GeneralUtil, $state, toaster, Mask, AuthenticateService, LocationService, $uibModal, $q, QueryDAO) {
        ...
        usercontroller.prefferedLanguage = 'en';
        var initPage = function () {
            GeneralUtil.getAllActiveLanguages().then(languageList => {
                usercontroller.languageList = languageList;
                ...
                usercontroller.userObj.prefferedLanguage = usercontroller.prefferedLanguage;
                ...
                usercontroller.userObj.isUserSmagTrained = 'false';
                if ($stateParams.id) {
            ...
            usercontroller.checkUsername().then(function () {
                if (usercontroller.userForm.$valid && usercontroller.selectedLocationsId.length > 0
                    ...
                    && ((usercontroller.userObj.contactNumber !== null && 
                        (usercontroller.env === 'imomcare'? (usercontroller.userObj.contactNumber.length === 9 || 
                            usercontroller.userObj.contactNumber.length === 10) 
                            :(usercontroller.userObj.contactNumber.length === 10)
                        ))
                        || usercontroller.userObj.contactNumber === null
                    ...
                    if (usercontroller.userObj.prefferedLanguage === '' || usercontroller.userObj.prefferedLanguage === null) {
                        userDto.prefferedLanguage = usercontroller.prefferedLanguage;
                    } else {
        ...
        usercontroller.getUser = function (id) {
            ...
            UserDAO.retrieveById(id).then(function (res) {
                ...
                if (res.prefferedLanguage) {
                    res.prefferedLanguage = GeneralUtil.getNewPreferredLanguage(res.prefferedLanguage);
                    usercontroller.userObj.prefferedLanguage = res.prefferedLanguage.toLowerCase();
                    if (usercontroller.languageList && usercontroller.languageList.length > 0) {
                        const isPreferredLanguageInList = usercontroller.languageList.some(language =>
                            language.languageKey.toLowerCase() === usercontroller.userObj.prefferedLanguage
                        );
        ...
        usercontroller.checkPhoneNumberOnChange = function (phoneNo,id) {
            ...
            usercontroller.phoneNumberFlag = false;
            usercontroller.checkPhoneNolength = false;
            usercontroller.minPhoneNoLength = usercontroller.env === 'imomcare' ? 9 : 10;

            if ((phoneNo !== undefined && phoneNo.length < usercontroller.minPhoneNoLength) && phoneNo) {
                ...
                usercontroller.checkPhoneNo = false;
                usercontroller.checkPhoneNolength = false;
            } else if (usercontroller.env === 'imomcare' && phoneNo !== undefined && (phoneNo.length == 10 || phoneNo.length == 9)) {
                usercontroller.checkPhoneNo = false;
                usercontroller.checkPhoneNolength = false;
            }
            else if (phoneNo !== undefined && phoneNo.length > 10) {
            ...
            if(usercontroller.env === 'imomcare' && phoneNo.length === 9){
                usercontroller.checkphoneNumber(phoneNo,id);
            }          
        };
                ...
                if (usercontroller.selectedLocation.finalSelected !== null) {
                    ...
                    if (usercontroller.selectedLocation.finalSelected.optionSelected) {
                        if (Array.isArray(usercontroller.selectedLocation.finalSelected.optionSelected) && usercontroller.selectedLocation.finalSelected.optionSelected.length > 0) {
                            for (let option of usercontroller.selectedLocation.finalSelected.optionSelected) {
                                selectedobjs.push({
                    ...
                    for (const element of usercontroller.selectedLocationsId) {
                        if (!angular.isUndefined(selectedobj) && selectedobj !== null && element.locationId === selectedobj.locationId) {
                            ...
                            for (let obj of selectedobjs) {
                                if (element.locationId === obj?.locationId) {
                                    usercontroller.duplicateEntry = true;
                                    usercontroller.isLocationButtonDisabled = false;
                                }
                            }
                    ...
                    if (!usercontroller.duplicateEntry && !angular.isUndefined(selectedobj)) {
                        ...
                        (async function () {
                            for (let obj of selectedobjs) {
                                if ((!usercontroller.allowedLocations || usercontroller.allowedLocations.length === 0) || (usercontroller.allowedLocations.length > 0 && usercontroller.allowedLocations.indexOf(obj.type) >= 0)) {
                                    let selectedLocationIds = _.pluck(usercontroller.selectedLocationsId, "locationId");
                                    usercontroller.isLocationButtonDisabled = true;
                                    try {
                                        res = await UserDAO.validateaoi(usercontroller.userObj.roleId, selectedLocationIds, obj.locationId, usercontroller.userObj.id);
                                        if (res.errorcode === 1) {
                                            usercontroller.errorCode = res.errorcode;
                                            if (res.data) {
                                                usercontroller.addedUsers = _.pluck(res.data, "userName").join();
                                                ...
                                                modalInstance.result.then(function () {
                                                    obj.locationFullName = res.message;
                                                    usercontroller.selectedLocationsId.push(obj);
                                                    let locations = _.pluck(usercontroller.selectedLocationsId, "locationId")
                                                    usercontroller.retrieveHealthInfrastructureByRole(usercontroller.userObj.roleId, locations)
                                                }, function () {
                                                });
        ...
        usercontroller.retrieveLocationByRoleId = function () {
            ...
            if (usercontroller.userObj.roleId) {
                ...
                return LocationService.retrieveLocationByRoleId(usercontroller.userObj.roleId).then(function (res) {
                    ...
                    usercontroller.isUserSmagTrained = res.roles.isSmagTrainedMandatory;
        ...
        usercontroller.infrastructureIdsChanged = () => {
            ...
            if (usercontroller.env === 'uttarakhand' || usercontroller.env === 'chip') {
        ...
        usercontroller.unlockUser = function (userId) {
            var modalInstance = $uibModal.open({
                templateUrl: 'app/common/views/confirmation.modal.html',
                controller: 'ConfirmModalController',
                windowClass: 'cst-modal',
            ...
            modalInstance.result.then(function () {
                ...
                Mask.show();
                QueryDAO.execute({
                    code: 'unlock_user_option',
                    parameters: parameters
                }).then(function (res) {
                    if(res.result.length>0){
                        toaster.pop('success','User unlocked successfully')
                    }else{
