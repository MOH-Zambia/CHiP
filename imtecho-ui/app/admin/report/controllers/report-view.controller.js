(function () {
    function ReportViewController($state, Mask, GeneralUtil, ReportDAO, ReportFieldUtil, $rootScope, APP_CONFIG, toaster,
                        ...
                        if (!filterField.isHidden) {
                            ...
                            if (rv.data[filterField.fieldName] && filterField.fieldType == 'comboForReport') {
                                ...
                                if ((rv.data[filterField.fieldName].startDate || rv.data[filterField.fieldName].endDate)) {
                                    ...
                                    filterField.searchField["date_range_type"] = rv.data.dateRangeType;
                                    let object = _.find(rv.reportObj.configJson.filterDisplay, function (filter) {
                                        return "from_" + filterField.fieldName == filter.fieldName;
                                    });
                                    if (object) {
                                        object.value = moment(new Date(moment(rv.data[filterField.fieldName].startDate).format("YYYY-MM-DD"))).format("DD/MM/YYYY");
                                    } else {
                                        rv.reportObj.configJson.filterDisplay.push({
                                            fieldName: "from_" + filterField.fieldName,
                                            displayName: "From " + filterField.displayName,
                                            value: moment(new Date(moment(rv.data[filterField.fieldName].startDate).format("YYYY-MM-DD"))).format("DD/MM/YYYY")
                                        });
                                    }
                                    object = _.find(rv.reportObj.configJson.filterDisplay, function (filter) {
                                        return "to_" + filterField.fieldName == filter.fieldName;
                                    ...
                                    if (object) {
                                        object.value = moment(new Date(moment(rv.data[filterField.fieldName].endDate).format("YYYY-MM-DD"))).format("DD/MM/YYYY");
                                    } else {
                                        rv.reportObj.configJson.filterDisplay.push({
                                            fieldName: "to_" + filterField.fieldName,
                                            displayName: "To " + filterField.displayName,
                                            value: moment(new Date(moment(rv.data[filterField.fieldName].endDate).format("YYYY-MM-DD"))).format("DD/MM/YYYY")
                                        })
                                    }
                                    if (rv.data.dateRangeType) {
                                        rv.reportObj.configJson.filterDisplay.push({
                                            fieldName: "date_range_type",
                                            displayName: "Date Range Type",
                                            value: rv.data.dateRangeType
                                        })
                                    }
                                }
                            } else {
                                ...
                                else if (filterField.fieldType === 'singleCheckbox' && (filterField.value === true || filterField.value === false)){
                                    filterField.searchField[filterField.fieldName] = filterField.value === true? 'Yes':'No';
                                } 
                                else {
        ...
        rv.navigateToEditReport = function (reportUUID) {
            let url = $state.href('techo.report.config', { "id": reportUUID });
        ...
        var init = function () {
            ...
            rv.configList = [];
            rv.pageval = {};
            rv.shouldShowSummaryPrompt = false
            rv.reportUUID = $state.params.id;
            rv.isdnhdd = ENV.implementation === "dnhdd";
            rv.selectedModel = rv.models[0].value;
            var configState = 'techo.report.config/{"id":"' + rv.reportUUID + '"}';
            ...
            AuthenticateService.getLoggedInUser().then(function (user) {
                rv.user = user.data;
                rv.userRoleId = user.data.roleId;
                rv.userRoleCode = user.data.roleCode;   
                rv.showEditButton = (rv.userRoleCode === 'argusadmin' || rv.userRoleCode === 'superadmin');
                ...
                if ($state.params.id && $state.params.id !== '') {
                    ...
                    ReportDAO.getDynamicDetail($state.params.id, $state.params.type).then(function (res) {
                        ...
                        if (rv.reportObj.lastUpdatedOn != null) {
                            rv.lastUpdatedOn = moment.unix(rv.reportObj.lastUpdatedOn/1000).format("DD/MM/YYYY (hh:mm)");
                        }
                    }, GeneralUtil.showMessageOnApiCallFailure).finally(function () {
                        Mask.hide();
                    });
        ...
        rv.generateSummary = function(event){
            question = rv.reportObj.configJson.containers.tableContainer[0].summaryRequest
            const payLoad = {...rv.filterObject , question,model:rv.selectedModel}
            Mask.show()
            ReportDAO.generateSummary(rv.reportObj.configJson.containers.tableContainer[0].queryUUID,payLoad).then(data=>{
               rv.summary = data
            //    console.log(rv.summary)
            }).catch(err=>{
        ...
        // rv.togglePrompt=()=>{
        //     rv.shouldShowSummaryPrompt = !rv.shouldShowSummaryPrompt
        // }
        ...
        rv.saveExcel = function () {
            ...
            var reportExcelDto = {
                ...
                paramObj: rv.paramMap,
                reportName: $rootScope.currentState.title,
                dataColspan: rv.reportObj.configJson.containers.tableContainer[0].dataColspan
            }
            Mask.show();
        ...
        var retrieveTableDataByQueryId = function (retrieveAll) {
            ...
            rv.reportObj.configJson.containers.fieldsContainer.forEach(function (field) {
                ...
                if (field.fieldType === 'dateRangePicker') {
                    ...
                    paramMap["from_" + field.fieldName] = moment(new Date(moment(rv.data[field.fieldName].startDate).format("YYYY-MM-DD"))).format("MM/DD/YYYY");
                    paramMap["to_" + field.fieldName] = moment(new Date(moment(rv.data[field.fieldName].endDate).format("YYYY-MM-DD"))).format("MM/DD/YYYY");
                    paramMap["date_range_type"] = rv.data.dateRangeType;
                }
                if (field.fieldType === 'location' && !!field.demographicFilterRequired && field.demographicFilterRequired) {
                    paramMap['demographic_' + field.fieldName] = field.selectedLocation.selectedDemographic;
                }
            ...
            if (rv.reportObj.configJson.layout.indexOf("WithPagination") >= 0 && !retrieveAll) {
                if (!rv.pagingService.pagingRetrivalOn && !rv.pagingService.allRetrieved) {
                    ...
                    return ReportDAO.retrieveDataByQueryUUID(rv.reportObj.configJson.containers.tableContainer[0].queryUUID, paramMap).then(function (resForData) {
                        ...
                        if (rv.reportObj.configJson.containers.offset == 0) {
                            rv.toggleFilter();
                        }
                        if(resForData.length !== 0){
                            rv.shouldShowSummaryPrompt = true
                            rv.summary = {}
                            }
    
                    }, GeneralUtil.showMessageOnApiCallFailure).finally(function () {
                        if(rv.hasOwnProperty('lastUpdatedOn')){
                            ReportDAO.getDynamicDetail($state.params.id, $state.params.type, false).then(function (res) {
                                rv.lastUpdatedOn = moment.unix(res.lastUpdatedOn/1000).format("DD/MM/YYYY (hh:mm)");
                            });
                        }
                        rv.pagingService.pagingRetrivalOn = false;
                        rv.pageLoaded = true;
