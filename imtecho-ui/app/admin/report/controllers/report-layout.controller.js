(function () {
    function ReportLayoutController($state, Mask, GeneralUtil, toaster, ReportDAO, REPORTS, LocationService, LAYOUTCONFIG, $uibModal, APP_CONFIG, GroupDAO, AuthenticateService, $q, PagingService,ENV) {
        var rl = this;
        rl.pagingService = PagingService.initialize();
        ...
        var init = function () {
            ...
            rl.availableImplicitParameters = ["#loggedInUserId#"];
            rl.retrieveAllReports();
            rl.dateRangeOptions = [];
            rl.locationTypes = [];
            // rl.isdnhdd = ENV.implementation === "dnhdd";
            rl.getLocationTypes = function () {
                Mask.show();
                LocationService.retrieveLocationTypes().then((res) => {
                    rl.locationTypes = res;
                }).catch((error) => {
            ...
            rl.getLocationTypes();
            rl.getPopoverContent = function () {
                let content = '';
                rl.locationTypes.sort((a, b) => {
                    return a.level - b.level;
                });
                let locations = rl.locationTypes;
                content += locations[0].name + '- ' + locations[0].level;
                for (let j = 1; j < locations.length; j++) {
                    if (locations[j].level == locations[j - 1].level) {
                        content += ' | ' + locations[j].name + '- ' + locations[j].level;
                    } else {
                        content += '<br>' + locations[j].name + '- ' + locations[j].level;
                    }
                }
                return content;
            };
            ...
            if ($state.params.id && $state.params.id !== '') {
                rl.retrieveGroups();
                let promises = [];
                promises.push(ReportDAO.getReportDetails({ uuid: $state.params.id }));
                        ...
                        if (!field.isInvalid && field.fieldName) {
                            if (field.fieldType === 'onlyMonthFromTo' || field.fieldType === 'dateFromTo' || field.fieldType === 'dateRangePicker') {
                                ...
                                if(field.fieldType === 'dateRangePicker'){
                                    rl.availableImplicitParameters.push('#date_range_type#');
                                }
                            } else {
        ...
        AuthenticateService.getLoggedInUser().then(({config,data})=>{
            rl.roleCode = data.roleCode
            console.log(rl.roleCode)
         }).catch(error=>{
            toaster.pop('error',"Error While Fetching The CurrentRole")
         })
        ...
        rl.retrieveAllReports = function () {
            ...
            ReportDAO.getReports({
                 offset: rl.pagingService.offSet,
                 limit: rl.pagingService.limit,
            }).then(function (res) {
