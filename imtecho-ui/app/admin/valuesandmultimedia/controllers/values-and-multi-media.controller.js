    ...
    function ValueNMultimediaController($uibModal, Mask, APP_CONFIG, QueryDAO, GeneralUtil, $rootScope, toaster, AuthenticateService, $filter) {
        ...
        vnm.openAddEditModal = function (value) {
            ...
            if (vnm.addForm.$valid) {
                if (value) {
                    if(value.constant){
                        vnm.canChangeConstant = false;
                    } else {
                        vnm.canChangeConstant = true;
                    }
                    vnm.editMode = true;
        ...
        vnm.addValue = function () {
            ...
            if (vnm.addModalForm.$valid && vnm.newValue.value  && ((vnm.newValue.field.field_type != 'M' &&vnm.newValue.constant) || (vnm.newValue.field.field_type == 'M'))) {
                ...
                if (vnm.editMode) {
                   var dto = {
                        ...
                        parameters: {
                            ...
                            constant: vnm.newValue.constant,
                ...
                QueryDAO.execute(dto).then(function (res) {
                    ...
                    console.log(error);
                    if(error.status===400){
                        toaster.pop('error', 'Error', 'Constant must be unique');
                    }else{
            ...
            } else if (vnm.addModalForm.$valid && (!vnm.newValue.value || !vnm.newValue.constant)) {
