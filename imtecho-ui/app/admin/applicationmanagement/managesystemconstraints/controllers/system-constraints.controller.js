        ...
        ctrl.downloadFieldConfigurations = (form) => {
            ...
            QueryDAO.executeAll(queryList).then((response) => {
                ...
                ctrl.configuredQuery = ctrl.configuredQuery.concat(
                    ...
                    values (cast('${form.uuid}' as uuid), ${form.formName ? "'" + form.formName.replace(/'/g, "''") + "'" : null}, ${form.formCode ? "'" + form.formCode.replace(/'/g, "''") + "'" : null}, ${form.menuConfigId ? form.menuConfigId : null}, ${form.webState ? "'" + form.webState.replace(/'/g, "''") + "'" : null}, ${form.createdBy ? form.createdBy : null},now(),${form.modifiedBy ? form.modifiedBy : null},now(), ${form.mobileState ? "'" + form.mobileState.replace(/'/g, "''") + "'" : null})
                    on conflict(uuid) do nothing;\n\n\n`
                );
                ctrl.configuredQuery = ctrl.configuredQuery.concat(
                    `update system_constraint_form_master
                    set web_template_config = ${form.webTemplateConfig ? "'" + form.webTemplateConfig.replace(/'/g, "''") + "'" : null}
                    where uuid = cast('${form.uuid}' as uuid);\n\n\n`
                );
                ctrl.configuredQuery = ctrl.configuredQuery.concat(
                    ...
                    if (Array.isArray(translations) && translations.length && !ctrl.withoutTranslationLabelComponents.includes(fieldMaster.field_type)) {
                        ...
                        translations.forEach((translation) => {
                            ctrl.configuredQuery = ctrl.configuredQuery.concat(
                                ...
                                values('${translation.country}','${translation.key}','${translation.language}','${translation.created_by}',now(),${translation.custom3b},${translation.text ? "'" + translation.text.replace(/'/g, "''") + "'" : null},${translation.translation_pending},now(),'${translation.app_name}');\n\n`
                            );
                        });
                    }
                });
