(function () {
    function MedplatFormMobileLayout(QueryDAO, $state, toaster, Mask, GeneralUtil, MedplatFormService, $q) {
        let ctrl = this;
        ...
        const _init = function () {
            ...
            if (ctrl.editMode) {
                Mask.show();
                MedplatFormService.getSystemConstraintFormConfigByUuid(ctrl.formMasterUuid).then(responses => {
        ...
        ctrl.saveConfiguration = () => {
            ...
            if (ctrl.mobileTemplateConfigForm.$valid) {
                ...
                Mask.show();
                MedplatFormService.createOrUpdateSystemConstraintForm(ctrl.systemConstraintFormMasterDto,'MOBILE',).then(() => {
                    ctrl.goBack();
        ...
        $(window).on("scroll", function () {
            $("#allFields").stop().animate({ "marginTop": ($(window).scrollTop()-50) + "px", "marginLeft": ($(window).scrollLeft()) + "px" }, "slow");
        });

        _init();
    }
    angular.module('imtecho.controllers').controller('MedplatFormMobileLayout', MedplatFormMobileLayout);
})();
