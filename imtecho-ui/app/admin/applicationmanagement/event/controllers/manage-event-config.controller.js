    ...
    function EventConfigurationController(NotificationDAO, $state, Mask, GeneralUtil, toaster, $uibModal, $q, EventConfigDAO, FormDAO, QueryDAO) {
        ...
        eventConfig.addConfiguration = function () {
            if (eventConfig.eventObj && eventConfig.eventObj.notificationConfigDetails && isFormValid()) {
                ...
                eventConfig.eventObj.notificationConfigDetails.push({ implicitParam: ['#resourceId#'] });
            }
        };
        ...
        var init = function () {
            ...
            FormDAO.retrieveAll({ is_active: true }).then(function (res) {
                ...
                EventConfigDAO.retrieveManualEvents().then(function (manualEvents) {
                    ...
                    if ($state.params.id) {
                        EventConfigDAO.retrieveByUUID($state.params.id).then(function (event) {
        ...
        eventConfig.setImplicitParam = function (config) {
            ...
            if(config.query && config.query!=undefined){
                const fromCount = (config.query.match(fromPattern) || []).length;
                const selectCount = (config.query.match(selectPattern) || []).length;
                if (config.query && selectCount > 0){
                    if (fromCount > 1 && (fromCount == selectCount)) {
                            var namedParameters = config.query.substring(6,config.query.search(/\bfrom\b(?!(.|\n)*(\bfrom\b))/i)).match(/(\w*\(.*?\) as \w*|[^\(\),]+)(?=\s*,|\s*$)/g);
                    } else if(fromCount != selectCount) {
                            var namedParameters = config.query.substring(6).match(/(\w*\(.*?\) as \w*|[^\(\),]+)(?=\s*,|\s*$)/gi);
                    } else if(fromCount == 1 && (fromCount == selectCount)){
                            var namedParameters = (config.query.substring(6, config.query.toLowerCase().indexOf("from"))).match(/(\w*\(.*?\) as \w*|[^\(\),]+)(?=\s*,|\s*$)/gi);
                    } else{
                        var namedParameters = config.query.substring(6).match(/(\w*\(.*?\) as \w*|[^\(\),]+)(?=\s*,|\s*$)/gi);
                    }
                    let finalParameters = namedParameters.filter(parameter => parameter.trim().length > 0)
                    _.each(finalParameters, function (parameter) {
                        let tempParameter;
                        tempParameter = parameter;
                        parameter = parameter.trim(); 
                        if(parameter.toLowerCase().indexOf('as') == 0){
                        ...
                        if (parameter != '*') {
                            var aliasIndex = parameter.toLowerCase().lastIndexOf(" as ");
                            if (aliasIndex >= 0) {
